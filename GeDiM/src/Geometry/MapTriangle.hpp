...
class MapTriangle
{
  ...
  private:
    /// Matrix BMatrix for linear map x = BMatrix * x_r + b from reference triangle [0,1]x[0,1]/2 to triangle with x
    /// points vertices the triangle to map vertices, size 3 x 3 return the resulting value, size 3 x 3
    static inline Eigen::Matrix3d BMatrix(const Eigen::Matrix3d &vertices)
    {
        Eigen::Matrix3d B;
