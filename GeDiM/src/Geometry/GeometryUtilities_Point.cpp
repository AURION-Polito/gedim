...
{
// ***************************************************************************
Eigen::VectorXd GeometryUtilities::PointDistances(const Eigen::MatrixXd &points, const Eigen::Vector3d &point) const
{
...
Eigen::MatrixXd GeometryUtilities::PointsDistance(const Eigen::MatrixXd &points) const
{
    ...
    Eigen::MatrixXd distances = Eigen::MatrixXd::Constant(numPoints, numPoints, -1.0);
...
// ***************************************************************************
vector<unsigned int> GeometryUtilities::FindPointInPoints(const Eigen::MatrixXd &points, const Eigen::Vector3d &point) const
{
    VectorXd pointDistances = PointDistances(points, point);
                                                                                     ...
                                                                                     const Vector3d &segmentEnd) const
{
    ...
    Output::Assert(IsValueZero(segmentTangent.z(), Tolerance1D()) && IsValueZero(pointDirection.z(), Tolerance1D()));
...
// ***************************************************************************
Eigen::MatrixXi GeometryUtilities::MakeConcatenation(const Eigen::MatrixXi &segments, const unsigned int starting_vertex) const
{
    ...
    while (num_visited < num_segments && num_taken < num_segments)
    {
                                                                                                 ...
                                                                                                 const Eigen::MatrixXd &polygonVertices) const
{
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        ...
        const Vector3d edgeTangent = SegmentTangent(edgeOrigin, edgeEnd);

        if (IsValueZero(edgeTangent.y(), Tolerance1D()))
        {
            if (IsValueZero(point.y() - edgeOrigin.y(), Tolerance1D()))
            {
                const double intersection_point_curvilinearCoordinate =
                    PointLineCurvilinearCoordinate(point, edgeOrigin, edgeTangent, edgeTangent.squaredNorm());
                const PointSegmentPositionTypes intersection_point_position =
                    PointSegmentPosition(intersection_point_curvilinearCoordinate);
        ...
        case PointSegmentPositionTypes::InsideSegment:
        case PointSegmentPositionTypes::OnSegmentEnd: {
            const double x_intersection = edgeOrigin.x() + intersection_edge_curvilinearCoordinate * edgeTangent.x();
    ...
    const vector<Eigen::Matrix3d> &polyhedronFaceRotationMatrices) const
{
    ...
    for (unsigned int f = 0; f < polyhedronFaces.size(); f++)
    {
        ...
        const PointPlanePositionTypes pointFacePlanePosition = PointPlanePosition(
            PointPlaneDistance(point, faceOutgoingNormal * polyhedronFaceNormals[f], faceVertices3D.col(0)));
    ...
    const std::vector<Eigen::MatrixXd> &polyhedron_tetrahedrons) const
{
    ...
    for (unsigned int f = 0; f < polyhedron_faces.size(); f++)
    {
        ...
        const PointPlanePositionTypes pointFacePlanePosition = PointPlanePosition(
            PointPlaneDistance(point, faceOutgoingNormal * polyhedron_faces_normals[f], faceVertices3D.col(0)));
...
// ***************************************************************************
bool GeometryUtilities::IsPointInsideTetrahedron(const Eigen::MatrixXd &tetrahedron, const Eigen::Vector3d &point) const
{
    MapTetrahedron map_tetra(*this);
    const auto map_data = map_tetra.Compute(tetrahedron);

    const Eigen::Vector3d ref_point = map_tetra.FInv(map_data, point);
...
{
    CompareTypes comparison = CompareValues(circleRadius, PointDistance(circleCenter, point), Tolerance1D());
                                                                                             ...
                                                                                             const double &circleRadius) const
{
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        positions[v] = PointCirclePosition(points.col(v), circleCenter, circleRadius);
    }
...
vector<unsigned int> GeometryUtilities::UnalignedPoints(const Eigen::MatrixXd &points, const unsigned int numDesiredUnalignedPoints) const
{
    ...
    for (unsigned int p = 0; p < numPoints; p++)
    {
        ...
        if (!PointIsAligned(segmentOrigin, segmentEnd, nextPoint))
            unalignedPoints.push_back(p);

        if (numDesiredUnalignedPoints > 0 && unalignedPoints.size() == numDesiredUnalignedPoints)
            break;
    }
