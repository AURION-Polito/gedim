                                                                                                  ...
                                                                                                  const Vector3d &secondSegmentEnd) const
{
    ...
    // Check non-coplanarity of segments: (x3-x1) != 0 && (x4-x1) != 0 && (x3-x2) != 0 && (x4-x2) != 0 && nDotRhs != 0
    if (!AreLineCoplanar(firstSegmentOrigin, t1, secondSegmentOrigin, t2))
    {
        // segments are not on the same plane
    ...
    // segments are not parallel if squared norm of cross product ||t1 x t2||^2 / (||t1||^2 * ||t2||^2) = 1.0 -
        ...
        IsValuePositive((1.0 - tangentsDot) * (1.0 + tangentsDot), Tolerance1DSquared() * Tolerance1DSquared()))
    {
        ...
        // Vector2d resultParametricCoordinates = matrixTangentVector.bdcSvd(ComputeFullU |
        // ComputeFullV).solve(rightHandSide);
        ...
        if ((result.FirstSegmentIntersections[0].Type == PointSegmentPositionTypes::OnSegmentOrigin ||
            ...
            result.IntersectionSegmentsType =
                GeometryUtilities::IntersectionSegmentSegmentResult::IntersectionSegmentTypes::SingleIntersection;
        else
            result.IntersectionSegmentsType =
                GeometryUtilities::IntersectionSegmentSegmentResult::IntersectionSegmentTypes::NoIntersection;
    }
    else
    {
        ...
        if (!PointIsAligned(firstSegmentOrigin, firstSegmentEnd, secondSegmentOrigin) &&
            !PointIsAligned(firstSegmentOrigin, firstSegmentEnd, secondSegmentEnd))
        {
        ...
        else
        {
            ...
            result.IntersectionSegmentsType =
                GeometryUtilities::IntersectionSegmentSegmentResult::IntersectionSegmentTypes::MultipleIntersections;
            ...
            result.FirstSegmentIntersections[0].CurvilinearCoordinate =
                PointCurvilinearCoordinate(secondSegmentOrigin, firstSegmentOrigin, firstSegmentEnd);
            result.FirstSegmentIntersections[1].CurvilinearCoordinate =
                PointCurvilinearCoordinate(secondSegmentEnd, firstSegmentOrigin, firstSegmentEnd);

            result.SecondSegmentIntersections[0].CurvilinearCoordinate =
                PointCurvilinearCoordinate(firstSegmentOrigin, secondSegmentOrigin, secondSegmentEnd);
            result.SecondSegmentIntersections[1].CurvilinearCoordinate =
                PointCurvilinearCoordinate(firstSegmentEnd, secondSegmentOrigin, secondSegmentEnd);
            ...
            // Check intersection position
            result.FirstSegmentIntersections[0].Type =
                PointSegmentPosition(result.FirstSegmentIntersections[0].CurvilinearCoordinate);
            result.FirstSegmentIntersections[1].Type =
                PointSegmentPosition(result.FirstSegmentIntersections[1].CurvilinearCoordinate);
            ...
            result.SecondSegmentIntersections[0].Type =
                PointSegmentPosition(result.SecondSegmentIntersections[0].CurvilinearCoordinate);
            result.SecondSegmentIntersections[1].Type =
                PointSegmentPosition(result.SecondSegmentIntersections[1].CurvilinearCoordinate);
            ...
            if (PointDistance(firstPoint, secondSegmentOrigin + result.SecondSegmentIntersections[0].CurvilinearCoordinate * t2) >
                                                                               ...
                                                                               const std::vector<double> &segmentsLength) const
{
    ...
    for (unsigned int s1 = 0; s1 < numSegments; s1++)
    {
        ...
        for (unsigned int s2 = s1 + 1; s2 < numSegments; s2++)
        {
            ...
            if (CheckNoSpheresIntersection(segmentOneBarycenter, segmentTwoBarycenter, segmentOneLength, segmentTwoLength))
                continue;

            const Gedim::GeometryUtilities::IntersectionSegmentSegmentResult result =
                IntersectionSegmentSegment(segmentOne.col(0), segmentOne.col(1), segmentTwo.col(0), segmentTwo.col(1));
                                                                                              ...
                                                                                              const Eigen::Vector3d &planeOrigin) const
{
    ...
    if (IsValueZero(planeNormal.dot(t.normalized()), Tolerance1DSquared()))
    {
        ...
        if (CompareValues(planeNormal.dot(segmentOrigin), planeNormal.dot(planeOrigin), Tolerance1DSquared()) ==
            GeometryUtilities::CompareTypes::Coincident)
        {
    ...
    else
    {
        ...
        result.SingleIntersection.CurvilinearCoordinate = planeNormal.dot(planeOrigin - segmentOrigin) / planeNormal.dot(t);
...
// ***************************************************************************
GeometryUtilities::IntersectionPolyhedronPlaneResult GeometryUtilities::IntersectionPolyhedronPlane(
    ...
    const Eigen::Vector3d &planeTranslation) const
{
    ...
    {
        const Eigen::Vector3d vertex = polyhedronVertices.col(v);

        auto &vertex_intersection = result.VertexIntersections.at(v);
        vertex_intersection.PlanePosition = PointPlanePosition(PointPlaneDistance(vertex, planeNormal, planeOrigin));
        switch (vertex_intersection.PlanePosition)
        {
        case PointPlanePositionTypes::Negative:
        case PointPlanePositionTypes::Positive:
            vertex_intersection.Type =
        ...
        case PointPlanePositionTypes::OnPlane: {
            ...
            intersectionCoordinates.push_back(vertex);
            ...
            intersection.VertexId = v;
        }
        break;
        default:
            throw std::runtime_error("Unknown vertex plane position");
        }
    }

    for (unsigned int e = 0; e < numPolyhedronEdges; e++)
    {
        auto &edge_intersection = result.EdgeIntersections[e];
        ...
        const auto &origin_intersection = result.VertexIntersections.at(edgeOriginId);
        const auto &end_intersection = result.VertexIntersections.at(edgeEndId);

        if (origin_intersection.Type == Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Unknown ||
            end_intersection.Type == Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Unknown)
            throw std::runtime_error("Unknown edge plane position");

        if (origin_intersection.Type == Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection &&
            end_intersection.Type == Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection)
        {
            edge_intersection.Intersection.Type = Gedim::GeometryUtilities::IntersectionSegmentPlaneResult::Types::MultipleIntersections;
        }
        else if (origin_intersection.PlanePosition == end_intersection.PlanePosition)
        {
            edge_intersection.Intersection.Type = Gedim::GeometryUtilities::IntersectionSegmentPlaneResult::Types::NoIntersection;
        }
        else
        {
            edge_intersection.Intersection =
                GeometryUtilities::IntersectionSegmentPlane(edgeOrigin, edgeEnd, planeNormal, planeOrigin);
        }

        switch (edge_intersection.Intersection.Type)
        {
        ...
        case Gedim::GeometryUtilities::IntersectionSegmentPlaneResult::Types::SingleIntersection: {
            switch (edge_intersection.Intersection.SingleIntersection.Type)
            {
            case Gedim::GeometryUtilities::PointSegmentPositionTypes::OnSegmentOrigin: {
                Gedim::Output::Assert(origin_intersection.Type ==
                                      Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection);
            }
            break;
            case Gedim::GeometryUtilities::PointSegmentPositionTypes::OnSegmentEnd: {
                Gedim::Output::Assert(end_intersection.Type ==
                                      Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection);
            }
            break;
            case Gedim::GeometryUtilities::PointSegmentPositionTypes::InsideSegment: {
                ...
                intersectionsList.push_back(GeometryUtilities::IntersectionPolyhedronPlaneResult::Intersection());
                intersectionCoordinates.push_back(edgeOrigin + edge_intersection.Intersection.SingleIntersection.CurvilinearCoordinate *
                                                                   (edgeEnd - edgeOrigin));
                GeometryUtilities::IntersectionPolyhedronPlaneResult::Intersection &intersection = intersectionsList.back();
    ...
    case 1: {
        ...
        for (unsigned int v = 0; v < numPolyhedronVertices; v++)
        {
            if (result.VertexIntersections[v].Type !=
                Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection)
                continue;
    ...
    default: {
        ...
        for (unsigned int f = 0; f < numPolyhedronFaces; f++)
        {
            ...
            {
                if (result.VertexIntersections[polyhedronFaces[f](0, v)].Type !=
                    Gedim::GeometryUtilities::IntersectionPolyhedronPlaneResult::VertexIntersection::Types::Intersection)
                {
            ...
            {
                if (result.EdgeIntersections[polyhedronFaces[f](1, e)].Intersection.Type !=
            ...
            Gedim::Output::Assert(faceEdgesIntersection);

            result.FaceIntersections[f].Type =
        ...
        else
        {
            ...
            vector<GeometryUtilities::IntersectionPolyhedronPlaneResult::Intersection> intersections(intersectionsList.begin(),
                                                                                                     intersectionsList.end());
...
// ***************************************************************************
GeometryUtilities::IntersectionPolyhedronLineResult GeometryUtilities::IntersectionPolyhedronLine(
    const Eigen::MatrixXd &polyhedronVertices,
    const Eigen::MatrixXi &polyhedronEdges,
    const vector<Eigen::MatrixXi> &polyhedronFaces,
    ...
    const Eigen::Vector3d &lineOrigin) const
{
    ...
    for (unsigned int i = 0; i < numPolyhedronVertices; i++)
    {
        ...
        {
            result.PolyhedronVertexIntersections[i].Type =
                IntersectionPolyhedronLineResult::PolyhedronVertexIntersection::Types::Intersection;
            result.PolyhedronVertexIntersections[i].LineIntersectionIndex = numberOfIntersections;

            // calcolo la coordinata curvilinea come rapporto tra la distanza del vertice dall'origine della retta, e la
            // lunghezza della tangente alla retta
            Vector3d origin;
            origin << 0.0, 0.0, 0.0;
            ...
            result.LineIntersections[numberOfIntersections].PolyhedronType =
                IntersectionPolyhedronLineResult::LineIntersection::Types::OnVertex;
    ...
    for (unsigned int i = 0; i < numPolyhedronEdges; i++)
    {
        ...
        IntersectionSegmentSegmentResult r = IntersectionSegmentSegment(edgeOrigin, edgeEnd, lineOrigin, s2);
        ...
        if (r.IntersectionSegmentsType == IntersectionSegmentSegmentResult::IntersectionSegmentTypes::SingleIntersection)
        {
            ...
            if (r.SecondSegmentIntersections[0].Type == PointSegmentPositionTypes::InsideSegment && intGiaPresente == false)
            {
                ...
                result.LineIntersections[numberOfIntersections].PolyhedronType =
                    IntersectionPolyhedronLineResult::LineIntersection::Types::OnEdge;
                result.LineIntersections[numberOfIntersections].PolyhedronIndex = i;
                coordCurv.push_back(max * c);

                result.PolyhedronEdgeIntersections[i].Type =
                    IntersectionPolyhedronLineResult::PolyhedronEdgeIntersection::Types::Intersection;
    ...
    for (unsigned int i = 0; i < numPolyhedronFaces; i++)
    {
        ...
        const Vector3d planeNormal = polyhedronFaceNormalDirections[i] ? polyhedronFaceNormals[i] : -polyhedronFaceNormals[i];

        IntersectionSegmentPlaneResult r = IntersectionSegmentPlane(lineOrigin, s2, planeNormal, planeOrigin);
        // se c'è intersezione con il piano contenente la faccia
        if (r.Type == IntersectionSegmentPlaneResult::Types::SingleIntersection ||
            r.Type == IntersectionSegmentPlaneResult::Types::MultipleIntersections)
        {
            Vector3d inters;
            double c;
            if (r.Type == IntersectionSegmentPlaneResult::Types::SingleIntersection)
            ...
            PointPlanePositionTypes type = PointPlanePosition(PointPlaneDistance(inters, planeNormal, planeOrigin));
            ...
            if (flag && !intGiaPresente)
            {
                ...
                coordCurv.push_back(max * c);
                result.LineIntersections[numberOfIntersections].PolyhedronType =
                    IntersectionPolyhedronLineResult::LineIntersection::Types::OnFace;
                result.LineIntersections[numberOfIntersections].PolyhedronIndex = i;

                result.PolyhedronFaceIntersections[i].Type =
                    IntersectionPolyhedronLineResult::PolyhedronFaceIntersection::Types::Intersection;
...
// ***************************************************************************
GeometryUtilities::IntersectionPolyhedronLineResult GeometryUtilities::IntersectionPolyhedronSegment(
    const Eigen::MatrixXd &polyhedronVertices,
    const Eigen::MatrixXi &polyhedronEdges,
    const vector<Eigen::MatrixXi> &polyhedronFaces,
    ...
    const IntersectionPolyhedronLineResult &polyhedronLineIntersections) const
{
    ...
    {
        if ((CompareValues(polyhedronLineIntersections.LineIntersections[i].CurvilinearCoordinate, 1, Tolerance1D()) ==
                 CompareTypes::FirstBeforeSecond ||
             CompareValues(polyhedronLineIntersections.LineIntersections[i].CurvilinearCoordinate, 1, Tolerance1D()) ==
                 CompareTypes::Coincident) &&
            (CompareValues(polyhedronLineIntersections.LineIntersections[i].CurvilinearCoordinate, 0, Tolerance1D()) ==
                 CompareTypes::SecondBeforeFirst ||
             CompareValues(polyhedronLineIntersections.LineIntersections[i].CurvilinearCoordinate, 0, Tolerance1D()) ==
                 CompareTypes::Coincident))
        {
            result.LineIntersections[numIntersSegm].CurvilinearCoordinate =
                polyhedronLineIntersections.LineIntersections[i].CurvilinearCoordinate;
            ...
            if (result.LineIntersections[numIntersSegm].PolyhedronType ==
                IntersectionPolyhedronLineResult::LineIntersection::Types::OnVertex)
            {
                result.PolyhedronVertexIntersections[polIndex].Type =
                    IntersectionPolyhedronLineResult::PolyhedronVertexIntersection::Types::Intersection;
                result.PolyhedronVertexIntersections[polIndex].LineIntersectionIndex = numIntersSegm;
            }
            else if (result.LineIntersections[numIntersSegm].PolyhedronType ==
                     IntersectionPolyhedronLineResult::LineIntersection::Types::OnEdge)
            {
                result.PolyhedronEdgeIntersections[polIndex].Type =
                    IntersectionPolyhedronLineResult::PolyhedronEdgeIntersection::Types::Intersection;
                result.PolyhedronEdgeIntersections[polIndex].LineIntersectionIndex = numIntersSegm;
                     ...
                     IntersectionPolyhedronLineResult::LineIntersection::Types::OnFace)
            {
                result.PolyhedronFaceIntersections[polIndex].Type =
                    IntersectionPolyhedronLineResult::PolyhedronFaceIntersection::Types::Intersection;
    ...
    if (numIntersLine > numIntersSegm)
    {
            ...
            (CompareValues(segmentOrigin(2), polyhedronVertices(2, 0), Tolerance1D()) == CompareTypes::SecondBeforeFirst))
        {
            ...
            result.LineIntersections[numIntersSegm].PolyhedronType =
                IntersectionPolyhedronLineResult::LineIntersection::Types::Inside;
    ...
    const Eigen::Vector3d &segmentTangent) const
{
    ...
    vector<vector<unsigned int>> celleInComune; // prima riga per il primo segmento, seconda riga per il secondo
                                                // segmento
    ...
    for (int i = 0; i < numCelle; i++)
    {
        IntersectionPolyhedronLineResult rl = IntersectionPolyhedronLine(polyhedrons[i].Vertices,
                                                                         ...
                                                                         segmentTangent,
                                                                         segmentOrigin);
        IntersectionPolyhedronLineResult rs =
            IntersectionPolyhedronSegment(polyhedrons[i].Vertices, polyhedrons[i].Edges, polyhedrons[i].Faces, segmentOrigin, segmentEnd, segmentTangent, rl);
        ...
        {
            inters = 2;
            celleInComune[dueInters].push_back(i); // se ho più di un'intersezione vuol dire che c'è un segmento in
                                                   // quella cella
            dueInters++;
        }
        else
        ...
        if (inters >= 1)
        {
            for (unsigned int j = 0; j < coordCurv.size(); j++)
            {
                ...
                {
                    if (CompareValues(coordCurv[j], rs.LineIntersections[k].CurvilinearCoordinate, Tolerance1D()) == CompareTypes::Coincident)
                    {
                        flag[k] = true;
                        cells[j].push_back(i);
                    }
            ...
            for (int k = 0; k < inters; k++)
            {
                if (flag[k] == false)
                {
                    if (CompareValues(rs.LineIntersections[k].CurvilinearCoordinate, 1.0, Tolerance1D()) == CompareTypes::Coincident)
                                                                                                ...
                                                                                                const double &circleRadius) const
{
    ...
    for (unsigned int e = 0; e < numEdges; e++)
    {
        ...
        IntersectionSegmentCircleResult intersection = IntersectionSegmentCircle(edgeOrigin, edgeEnd, circleCenter, circleRadius);
    ...
    result.Intersections = vector<IntersectionPolygonCircleResult::Intersection>(intersections.begin(), intersections.end());
    return result;
}
// ***************************************************************************
} // namespace Gedim
