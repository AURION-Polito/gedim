...
namespace Gedim
{
  ...
  {
    Output::Assert(static_cast<unsigned int>(polygonVertices.cols()) == static_cast<unsigned int>(heightVectors.size()) &&
    ...
    for (unsigned int v = 0; v < numPolygonVertices; v++)
    {
      ...
      polyhedron.Faces[2 + v].row(0) << v, (v + 1) % numPolygonVertices,
          numPolygonVertices + (v + 1) % numPolygonVertices, numPolygonVertices + v;
      polyhedron.Faces[2 + v].row(1) << v, 2 * numPolygonVertices + (v + 1) % numPolygonVertices,
          numPolygonVertices + v, 2 * numPolygonVertices + v;
    }
                                                               ...
                                                               const VectorXd &referenceQuadratureWeights) const
  {
    ...
    for (unsigned int f = 0; f < numFaces; f++)
    {
      ...
      for (unsigned int t = 0; t < numFaceTriangles; t++)
      {
        ...
        const MatrixXd faceWeightsTimesNormal =
        ...
        volume += 1.0 / 3.0 *
                  (faceQuadraturePoints.row(0) * faceWeightsTimesNormal.col(0) +
                   faceQuadraturePoints.row(1) * faceWeightsTimesNormal.col(1) +
                   faceQuadraturePoints.row(2) * faceWeightsTimesNormal.col(2))
                  .sum();
      }
    }
                                                               ...
                                                               const Eigen::VectorXd &referenceQuadratureWeights) const
  {
    ...
    const unsigned int numQuadraturePoints = numPolyhedronTetra * numReferenceQuadraturePoints;
    ...
    for (unsigned int t = 0; t < numPolyhedronTetra; t++)
    {
      ...
      quadratureWeights.segment(numReferenceQuadraturePoints * t, numReferenceQuadraturePoints) =
          referenceQuadratureWeights.array() * mapTetra.DetJ(mapData, referenceQuadratureWeights).array().abs();
    }
                                                        ...
                                                        const double &polyhedronVolume) const
  {
    ...
    for (unsigned int f = 0; f < numFaces; f++)
    {
      ...
      for (unsigned int t = 0; t < numFaceTriangles; t++)
      {
        ...
        const Eigen::Matrix3d mappedQuadraturePoints = mapping.F(mapData, quadraturePoints);
        const Eigen::Matrix3d quadraturePoints3D =
  ...
  // ***************************************************************************
  MatrixXd GeometryUtilities::PolyhedronEdgesCentroid(const Eigen::MatrixXd &polyhedronVertices, const Eigen::MatrixXi &polyhedronEdges) const
  {
    ...
    {
      edgesCentroid.col(e) =
          SegmentBarycenter(polyhedronVertices.col(polyhedronEdges(0, e)), polyhedronVertices.col(polyhedronEdges(1, e)));
    }

    return edgesCentroid;
  }
  // ***************************************************************************
  VectorXd GeometryUtilities::PolyhedronEdgesLength(const Eigen::MatrixXd &polyhedronVertices, const Eigen::MatrixXi &polyhedronEdges) const
  {
    ...
    {
      edgesLength[e] =
          SegmentLength(polyhedronVertices.col(polyhedronEdges(0, e)), polyhedronVertices.col(polyhedronEdges(1, e)));
    }

    return edgesLength;
  }
  // ***************************************************************************
  MatrixXd GeometryUtilities::PolyhedronEdgeTangents(const Eigen::MatrixXd &polyhedronVertices, const Eigen::MatrixXi &polyhedronEdges) const
  {
    ...
    {
      edgeTangents.col(e) =
          SegmentTangent(polyhedronVertices.col(polyhedronEdges(0, e)), polyhedronVertices.col(polyhedronEdges(1, e)));
    }
                                                                 ...
                                                                 const Eigen::MatrixXd &polyhedronEdgeTangents) const
  {
    ...
    for (unsigned int f = 0; f < polyhedronFaces.size(); f++)
    {
      ...
      for (unsigned int e = 0; e < faceEdgesIndices.size(); e++)
        faceEdgeTangents.col(e)
            << (polyhedronFaceEdgeDirections[f][e] ? 1.0 : -1.0) * polyhedronEdgeTangents.col(faceEdgesIndices[e]);
    }
  ...
  // ***************************************************************************
  std::vector<std::array<Eigen::Vector3d, 2>> GeometryUtilities::PolyhedronFaceTangents(
      const std::vector<Eigen::MatrixXd> &polyhedronFacesVertices,
      ...
      const std::vector<bool> &polyhedronFacesNormalDirection) const
  {
    ...
    {
      const double normal_direction = polyhedronFacesNormalDirection.at(f) ? +1.0 : -1.0;
      facesTangents.push_back(PolygonTangents(polyhedronFacesVertices.at(f), normal_direction * polyhedronFacesNormal.at(f)));
    }
  ...
  // ***************************************************************************
  Matrix3d GeometryUtilities::PolyhedronMass(const Eigen::Vector3d &polyhedronCentroid,
                                             const std::vector<MatrixXd> &polyhedronTetrahedronPoints) const
  {
    ...
    // Create Polyhedron quadrature points
    const unsigned int numPolyhedronTetrahedrons = polyhedronTetrahedronPoints.size();

    const unsigned int numTetrahedronQuadraturePoints = referencePoints.cols();
    const unsigned int numPolyhedronQuadraturePoints = numPolyhedronTetrahedrons * numTetrahedronQuadraturePoints;
    ...
    {
      const Eigen::MatrixXd &tetrahedronVertices = polyhedronTetrahedronPoints[t];

      Gedim::MapTetrahedron mapTetrahedron(*this);
      MapTetrahedron::MapTetrahedronData mapTetrahedronData = mapTetrahedron.Compute(tetrahedronVertices);
      polyhedronQuadraturePoints.block(0, numTetrahedronQuadraturePoints * t, 3, numTetrahedronQuadraturePoints) =
          mapTetrahedron.F(mapTetrahedronData, referencePoints);
      polyhedronQuadratureWeights.segment(numTetrahedronQuadraturePoints * t, numTetrahedronQuadraturePoints) =
          referenceWeights.array() * mapTetrahedron.DetJ(mapTetrahedronData, referencePoints).array().abs();
    }

    // Compute Mass Matrix
    Eigen::Matrix3d mass = Eigen::Matrix3d::Zero();

    mass(0, 0) = ((polyhedronQuadraturePoints.row(0).array() - polyhedronCentroid.x()).square() *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();
    mass(1, 1) = ((polyhedronQuadraturePoints.row(1).array() - polyhedronCentroid.y()).square() *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();
    mass(2, 2) = ((polyhedronQuadraturePoints.row(2).array() - polyhedronCentroid.z()).square() *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();

    mass(0, 1) = ((polyhedronQuadraturePoints.row(0).array() - polyhedronCentroid.x()) *
                  (polyhedronQuadraturePoints.row(1).array() - polyhedronCentroid.y()) *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();
    mass(1, 0) = mass(0, 1);

    mass(0, 2) = ((polyhedronQuadraturePoints.row(0).array() - polyhedronCentroid.x()) *
                  (polyhedronQuadraturePoints.row(2).array() - polyhedronCentroid.z()) *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();
    mass(2, 0) = mass(0, 2);

    mass(1, 2) = ((polyhedronQuadraturePoints.row(1).array() - polyhedronCentroid.y()) *
                  (polyhedronQuadraturePoints.row(2).array() - polyhedronCentroid.z()) *
                  polyhedronQuadratureWeights.transpose().array())
                 .sum();
    mass(2, 1) = mass(1, 2);
                                                ...
                                                const std::vector<Eigen::MatrixXd> &polyhedronTetrahedraPoints) const
  {
    ...
    const unsigned int numPolyhedronQuadraturePoints = numPolyhedronTetra * numTetraQuadraturePoints;
    ...
    inertia(0, 0) = (((polyhedronQuadraturePoints.row(1).array() - polyhedronCentroid.y()).square() +
                      (polyhedronQuadraturePoints.row(2).array() - polyhedronCentroid.z()).square()) *
                     polyhedronQuadratureWeights.transpose().array())
                    .sum();
    inertia(1, 1) = (((polyhedronQuadraturePoints.row(0).array() - polyhedronCentroid.x()).square() +
                      (polyhedronQuadraturePoints.row(2).array() - polyhedronCentroid.z()).square()) *
                     polyhedronQuadratureWeights.transpose().array())
                    .sum();
    inertia(2, 2) = (((polyhedronQuadraturePoints.row(1).array() - polyhedronCentroid.y()).square() +
                                             ...
                                             const Eigen::Vector3d &polyhedronInternalPoint) const
  {
    ...
    for (unsigned int f1 = 0; f1 < numPolyhedronFaces; f1++)
    {
      ...
      if (!PolygonIsConvex(polyhedronFaceRotatedVertices[f1], ExtractPoints(polyhedronFaceRotatedVertices[f1], faceConvexHull)))
      {
        return false;
      }
                                                                 ...
                                                                 const vector<Vector3d> &polyhedronFaceNormals) const
  {
    ...
    for (unsigned int f = 0; f < polyhedronFaceVertices.size(); f++)
    {
      ...
      const PointPlanePositionTypes pointFacePosition =
          PointPlanePosition(PointPlaneDistance(pointInsidePolyhedron, normal, polyhedronFaceVertices[f].col(0)));
          ...
          case IntersectionSegmentPlaneResult::Types::SingleIntersection: {
            ...
            switch (intersections.SingleIntersection.Type)
            {
              ...
              case PointSegmentPositionTypes::InsideSegment: {
                ...
                {
                  if (AreValuesEqual(curvilinearCoordinate, intersections.SingleIntersection.CurvilinearCoordinate, Tolerance1D()))
                  {
                ...
                const Eigen::Vector3d pointIntersection =
                    segmentOrigin + intersections.SingleIntersection.CurvilinearCoordinate * normal;

                const Eigen::Vector3d pointIntersection2D =
                    RotatePointsFrom3DTo2D(pointIntersection,
                                           polyhedronFaceRotationMatrices[f2].transpose(),
                                           polyhedronFaceTranslations[f2]);
                const PointPolygonPositionResult &intersectionPosition =
                    PointPolygonPosition_RayCasting(pointIntersection2D, polyhedronFaceRotatedVertices[f2]);
  ...
  // ***************************************************************************
  std::vector<std::vector<unsigned int>> GeometryUtilities::PolyhedronFaceTriangulationsByEarClipping(
      const unsigned int numPolyhedronFaces,
  ...
  // ***************************************************************************
  std::vector<std::vector<Matrix3d>> GeometryUtilities::PolyhedronFaceExtractTriangulationPoints(
      ...
      const std::vector<std::vector<unsigned int>> &polyhedronFaceTriangulations) const
  {
    ...
    for (unsigned int f = 0; f < numFaces; f++)
      faceTriangulationsPoints[f] = ExtractTriangulationPoints(polyhedronFaceVertices[f], polyhedronFaceTriangulations[f]);

    return faceTriangulationsPoints;
  }
  // ***************************************************************************
  std::vector<std::vector<Matrix3d>> GeometryUtilities::PolyhedronFaceTriangulationPointsByInternalPoint(
      const vector<Eigen::MatrixXd> &polyhedronFaceVertices,
      const std::vector<Eigen::Vector3d> &polyhedronFaceInternalPoints,
      const std::vector<std::vector<unsigned int>> &polyhedronFaceTriangulations) const
  {
  ...
  // ***************************************************************************
  vector<vector<unsigned int>> GeometryUtilities::PolyhedronFaceTriangulationsByInternalPoint(
      ...
      const vector<Eigen::Vector3d> &polyhedronFaceInternalPoints) const
  {
    ...
    for (unsigned int f = 0; f < polyhedronFaces.size(); f++)
    {
      polyhedronFacesTriangulation[f] =
          PolygonTriangulationByInternalPoint(polyhedronFaceVertices[f], polyhedronFaceInternalPoints[f]);
    }
  ...
  // ***************************************************************************
  vector<unsigned int> GeometryUtilities::PolyhedronCoordinateSystem(const Eigen::MatrixXd &, const Eigen::MatrixXi &polyhedronEdges)
  {
                                                ...
                                                const std::string &exportFolder) const
  {
...

    {
      ...
      exporter.AddPoints(polyhedronVertices,
                         {{"Index",
                           ...
                           verticesIndex.data()}});
      exporter.Export(exportFolder + "/Polyhedron_Vertices.vtu");
    }

    {
      ...
      exporter.AddSegments(
            polyhedronVertices,
            polyhedronEdges,
            {{"Index", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(edgesIndex.size()), edgesIndex.data()}});
      exporter.Export(exportFolder + "/Polyhedron_Edges.vtu");
    }

    {
      ...
      exporter.AddPolygons(
            polyhedronVertices,
            facesVertices,
            {{"Index", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(facesIndex.size()), facesIndex.data()}});
      exporter.Export(exportFolder + "/Polyhedron_Faces.vtu");
    }

    {
      // export polyhedron
      VTKUtilities exporter;
      exporter.AddPolyhedron(polyhedronVertices, facesVertices);
                                                ...
                                                const std::string &exportFolder) const
  {
...

    {
      ...
      exporter.Export(exportFolder + "/" + "Polyhedron_Tetra.vtu");
    }
...

    {
      ...
      for (unsigned int f = 0; f < polyhedronFaces3DTriangles.size(); f++)
      {
        ...
        for (unsigned int t = 0; t < polyhedronFaces3DTriangles[f].size(); t++)
        {
          ...
          exporter.AddPolygon(
                polyhedronFaces3DTriangles[f][t],
                {{"Face", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(face.size()), face.data()},
                 {"Id", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(id.size()), id.data()}});
        }
      }

      exporter.Export(exportFolder + "/" + "Polyhedron_FacesTriangles.vtu");
    }

    {
      ...
      for (unsigned int f = 0; f < polyhedronFaces3DNormal.size(); f++)
      {
        ...
        exporter.AddSegment(polyhedronFaces3DInternalPoint[f],
                            polyhedronFaces3DInternalPoint[f] + normalDirection[0] * polyhedronFaces3DNormal[f],
        {{"Face", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(face.size()), face.data()},
          {"NormalDirection",
            Gedim::VTPProperty::Formats::Cells,
      ...
      exporter.Export(exportFolder + "/" + "Polyhedron_FacesNormal.vtu");
    }
...

    {
      ...
      exporter.Export(exportFolder + "/" + "Polyhedron_FacesCentroid.vtu");
    }
  }
  // ***************************************************************************
  std::vector<unsigned int> GeometryUtilities::UnalignedPolyhedronPoints(
      const Eigen::MatrixXd &polyhedronVertices,
      const std::vector<Eigen::MatrixXi> &polyhedronFaces,
      const std::vector<Eigen::Vector3d> &polyhedronFacesTranslation,
      const std::vector<Eigen::Matrix3d> &polyhedronFacesRotationMatrix,
    ...
    for (const std::vector<unsigned int> &faces : polyhedronUnaligedFaces)
    {
      ...
      if (numAlignedFaces > 1)
      {
        ...
        for (unsigned int f = 0; f < numAlignedFaces; f++)
          numFacesUnalignedVertices[f + 1] +=
              numFacesUnalignedVertices[f] + polyhedronFacesUnalignedVertices[faces[f]].size();
        ...
        for (unsigned int f = 0; f < numAlignedFaces; f++)
        {
          ...
          for (unsigned int v = 0; v < polyhedronFacesUnalignedVertices[polyhedronFace].size(); v++)
          {
            const unsigned int vertexIndex =
                polyhedronFaces[polyhedronFace](0, polyhedronFacesUnalignedVertices[polyhedronFace][v]);
            unifiedUnalignedFacesVertices.col(numFacesUnalignedVertices[f] + v) = polyhedronVertices.col(vertexIndex);
          }
        }

        const Eigen::MatrixXd unifiedUnalignedFacesVertices2D =
            RotatePointsFrom3DTo2D(unifiedUnalignedFacesVertices,
                                   polyhedronFacesRotationMatrix[faces[0]].transpose(),
            polyhedronFacesTranslation[faces[0]]);

        const std::vector<unsigned int> convexHull = ConvexHull(unifiedUnalignedFacesVertices2D, false);

        for (const unsigned int convexHullVertex : convexHull)
        {
          unsigned int f = 0;
          while (f < numAlignedFaces && convexHullVertex >= numFacesUnalignedVertices[f + 1])
            f++;
          ...
          if (unalignedVertices.find(polyhedronVertex) == unalignedVertices.end())
      ...
      else
      {
        ...
        for (const unsigned int unalignedFaceLocalPoint : polyhedronFacesUnalignedVertices[f])
        {
          ...
          if (unalignedVertices.find(unalignedFacePoint) == unalignedVertices.end())
            unalignedVertices.insert(unalignedFacePoint);
        }
      }
    }

    return std::vector<unsigned int>(unalignedVertices.begin(), unalignedVertices.end());
  }
  // ***************************************************************************
  GeometryUtilities::AlignedPolyhedronEdgesResult GeometryUtilities::AlignedPolyhedronEdges(
      const Eigen::MatrixXd &polyhedronVertices,
      ...
      const Eigen::VectorXd &polyhedronEdgeSquaredLenghts) const
  {
    ...
    while (!queue.empty())
    {
      ...
      for (unsigned int av = 0; av < verticesAdjacency[visitedVertex].size(); av++)
      {
        ...
        {
          const std::vector<bool> pointsAreOnLine =
              PointsAreOnLine(edgeCoordinates, alignedEdge.LineOrigin, alignedEdge.LineTangent);

          if (!pointsAreOnLine[0] || !pointsAreOnLine[1])
            continue;
        ...
        else
        {
          ...
          if (find(alignedEdge.Vertices.begin(), alignedEdge.Vertices.end(), visitedVertex) ==
              alignedEdge.Vertices.end())
            alignedEdge.Vertices.push_back(visitedVertex);

          if (find(alignedEdge.Vertices.begin(), alignedEdge.Vertices.end(), adjacentVertex) ==
              alignedEdge.Vertices.end())
            alignedEdge.Vertices.push_back(adjacentVertex);
        }
      }
    ...
    {
      std::vector<unsigned int> alignedVertices(alignedEdge.Vertices.begin(), alignedEdge.Vertices.end());
      ...
      result.AlignedEdgesVertices[alignedEdgeIndex][orderedVerticesIndex.size() - 1] =
          alignedVertices.at(orderedVerticesIndex.at(orderedVerticesIndex.size() - 1));
                                                                      ...
                                                                      const std::vector<std::vector<unsigned int>>& facets) const
  {
    ...
    for (unsigned int f = 0; f < facets.size(); ++f)
    {
      ...
      for (unsigned int f_v = 0; f_v < num_face_vertices; ++f_v)
      {
        ...
        auto edge = std::make_pair(std::min(edge_origin, edge_end),
                                   std::max(edge_origin, edge_end));
