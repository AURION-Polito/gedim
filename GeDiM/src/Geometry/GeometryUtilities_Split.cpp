...
GeometryUtilities::SplitPolygonWithSegmentResult GeometryUtilities::SplitPolygonWithSegment(const GeometryUtilities::SplitPolygonInput &input) const
{
        ...
        input.Segment.End.Type == GeometryUtilities::SplitPolygonInput::SplitSegment::Vertex::Types::Vertex)
    {
        ...
        if ((originIndex + 1) % input.NumberPolygonVertices == endIndex || (endIndex + 1) % input.NumberPolygonVertices == originIndex)
        {
            // No split needed
        ...
        input.Segment.End.Type == GeometryUtilities::SplitPolygonInput::SplitSegment::Vertex::Types::Edge)
    {
        ...
        if (result.Type == SplitPolygonWithSegmentResult::Types::PolygonUpdate)
        {
            ...
            for (unsigned int v = 0; v < newVertices.size(); v++)
            {
                ...
                if (origin < input.NumberPolygonVertices && end < input.NumberPolygonVertices)
                {
                    // old edge
                ...
                else
                {
                    ...
                    if (origin < input.NumberPolygonVertices)
                    {
                        ...
                        newVertexTypes.insert(pair<unsigned int, SplitPolygonWithSegmentResult::NewVertex>(
                            end,
        ...
        do
        {
                ...
                input.Segment.Origin.Index == v)
            {
                ...
                if (input.Segment.End.Type == GeometryUtilities::SplitPolygonInput::SplitSegment::Vertex::Types::Vertex)
                {
                    ...
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices,
                ...
                else
                {
                    ...
                    newVertices.insert(pair<unsigned int, SplitPolygonWithSegmentResult::NewVertex>(
                        input.NumberPolygonVertices,
                        SplitPolygonWithSegmentResult::NewVertex()));
                    ...
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices + 1,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices + 2,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    ...
                    newEdgeOne.Cell2DNeighbours = polygons.size() == 1 ? vector<unsigned int>{1, 0}
                                                                       : vector<unsigned int>{0, 1};
                     ...
                     input.Segment.End.Index == v)
            {
                ...
                else
                {
                    ...
                    newEdgeOne.Type = SplitPolygonWithSegmentResult::NewEdge::Types::EdgeNew;
                    newEdgeOne.Cell2DNeighbours = polygons.size() == 1 ? vector<unsigned int>{0, 1}
                ...
                if (input.Segment.End.Type == GeometryUtilities::SplitPolygonInput::SplitSegment::Vertex::Types::Edge)
                {
                    ...
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices + 2,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices + 3,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    newEdges.insert(
                        pair<unsigned int, SplitPolygonWithSegmentResult::NewEdge>(input.NumberPolygonVertices + 4,
                                                                                   SplitPolygonWithSegmentResult::NewEdge()));
                    SplitPolygonWithSegmentResult::NewEdge &newEdgeOne = newEdges[input.NumberPolygonVertices];
                    newEdgeOne.OriginId = input.NumberPolygonVertices;
    ...
    for (map<unsigned int, SplitPolygonWithSegmentResult::NewEdge>::const_iterator it = newEdges.begin(); it != newEdges.end(); it++)
    {
        result.NewEdges.push_back(SplitPolygonWithSegmentResult::NewEdge());
        SplitPolygonWithSegmentResult::NewEdge &newEdge = result.NewEdges.back();
        newEdge = it->second;
...
// ***************************************************************************
GeometryUtilities::SplitPolygonWithCircleResult GeometryUtilities::SplitPolygonWithCircle(
    ...
    const PolygonCirclePositionTypes &polygonCirclePosition) const
{
    ...
    case PolygonCirclePositionTypes::PolygonOutsideCircleOneIntersectionTangentOnEdge: {
        ...
        throw runtime_error("Case not managed yet. TODO: update the polygon adding the new vertex tangent to the "
                            "circle");
    }
    break;
    ...
    case PolygonCirclePositionTypes::CircleInsidePolygonNoIntersection: {
        ...
        throw runtime_error("Case not managed yet. TODO: split the polygon in sub-triangles with the center of the "
                            "circle and call this function with sub-triangles generated, then unify the results");
    }
    break;
    case PolygonCirclePositionTypes::CirclePolygonMultipleIntersections: {
        ...
        for (unsigned int v = 0; v < numVertices; v++)
        {
                   ...
                   polygonCircleIntersections.Intersections[checkIntersection].Index == v)
            {
                ...
                Output::Assert(intersectionIndexType != IntersectionPolygonCircleResult::Intersection::IndexTypes::Unknown);

                if (intersectionIndexType == IntersectionPolygonCircleResult::Intersection::IndexTypes::Vertex)
                {
                ...
                else if (intersectionIndexType == IntersectionPolygonCircleResult::Intersection::IndexTypes::Edge)
                {
        ...
        result.NewVertices = vector<SplitPolygonWithCircleResult::NewVertex>(newVertices.begin(), newVertices.end());
        ...
        {
            numNewPolygonVertices[c] =
                result.CircleIntersectionsNewVerticesPosition[c + 1] - result.CircleIntersectionsNewVerticesPosition[c] + 1;
        }

        numNewPolygonVertices[numNewPolygons - 2] =
        ...
        for (unsigned int c = 0; c < numNewPolygons - 1; c++)
        {
            ...
            for (unsigned int v = 0; v < numNewPolygonVertices[c]; v++)
            {
                ...
                if (result.NewVertices[newVertexIndex].Type != SplitPolygonWithCircleResult::NewVertex::Types::PolygonVertex)
                    continue;
            ...
            for (unsigned int v = 0; v < numNewPolygonVertices[c]; v++)
            {
                ...
                SplitPolygonWithCircleResult::NewEdge::Types edgeType = SplitPolygonWithCircleResult::NewEdge::Types::Unknown;
                ...
                {
                    if (result.NewVertices[origin].Type == SplitPolygonWithCircleResult::NewVertex::Types::CircleIntersection &&
                        result.NewVertices[end].Type == SplitPolygonWithCircleResult::NewVertex::Types::CircleIntersection)
                    {
        ...
        SplitPolygonWithCircleResult::NewPolygon &lastPolygon = result.NewPolygons[numNewPolygons - 1];
        const unsigned int lastPolygonNumVertices = numNewPolygonVertices[numNewPolygons - 1];
        ...
        for (unsigned int v = 0; v < lastPolygonNumVertices; v++)
        {
            ...
            SplitPolygonWithCircleResult::NewEdge::Types edgeType = SplitPolygonWithCircleResult::NewEdge::Types::Unknown;
            SplitPolygonWithCircleResult::NewEdge::ArcTypes edgeArcType = SplitPolygonWithCircleResult::NewEdge::ArcTypes::Unknown;

            if ((result.NewVertices[origin].Type == SplitPolygonWithCircleResult::NewVertex::Types::CircleIntersection ||
                 result.NewVertices[origin].Type == SplitPolygonWithCircleResult::NewVertex::Types::Both) &&
                (result.NewVertices[end].Type == SplitPolygonWithCircleResult::NewVertex::Types::CircleIntersection ||
                 result.NewVertices[end].Type == SplitPolygonWithCircleResult::NewVertex::Types::Both))
            {
                const unsigned int originIntersectionIndex = result.NewVertices[origin].IntersectionIndex;
                const unsigned int endIntersectionIndex = result.NewVertices[end].IntersectionIndex;
                ...
                if (polygonCircleIntersections.Intersections[originIntersectionIndex].IndexType ==
                    ...
                    polygonCircleIntersections.Intersections[originIntersectionIndex].Index ==
                        polygonCircleIntersections.Intersections[endIntersectionIndex].Index)
                {
                    // intersections on the same edge
                    edgeType = SplitPolygonWithCircleResult::NewEdge::Types::Segment;
                }
                else if (polygonCircleIntersections.Intersections[originIntersectionIndex].IndexType ==
                             IntersectionPolygonCircleResult::Intersection::IndexTypes::Edge &&
                         polygonCircleIntersections.Intersections[endIntersectionIndex].IndexType ==
                             IntersectionPolygonCircleResult::Intersection::IndexTypes::Vertex &&
                         (polygonCircleIntersections.Intersections[originIntersectionIndex].Index + 1) % polygonVertices.cols() ==
                             polygonCircleIntersections.Intersections[endIntersectionIndex].Index)
                {
                    // intersection on edge and vertex
                    edgeType = SplitPolygonWithCircleResult::NewEdge::Types::Segment;
                }
                else if (polygonCircleIntersections.Intersections[originIntersectionIndex].IndexType ==
                             IntersectionPolygonCircleResult::Intersection::IndexTypes::Vertex &&
                         ...
                         polygonCircleIntersections.Intersections[originIntersectionIndex].Index ==
                             polygonCircleIntersections.Intersections[endIntersectionIndex].Index)
                {
                    // intersection on vertex and edge
                    edgeType = SplitPolygonWithCircleResult::NewEdge::Types::Segment;
                }
                else if (polygonCircleIntersections.Intersections[originIntersectionIndex].IndexType ==
                             IntersectionPolygonCircleResult::Intersection::IndexTypes::Vertex &&
                                                                  ...
                                                                  const IntersectionPolygonCircleResult &polygonCircleIntersections) const
{
    ...
    for (unsigned int nv = 0; nv < numNewVertices; nv++)
    {
        ...
        case SplitPolygonWithCircleResult::NewVertex::Types::CircleIntersection: {
            const IntersectionPolygonCircleResult::Intersection &intersection =
                polygonCircleIntersections.Intersections[splitResult.NewVertices[nvIndex].IntersectionIndex];
            Output::Assert(intersection.IndexType ==
