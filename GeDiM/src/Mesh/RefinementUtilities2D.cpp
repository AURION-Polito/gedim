...
{
// ***************************************************************************
RefinementUtilities::CheckSplitType_Result RefinementUtilities::SplitPolygon_CheckSplitType(
    ...
    const RefinePolygon_CheckResult &cell2DCheckToRefine) const
{
    ...
    const GeometryUtilities::LinePolygonPositionResult::EdgeIntersection &edgeIntersectionOne =
        cell2DCheckToRefine.Cell1DsIntersection[0];
    const GeometryUtilities::LinePolygonPositionResult::EdgeIntersection &edgeIntersectionTwo =
        cell2DCheckToRefine.Cell1DsIntersection[1];
    ...
    if (isTriangle && cell1DToSplitTwo.Type == RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
    {
        result.Type = CheckSplitType_Result::SplitTypes::NewVertexFrom;
        return result;
    }

    if (isTriangle && cell1DToSplitOne.Type == RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
    {
        result.Type = CheckSplitType_Result::SplitTypes::NewVertexTo;
        return result;
    }

    if (!isTriangle && !cell1DToSplitOne.IsToSplit && !cell1DToSplitTwo.IsToSplit)
    {
        unsigned int fromVertex =
            geometryUtilities.IsValueGreaterOrEqual(0.5, edgeIntersectionOne.CurvilinearCoordinate, geometryUtilities.Tolerance1D())
                ? edgeIntersectionOne.Index
                : (edgeIntersectionOne.Index + 1) % cell2DNumVertices;
        unsigned int toVertex =
            geometryUtilities.IsValueGreaterOrEqual(0.5, edgeIntersectionTwo.CurvilinearCoordinate, geometryUtilities.Tolerance1D())
        ...
        if (AreVerticesAligned(cell2DVertices, fromVertex, toVertex))
        {
            ...
            if (cell1DToSplitOne.Type != RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside &&
                cell1DToSplitTwo.Type != RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
            {
                // no fixed vertices, flip toVertex
                toVertex = (toVertex == edgeIntersectionTwo.Index) ? (edgeIntersectionTwo.Index + 1) % cell2DNumVertices
                                                                   : edgeIntersectionTwo.Index;
            }
            else if (cell1DToSplitOne.Type == RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside &&
                     cell1DToSplitTwo.Type != RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
            {
                // fromVertex is fixed
                toVertex = (toVertex == edgeIntersectionTwo.Index) ? (edgeIntersectionTwo.Index + 1) % cell2DNumVertices
            ...
            {
                // toVertex is fixed
                fromVertex = (fromVertex == edgeIntersectionOne.Index) ? (edgeIntersectionOne.Index + 1) % cell2DNumVertices
                                                                       : edgeIntersectionOne.Index;
            }
    ...
    if (cell1DToSplitOne.IsToSplit && !cell1DToSplitTwo.IsToSplit)
    {
        result.Type = CheckSplitType_Result::SplitTypes::NewVertexFrom;
        return result;
    }
                                                      ...
                                                      Gedim::IMeshDAO &mesh) const
{
...

    {
        ...
        const Eigen::MatrixXd newCell2D_Vertices =
            geometryUtilities.RotatePointsFrom3DTo2D(polygon_3D_vertices, cell2DRotation.transpose(), cell2DTranslation);
        const std::vector<unsigned int> unaligned = geometryUtilities.UnalignedPoints(newCell2D_Vertices);
        const Eigen::MatrixXd unalignedVertices = geometryUtilities.ExtractPoints(newCell2D_Vertices, unaligned);
        const std::vector<unsigned int> triangles = geometryUtilities.PolygonTriangulationByFirstVertex(unalignedVertices);

        const std::vector<Eigen::Matrix3d> trianglesVertices =
            geometryUtilities.ExtractTriangulationPoints(unalignedVertices, triangles);
                                                                                         ...
                                                                                         Gedim::IMeshDAO &mesh) const
{
...

    {
        ...
        // Check split areas
        if (!SplitPolygon_IsAreaPositive(subCells[0].row(0).transpose(), cell2DRotation, cell2DTranslation, mesh))
        {
        ...
        if (!SplitPolygon_IsAreaPositive(subCells[1].row(0).transpose(), cell2DRotation, cell2DTranslation, mesh))
        {
            result.Type = SplitPolygon_Result::Types::NoSplit;
            return result;
        }
    ...
    mesh.Cell1DInsertExtremes(newCell1DIndex, mesh.Cell2DVertex(cell2DIndex, toVertex), mesh.Cell2DVertex(cell2DIndex, fromVertex));
    ...
    result.NewCell2DsIndex = meshUtilities.SplitCell2D(cell2DIndex, subCells, mesh);

    mesh.Cell1DInsertNeighbourCell2D(newCell1DIndex, 0,
                                     result.NewCell2DsIndex[1]); // right
    mesh.Cell1DInsertNeighbourCell2D(newCell1DIndex, 1,
                                                                                         ...
                                                                                         Gedim::IMeshDAO &mesh) const
{
    ...
    result.NewCell1DIndex = mesh.Cell1DAppend(1);
    const unsigned int &newCell1DIndex = result.NewCell1DIndex;
    mesh.Cell1DInsertExtremes(newCell1DIndex, mesh.Cell2DVertex(cell2DIndex, toVertex), fromNewCell0DIndex);
    mesh.Cell1DSetMarker(newCell1DIndex, 0);
    mesh.Cell1DSetState(newCell1DIndex, true);
    mesh.Cell1DInitializeNeighbourCell2Ds(newCell1DIndex, 2);
                                                                                       ...
                                                                                       Gedim::IMeshDAO &mesh) const
{
    ...
    result.NewCell1DIndex = mesh.Cell1DAppend(1);
    const unsigned int &newCell1DIndex = result.NewCell1DIndex;
    mesh.Cell1DInsertExtremes(newCell1DIndex, toNewCell0DIndex, mesh.Cell2DVertex(cell2DIndex, fromVertex));
    mesh.Cell1DSetMarker(newCell1DIndex, 0);
    mesh.Cell1DSetState(newCell1DIndex, true);
    mesh.Cell1DInitializeNeighbourCell2Ds(newCell1DIndex, 2);
                                                                                       ...
                                                                                       Gedim::IMeshDAO &mesh) const
{
    ...
    result.NewCell1DIndex = mesh.Cell1DAppend(1);
    const unsigned int &newCell1DIndex = result.NewCell1DIndex;
    mesh.Cell1DInsertExtremes(newCell1DIndex, toNewCell0DIndex, fromNewCell0DIndex);
    mesh.Cell1DSetMarker(newCell1DIndex, 0);
    mesh.Cell1DSetState(newCell1DIndex, true);
    mesh.Cell1DInitializeNeighbourCell2Ds(newCell1DIndex, 2);
                                                                                           ...
                                                                                           const Eigen::VectorXd &triangle_edges_length) const
{
    ...
    const Eigen::VectorXd edge_middle_point =
        0.5 * (triangle_vertices.col(max_edge_end_index) + triangle_vertices.col(max_edge_origin_index));
...
RefinementUtilities::Cell2Ds_GeometricData RefinementUtilities::RefinePolygonCell_InitializeGeometricData(const Gedim::IMeshDAO &mesh) const
{
    ...
    std::iota(std::begin(geometricData.Cell1Ds.Aligned), std::end(geometricData.Cell1Ds.Aligned), 1);
    geometricData.Cell1Ds.MaxAligned = mesh.Cell1DTotalNumber();

    RefinePolygonCell_UpdateGeometricData(mesh, cell2DsIndex, geometricData);
                                                                ...
                                                                Cell2Ds_GeometricData &geometricData) const
{
    ...
    for (unsigned int c = 0; c < cell2DsIndex.size(); c++)
    {
        ...
        convexCell2DUnalignedVertices = geometryUtilities.ExtractPoints(convexCell2DVertices, convexCell2DUnalignedVerticesFilter);
        ...
        for (unsigned int e = 0; e < numCell2DEdges; e++)
        {
            ...
            const unsigned int end = mesh.Cell2DVertex(cell2DIndex, (e + 1) % numCell2DEdges);

            geometricData.Cell2Ds.EdgesDirection[cell2DIndex][e] = (mesh.Cell2DFindEdgeByExtremes(cell2DIndex, origin, end) == e);
        }
        ...
        geometricData.Cell2Ds.CentroidEdgesDistance[cell2DIndex] =
            geometryUtilities.PolygonCentroidEdgesDistance(convexCell2DVertices,
                                                           convexCell2DCentroid,
                                                           geometricData.Cell2Ds.EdgesNormal[cell2DIndex]);
        geometricData.Cell2Ds.CentroidVerticesDistance[cell2DIndex] =
            geometryUtilities.PolygonCentroidVerticesDistance(convexCell2DVertices, convexCell2DCentroid);
        geometricData.Cell2Ds.InRadius[cell2DIndex] =
            geometryUtilities.PolygonInRadius(geometricData.Cell2Ds.CentroidEdgesDistance[cell2DIndex]);
        geometricData.Cell2Ds.Inertia[cell2DIndex] =
            geometryUtilities.PolygonInertia(convexCell2DCentroid, convexCell2DTriangulationPoints);

        geometricData.Cell2Ds.Quality[cell2DIndex] =
            std::min(geometricData.Cell2Ds.EdgesLength[cell2DIndex].minCoeff(), geometricData.Cell2Ds.InRadius[cell2DIndex]);
...
// ***************************************************************************
RefinementUtilities::RefinePolygon_CheckResult::Cell1DToSplit RefinementUtilities::RefinePolygonCell_IsCell1DToSplit(
    ...
    const Gedim::IMeshDAO &mesh) const
{
    ...
    result.IsIntersectionInside =
        (edgeIntersection.Type == GeometryUtilities::LinePolygonPositionResult::EdgeIntersection::Types::InsideEdge);
    ...
    result.IsEdgeLengthEnough = !geometryUtilities.IsValueZero(0.5 * cell1DLength, geometryUtilities.Tolerance1D());
    ...
    result.IsNeighQualityEnough.resize(mesh.Cell1DNumberNeighbourCell2D(cell1DIndex), true);
    ...
    for (unsigned int c2Dn = 0; c2Dn < mesh.Cell1DNumberNeighbourCell2D(cell1DIndex); c2Dn++)
    {
        ...
        result.IsNeighQualityEnough[c2Dn] =
            geometryUtilities.IsValueGreaterOrEqual(0.5 * cell1DLength,
                                                    cell1DsQualityWeight * cell2DsQuality[cell2DNeighIndex],
                                                    geometryUtilities.Tolerance1D());

        if (cell2DNeighIndex == cell2DIndex && !result.IsNeighQualityEnough[c2Dn])
        {
    ...
    result.IsNeighAlignedRespect.resize(mesh.Cell1DNumberNeighbourCell2D(cell1DIndex), true);
    ...
    for (unsigned int c2Dn = 0; c2Dn < mesh.Cell1DNumberNeighbourCell2D(cell1DIndex); c2Dn++)
    {
        ...
        if (cell2DNeighIndex == cell2DIndex && !result.IsNeighAlignedRespect[c2Dn])
        {
                                                                                              ...
                                                                                              const Eigen::Vector3d &centroid) const
{
    ...
    result.LineTangent = geometryUtilities.SegmentNormal(unalignedVertices.col(row), unalignedVertices.col(col));
                                                                                             ...
                                                                                             const Eigen::Matrix3d &inertia) const
{
    ...
    if (unalignedVertices.cols() == 3)
    {
        ...
        result.LineTangent = geometryUtilities
                                 .SegmentTangent(unalignedVertices.col(oppositeVertexIndex),
                                                 0.5 * (unalignedVertices.col(maxEdgeLocalIndex) +
                                                        unalignedVertices.col((maxEdgeLocalIndex + 1) % 3)))
                                 .normalized();
                                                                                         ...
                                                                                         Gedim::IMeshDAO &mesh) const
{
    ...
    if (geometryUtilities.IsValueZero(0.5 * cell2DArea, geometryUtilities.Tolerance2D()) ||
        geometryUtilities.IsValueZero(0.5 * cell2DEdgesLength[edgeIndex], geometryUtilities.Tolerance1D()))
    {
    ...
    const SplitCell1D_Result splitCell1D = SplitCell1D_MiddlePoint(cell1DIndex, mesh);
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    GeometryUtilities::LinePolygonPositionResult linePolygonPosition =
        geometryUtilities.LinePolygonPosition(lineTangent, lineOrigin, cell2DVertices);
    ...
    const GeometryUtilities::LinePolygonPositionResult::EdgeIntersection &edgeIntersectionOne =
        linePolygonPosition.EdgeIntersections[0];
    ...
    {
        const GeometryUtilities::LinePolygonPositionResult::EdgeIntersection &edgeIntersectionTwo =
            linePolygonPosition.EdgeIntersections[i];
        ...
        case GeometryUtilities::LinePolygonPositionResult::EdgeIntersection::Types::OnEdgeOrigin: {
            ...
            const unsigned int cell1DOriginIndex = cell2DEdgesDirection[edgeIntersectionTwo.Index]
                                                       ? mesh.Cell1DOrigin(cell1DIndexTwo)
                                                       : mesh.Cell1DEnd(cell1DIndexTwo);
            if (mesh.Cell1DOrigin(cell1DIndexOne) != cell1DOriginIndex && mesh.Cell1DEnd(cell1DIndexOne) != cell1DOriginIndex)
                findEdgeIntersectionTwo = &linePolygonPosition.EdgeIntersections[i];
        }
        break;
        case GeometryUtilities::LinePolygonPositionResult::EdgeIntersection::Types::OnEdgeEnd: {
            const unsigned int cell1DIndexTwo = mesh.Cell2DEdge(cell2DIndex, edgeIntersectionTwo.Index);
            const unsigned int cell1DEndIndex = cell2DEdgesDirection[edgeIntersectionTwo.Index]
                                                    ? mesh.Cell1DOrigin(cell1DIndexTwo)
                                                    : mesh.Cell1DEnd(cell1DIndexTwo);
            if (mesh.Cell1DOrigin(cell1DIndexOne) != cell1DEndIndex && mesh.Cell1DEnd(cell1DIndexOne) != cell1DEndIndex)
                findEdgeIntersectionTwo = &linePolygonPosition.EdgeIntersections[i];
        }
        break;
        default:
    ...
    const RefinePolygon_CheckResult::Cell1DToSplit createNewVertexOne =
        RefinePolygonCell_IsCell1DToSplit(cell1DIndexOne,
                                          cell2DIndex,
                                          edgeIntersectionOne,
                                          cell2DsEdgesLength,
                                          ...
                                          cell1DsAligned,
                                          mesh);
    const RefinePolygon_CheckResult::Cell1DToSplit createNewVertexTwo =
        RefinePolygonCell_IsCell1DToSplit(cell1DIndexTwo,
                                          cell2DIndex,
                                          edgeIntersectionTwo,
                                          cell2DsEdgesLength,
                                          cell1DsQualityWeight,
...
// ***************************************************************************
RefinementUtilities::RefinePolygon_Result RefinementUtilities::RefinePolygonCell_ByDirection(
    const unsigned int &cell2DIndex,
    const Gedim::GeometryUtilities::PolygonTypes &cell2DPolygonType,
    const Gedim::GeometryUtilities::PolygonTypes &cell2DUnalignedPolygonType,
    const Eigen::MatrixXd &cell2DVertices,
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    if (extendToNeighbours && cell2DUnalignedPolygonType != Gedim::GeometryUtilities::PolygonTypes::Triangle &&
        !SplitPolygon_CheckIsNotToExtend(createNewVertexOne, createNewVertexTwo))
    {
    ...
    const CheckSplitType_Result splitTypeResult =
        SplitPolygon_CheckSplitType(cell2DPolygonType, cell2DUnalignedPolygonType, cell2DVertices, cell2DCheckToRefine);
    ...
    case CheckSplitType_Result::SplitTypes::NoNewVertices: {
        ...
        if (AreVerticesAligned(cell2DVertices, fromVertex, toVertex))
        {
            if (extendToNeighbours && !SplitPolygon_CheckIsToSplit_Relaxed(createNewVertexOne, createNewVertexTwo))
            {
                result.ResultType = RefinePolygon_Result::ResultTypes::SplitQualityCheckCell2DFailed;
                result.SplitType = CheckSplitType_Result::SplitTypes::NoSplit;
                return result;
        ...
        const SplitPolygon_Result splitResult =
            SplitPolygon_NoNewVertices(cell2DIndex, cell2DNumVertices, fromVertex, toVertex, cell2DRotation, cell2DTranslation, mesh);
    ...
    case CheckSplitType_Result::SplitTypes::NewVertexFrom: {
        ...
        if (createNewVertexOne.Type == RefinePolygon_CheckResult::Cell1DToSplit::Types::EdgeLengthNotEnough)
        {
            result.ResultType = RefinePolygon_Result::ResultTypes::Cell2DSplitUnderTolerance;
            result.SplitType = CheckSplitType_Result::SplitTypes::NoSplit;
            return result;
        ...
        // new vertex one
        Gedim::Output::Assert(edgeIntersectionOne.Type ==
        ...
        if (geometryUtilities.PointIsAligned(cell2DVertices.col(edgeIntersectionOne.Index), cell2DVertices.col(toVertex), middleCoordinate) ||
        ...
        {
            // try to flip vertices
            if (createNewVertexTwo.Type != RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
            {
                // no fixed vertices, flip toVertex
                toVertex = (toVertex == edgeIntersectionTwo.Index) ? (edgeIntersectionTwo.Index + 1) % cell2DNumVertices
                                                                   : edgeIntersectionTwo.Index;
        ...
        const SplitCell1D_Result splitCell1DOne = SplitCell1D_MiddlePoint(cell1DIndexOne, mesh);
    ...
    case CheckSplitType_Result::SplitTypes::NewVertexTo: {
        ...
        if (createNewVertexTwo.Type == RefinePolygon_CheckResult::Cell1DToSplit::Types::EdgeLengthNotEnough)
        {
            result.ResultType = RefinePolygon_Result::ResultTypes::Cell2DSplitUnderTolerance;
            result.SplitType = CheckSplitType_Result::SplitTypes::NoSplit;
            return result;
        ...
        // new vertex two
        Gedim::Output::Assert(edgeIntersectionTwo.Type ==
                              GeometryUtilities::LinePolygonPositionResult::EdgeIntersection::Types::InsideEdge);
        ...
        if (geometryUtilities.PointIsAligned(cell2DVertices.col(fromVertex), cell2DVertices.col(edgeIntersectionTwo.Index), middleCoordinate) ||
            geometryUtilities.PointIsAligned(cell2DVertices.col(fromVertex),
                                             cell2DVertices.col((edgeIntersectionTwo.Index + 1) % cell2DNumVertices),
                                             middleCoordinate))
        {
            // try to flip vertices
            if (createNewVertexOne.Type != RefinePolygon_CheckResult::Cell1DToSplit::Types::NotInside)
            {
                // no fixed vertices, flip toVertex
                fromVertex = (fromVertex == edgeIntersectionOne.Index) ? (edgeIntersectionOne.Index + 1) % cell2DNumVertices
                                                                       : edgeIntersectionOne.Index;
        ...
        const SplitCell1D_Result splitCell1DTwo = SplitCell1D_MiddlePoint(cell1DIndexTwo, mesh);
    ...
    case CheckSplitType_Result::SplitTypes::NewVertices: {
        ...
        result.NewCell0DsIndex = {splitCell1DOne.NewCell0DIndex, splitCell1DTwo.NewCell0DIndex};
...
// ***************************************************************************
RefinementUtilities::RefinePolygon_UpdateNeighbour_Result RefinementUtilities::RefinePolygonCell_UpdateNeighbours(
    const unsigned int &cell2DIndex,
    const unsigned int &cell1DIndex,
    const unsigned int &newCell0DIndex,
    const std::vector<unsigned int> &splitCell1DsIndex,
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int n = 0; n < mesh.Cell1DNumberNeighbourCell2D(cell1DIndex); n++)
    {
        ...
        const std::vector<unsigned int> newCell2Ds = meshUtilities.SplitCell2D(neighCell2DIndex, subCells, mesh);
