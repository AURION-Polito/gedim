                                               ...
                                               Gedim::IMeshDAO &mesh) const
{
...

    {
        ...
        for (const auto &polyhedron : polyhedra)
        {
            ...
            for (unsigned int p_v = 0; p_v < polyhedron.Vertices.cols(); ++p_v)
            {
                ...
                for (auto cell0D_it = cell0Ds_filter.begin(); cell0D_it != cell0Ds_filter.end(); cell0D_it++)
                {
                    if (geometry_utilities.PointsAreCoincident(point, cell0D_it->coordinate))
                    {
                        found = cell0D_it;
                        break;
                    }
                ...
                if (found == cell0Ds_filter.end())
                {
                    ...
                    cell0Ds_filter.push_back({point, static_cast<unsigned int>(cell0Ds_filter.size())});
                }
                else
                {
            ...
            for (unsigned int p_e = 0; p_e < polyhedron.Edges.cols(); ++p_e)
            {
                ...
                std::pair<unsigned int, unsigned int> edge_extr = {std::min(edge_org, edge_end), std::max(edge_org, edge_end)};
                ...
                if (found == cell1Ds_filter.end())
                {
                    ...
                    cell1Ds_filter.insert(std::make_pair(edge_extr, static_cast<unsigned int>(cell1Ds_filter.size())));
                }
                else
                {
            ...
            for (unsigned int p_f = 0; p_f < polyhedron.Faces.size(); ++p_f)
            {
                ...
                if (found == cell2Ds_filter.end())
                {
                    ...
                    cell2Ds_filter.insert(std::make_pair(
                        face_key,
                        cell2D_filter({face_vertices, face_edges, static_cast<unsigned int>(cell2Ds_filter.size())})));
                }
                else
                {
    ...
    FillMesh3D(cell0Ds, cell1Ds, cell2Ds_vertices, cell2Ds_edges, polyhedra_cell0Ds, polyhedra_cell1Ds, polyhedra_cell2Ds, mesh);
}
// ***************************************************************************
} // namespace Gedim
