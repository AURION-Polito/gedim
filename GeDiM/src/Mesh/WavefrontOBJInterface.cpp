...
namespace Gedim
{
  ...
  WavefrontOBJInterface::OBJMesh WavefrontOBJInterface::StringsToOBJMesh(const std::vector<std::string> &fileLines) const
  {
    ...
    mesh.Cell2Ds = std::vector<Eigen::VectorXi>(faces.begin(),
                                                faces.end());
  ...
  // ***************************************************************************
  std::vector<std::string> WavefrontOBJInterface::ImportMeshFromFile(const std::string &OBJFilePath, const MeshUtilities &meshUtilities, IMeshDAO &mesh) const
  {
