...
{
// ***************************************************************************
IntersectorMesh2DSegment::IntersectorMesh2DSegment(const Gedim::IMeshDAO &mesh, const Gedim::GeometryUtilities &geometryUtilities)
    : _mesh(mesh), _geometryUtilities(geometryUtilities)
{
}
                                                        ...
                                                        vector<double> &curvilinearCoordinates)
{
    ...
    for (std::map<double, IntersectorMesh2DSegment::IntersectionMesh::IntersectionMeshPoint>::const_iterator it =
             intersectingMesh.Points.begin();
         it != intersectingMesh.Points.end();
         it++)
    {
...
// ***************************************************************************
IntersectorMesh2DSegment::IntersectionMesh::IntersectionMeshPoint &IntersectorMesh2DSegment::InsertNewIntersection(
    ...
    bool &found)
{
    ...
    result.Points.insert(pair<double, IntersectionMesh::IntersectionMeshPoint>(curvilinearCoordinate,
                                                        ...
                                                        IntersectorMesh2DSegment::IntersectionMesh &result)
{
    for (unsigned int e = 0; e < _mesh.Cell1DTotalNumber(); e++)
    {
        ...
        const Eigen::Vector3d edgeBarycenter = _geometryUtilities.SegmentBarycenter(edgeOrigin, edgeEnd);
        const double edgeLength = _geometryUtilities.SegmentLength(edgeOrigin, edgeEnd);

        if (_geometryUtilities.CheckNoSpheresIntersection(edgeBarycenter, segmentBarycenter, edgeLength, segmentLength))
            continue;

        Gedim::GeometryUtilities::IntersectionSegmentSegmentResult intersectionSegmentSegmentResult =
            _geometryUtilities.IntersectionSegmentSegment(edgeOrigin, edgeEnd, segmentOrigin, segmentEnd);
            ...
            Gedim::GeometryUtilities::IntersectionSegmentSegmentResult::IntersectionSegmentTypes::SingleIntersection)
        {
            ...
            const Gedim::GeometryUtilities::PointSegmentPositionTypes &segmentIntersectionType =
                intersectionSegmentSegmentResult.SecondSegmentIntersections[0].Type;
            ...
            const Gedim::GeometryUtilities::PointSegmentPositionTypes &edgeIntersectionType =
                intersectionSegmentSegmentResult.FirstSegmentIntersections[0].Type;
                 ...
                 Gedim::GeometryUtilities::IntersectionSegmentSegmentResult::IntersectionSegmentTypes::MultipleIntersections)
        {
            ...
            const double &segmentFirstCurvilinearCoordinate =
                intersectionSegmentSegmentResult.SecondSegmentIntersections[0].CurvilinearCoordinate;
            const double &segmentSecondCurvilinearCoordinate =
                intersectionSegmentSegmentResult.SecondSegmentIntersections[1].CurvilinearCoordinate;
            ...
            const Gedim::GeometryUtilities::PointSegmentPositionTypes &edgeFirstIntersectionType =
                intersectionSegmentSegmentResult.FirstSegmentIntersections[indexEdgeFirstIntersection].Type;
            const Gedim::GeometryUtilities::PointSegmentPositionTypes &edgeSecondIntersectionType =
                intersectionSegmentSegmentResult.FirstSegmentIntersections[indexEdgeSecondIntersection].Type;
...
// ***************************************************************************
void IntersectorMesh2DSegment::SmoothIntersections(const Vector3d &, const Vector3d &, IntersectorMesh2DSegment::IntersectionMesh &result)
{
    ...
    for (unsigned int p = 0; p < numSegments; p++)
    {
            ...
            intersectionPoint.Vertex2DIds == intersectionPointNext.Vertex2DIds)
        {
            ...
            std::merge(intersectionPoint.Cell2DIds.begin(),
                       ...
                       intersectionPointNext.Cell2DIds.begin(),
                       intersectionPointNext.Cell2DIds.end(),
                       std::inserter(avgPoint.Cell2DIds, avgPoint.Cell2DIds.begin()));
