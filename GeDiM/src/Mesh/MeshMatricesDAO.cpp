                                                                ...
                                                                const T &elementInitialization)
{
    ...
    elementVector.resize(numberElementSize * numberElements, elementInitialization);
}
// ***************************************************************************
template <typename T>
                                             ...
                                             const T &elementInitialization)
{
    ...
    elementVector.resize(totalSize, elementInitialization);
}
// ***************************************************************************
template <typename T>
...
{
    int numOriginalElements = (numberElementVector[vectorIndex + 1] - numberElementVector[vectorIndex]);
    int newElements = newNumberElements - numOriginalElements;
    unsigned int numOldElements = numberElementVector[numberElements];

    for (unsigned int next = vectorIndex; next < numberElements; next++)
        numberElementVector[next + 1] = numberElementVector[next + 1] + newElements;

    if (newElements > 0)
    {
        elementVector.resize(numberElementVector[numberElements], newElementInitialization);
        std::rotate(elementVector.begin() + numberElementVector[vectorIndex] + numOriginalElements,
                    elementVector.begin() + numOldElements,
                    elementVector.end());
    }
    else
        elementVector.erase(elementVector.begin() + numberElementVector[vectorIndex] + newNumberElements,
                            elementVector.begin() + numberElementVector[vectorIndex] + numOriginalElements);
}
// ***************************************************************************
                                                      ...
                                                      const T &newElementInitialization)
{
    ...
    for (auto it = elements.begin(); it != elements.end(); it++)
    {
        ...
        AlignContainerHigherElements(v, minElement, newElementInitialization);
        it->second.clear();
...
// ***************************************************************************
template <class Container, class T>
void MeshMatricesDAO::AlignContainerHigherElements(Container &elements, const T &minElement, const T &newElementInitialization)
{
    for (auto it = elements.begin(); it != elements.end(); it++)
    {
        if (*it > minElement)
...
// ***************************************************************************
template <class Container, class T>
void MeshMatricesDAO::AlignContainerElements(Container &elements, const T &element, const T &newElementInitialization)
{
    for (auto it = elements.begin(); it != elements.end(); it++)
    {
        if ((*it) == element)
...
// ***************************************************************************
template <typename T>
void MeshMatricesDAO::AlignSparseMatrixHigherElements(Eigen::SparseMatrix<T> &matrix, const T &minElement)
{
...
void MeshMatricesDAO::Cell0DRemove(const unsigned int &cell0DIndex)
{
    ...
    for (unsigned int p = 0; p < Cell0DNumberDoubleProperties(); p++)
    {
        ResizeNumberVectorWithNewNumberElements(_mesh.Cell0DDoublePropertySizes[p],
                                                ...
                                                cell0DIndex,
                                                0);
        _mesh.Cell0DDoublePropertySizes[p].erase(std::next(_mesh.Cell0DDoublePropertySizes[p].begin(), cell0DIndex));
    }

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell0DNeighbourCell1D, _mesh.Cell0DNeighbourCell1Ds, _mesh.NumberCell0D, cell0DIndex, 0);
    _mesh.NumberCell0DNeighbourCell1D.erase(std::next(_mesh.NumberCell0DNeighbourCell1D.begin(), cell0DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell0DNeighbourCell2D, _mesh.Cell0DNeighbourCell2Ds, _mesh.NumberCell0D, cell0DIndex, 0);
    _mesh.NumberCell0DNeighbourCell2D.erase(std::next(_mesh.NumberCell0DNeighbourCell2D.begin(), cell0DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell0DNeighbourCell3D, _mesh.Cell0DNeighbourCell3Ds, _mesh.NumberCell0D, cell0DIndex, 0);
    _mesh.NumberCell0DNeighbourCell3D.erase(std::next(_mesh.NumberCell0DNeighbourCell3D.begin(), cell0DIndex));
    ...
    AlignContainerHigherElements(_mesh.Cell1DVertices, cell0DIndex, _mesh.NumberCell0D);
    AlignContainerHigherElements(_mesh.Cell2DVertices, cell0DIndex, _mesh.NumberCell0D);
    AlignContainerHigherElements(_mesh.Cell3DVertices, cell0DIndex, _mesh.NumberCell0D);

    AlignMapContainerHigherElements(_mesh.UpdatedCell0Ds, cell0DIndex, _mesh.NumberCell0D);
}
// ***************************************************************************
void MeshMatricesDAO::Cell0DInsertCoordinates(const unsigned int &cell0DIndex, const Vector3d &coordinates)
{
    Output::Assert(cell0DIndex < Cell0DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInsertUpdatedCell0D(const unsigned int &cell0DIndex, const unsigned int &updatedCell0DIdex)
{
    Output::Assert(cell0DIndex < Cell0DTotalNumber());
...
// ***************************************************************************
bool MeshMatricesDAO::Cell0DUpdatedCell0Ds(const unsigned int &cell0DIndex, list<unsigned int> &updatedCell0DIds) const
{
    Output::Assert(cell0DIndex < Cell0DTotalNumber());
    ...
    {
        Cell0DUpdatedCell0Ds(childId, updatedCell0DIds);
    }
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInitializeNeighbourCell1Ds(const unsigned int &cell0DIndex, const unsigned int &numberNeighbourCell1Ds)
{
    Output::Assert(cell0DIndex < Cell0DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInitializeNeighbourCell1Ds(const unsigned int &cell0DIndex, const std::vector<unsigned int> &neighbourCell1Ds)
{
    Cell0DInitializeNeighbourCell1Ds(cell0DIndex, neighbourCell1Ds.size());

    for (unsigned int n = 0; n < neighbourCell1Ds.size(); n++)
    {
        Cell0DInsertNeighbourCell1D(cell0DIndex, n, neighbourCell1Ds.at(n));
    }
}
// ***************************************************************************
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInitializeNeighbourCell2Ds(const unsigned int &cell0DIndex, const std::vector<unsigned int> &neighbourCell2Ds)
{
    Cell0DInitializeNeighbourCell2Ds(cell0DIndex, neighbourCell2Ds.size());

    for (unsigned int n = 0; n < neighbourCell2Ds.size(); n++)
    {
        Cell0DInsertNeighbourCell2D(cell0DIndex, n, neighbourCell2Ds.at(n));
    }
}
// ***************************************************************************
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInitializeNeighbourCell3Ds(const unsigned int &cell0DIndex, const unsigned int &numberNeighbourCell3Ds)
{
    Output::Assert(cell0DIndex < Cell0DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell0DInitializeNeighbourCell3Ds(const unsigned int &cell0DIndex, const std::vector<unsigned int> &neighbourCell3Ds)
{
    Cell0DInitializeNeighbourCell3Ds(cell0DIndex, neighbourCell3Ds.size());

    for (unsigned int n = 0; n < neighbourCell3Ds.size(); n++)
    {
        Cell0DInsertNeighbourCell3D(cell0DIndex, n, neighbourCell3Ds.at(n));
    }
}
// ***************************************************************************
...
unsigned int MeshMatricesDAO::Cell0DAddDoubleProperty(const string &propertyId)
{
    ...
    _mesh.Cell0DDoublePropertyIndices.insert(pair<string, unsigned int>(propertyId, propertyIndex));
                                                            ...
                                                            const std::vector<unsigned int> &propertySizes)
{
    ...
    InitializeNumberVector(_mesh.Cell0DDoublePropertySizes[propertyIndex], _mesh.Cell0DDoublePropertyValues[propertyIndex], propertySizes, 0.0);
}
// ***************************************************************************
...
void MeshMatricesDAO::Cell1DRemove(const unsigned int &cell1DIndex)
{
    ...
    for (unsigned int p = 0; p < Cell1DNumberDoubleProperties(); p++)
    {
        ResizeNumberVectorWithNewNumberElements(_mesh.Cell1DDoublePropertySizes[p],
                                                ...
                                                cell1DIndex,
                                                0);
        _mesh.Cell1DDoublePropertySizes[p].erase(std::next(_mesh.Cell1DDoublePropertySizes[p].begin(), cell1DIndex));
    }

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell1DNeighbourCell2D, _mesh.Cell1DNeighbourCell2Ds, _mesh.NumberCell1D, cell1DIndex, 0);
    _mesh.NumberCell1DNeighbourCell2D.erase(std::next(_mesh.NumberCell1DNeighbourCell2D.begin(), cell1DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell1DNeighbourCell3D, _mesh.Cell1DNeighbourCell3Ds, _mesh.NumberCell1D, cell1DIndex, 0);
    _mesh.NumberCell1DNeighbourCell3D.erase(std::next(_mesh.NumberCell1DNeighbourCell3D.begin(), cell1DIndex));
    ...
    AlignContainerHigherElements(_mesh.Cell0DNeighbourCell1Ds, cell1DIndex, std::numeric_limits<unsigned int>::max());
    AlignContainerHigherElements(_mesh.Cell2DEdges, cell1DIndex, _mesh.NumberCell1D);
    AlignContainerHigherElements(_mesh.Cell3DEdges, cell1DIndex, _mesh.NumberCell1D);

    AlignMapContainerHigherElements(_mesh.UpdatedCell1Ds, cell1DIndex, _mesh.NumberCell1D);
    AlignContainerHigherElements(_mesh.Cell1DOriginalCell1Ds, cell1DIndex, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
void MeshMatricesDAO::Cell1DInsertExtremes(const unsigned int &cell1DIndex, const unsigned int &originCell0DIndex, const unsigned int &endCell0DIndex)
{
...
{
    Gedim::Output::Assert(cell1DExtremes.rows() == 2 && cell1DExtremes.cols() == Cell1DTotalNumber());
    ...
    for (unsigned int cell1DIndex = 0; cell1DIndex < Cell1DTotalNumber(); cell1DIndex++)
    {
        ...
        _mesh.Cell1DVertices[2 * cell1DIndex] = originCell0DIndex;
        _mesh.Cell1DVertices[2 * cell1DIndex + 1] = endCell0DIndex;
        triplets.push_back(Eigen::Triplet<unsigned int>(originCell0DIndex, endCell0DIndex, cell1DIndex + 1));
    }
}
// ***************************************************************************
...
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell1DByExtremes(const unsigned int &originCell0DIndex, const unsigned int &endCell0DIndex) const
{
    Gedim::Output::Assert(originCell0DIndex < Cell0DTotalNumber());
    Gedim::Output::Assert(endCell0DIndex < Cell0DTotalNumber());
    ...
    {
        if (_mesh.Cell1DVertices[2 * e] == originCell0DIndex && _mesh.Cell1DVertices[2 * e + 1] == endCell0DIndex)
            return e;
    }
...
// ***************************************************************************
void MeshMatricesDAO::Cell1DInitializeNeighbourCell2Ds(const unsigned int &cell1DIndex, const unsigned int &numberNeighbourCell2Ds)
{
    Output::Assert(cell1DIndex < Cell1DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell1DInitializeNeighbourCell2Ds(const unsigned int &cell1DIndex, const std::vector<unsigned int> &neighbourCell2Ds)
{
    Cell1DInitializeNeighbourCell2Ds(cell1DIndex, neighbourCell2Ds.size());

    for (unsigned int n = 0; n < neighbourCell2Ds.size(); n++)
    {
        Cell1DInsertNeighbourCell2D(cell1DIndex, n, neighbourCell2Ds.at(n));
    }
}
// ***************************************************************************
void MeshMatricesDAO::Cell1DInsertUpdatedCell1D(const unsigned int &cell1DIndex, const unsigned int &updatedCell1DIdex)
{
    Output::Assert(cell1DIndex < Cell1DTotalNumber());
...
// ***************************************************************************
bool MeshMatricesDAO::Cell1DUpdatedCell1Ds(const unsigned int &cell1DIndex, list<unsigned int> &updatedCell1DIds) const
{
    Output::Assert(cell1DIndex < Cell1DTotalNumber());
    ...
    for (const unsigned int &childId : iter->second)
    {
        Cell1DUpdatedCell1Ds(childId, updatedCell1DIds);
    }
...
// ***************************************************************************
void MeshMatricesDAO::Cell1DInitializeNeighbourCell3Ds(const unsigned int &cell1DIndex, const unsigned int &numberNeighbourCell3Ds)
{
    Output::Assert(cell1DIndex < Cell1DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell1DInitializeNeighbourCell3Ds(const unsigned int &cell1DIndex, const std::vector<unsigned int> &neighbourCell3Ds)
{
    Cell1DInitializeNeighbourCell3Ds(cell1DIndex, neighbourCell3Ds.size());

    for (unsigned int n = 0; n < neighbourCell3Ds.size(); n++)
    {
        Cell1DInsertNeighbourCell3D(cell1DIndex, n, neighbourCell3Ds.at(n));
    }
}
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell1DAddDoubleProperty(const string &propertyId)
{
    ...
    _mesh.Cell1DDoublePropertyIndices.insert(pair<string, unsigned int>(propertyId, propertyIndex));
                                                            ...
                                                            const std::vector<unsigned int> &propertySizes)
{
    ...
    InitializeNumberVector(_mesh.Cell1DDoublePropertySizes[propertyIndex], _mesh.Cell1DDoublePropertyValues[propertyIndex], propertySizes, 0.0);
}
// ***************************************************************************
...
void MeshMatricesDAO::Cell2DRemove(const unsigned int &cell2DIndex)
{
    ...
    for (unsigned int p = 0; p < Cell2DNumberDoubleProperties(); p++)
    {
        ResizeNumberVectorWithNewNumberElements(_mesh.Cell2DDoublePropertySizes[p],
                                                ...
                                                cell2DIndex,
                                                0);
        _mesh.Cell2DDoublePropertySizes[p].erase(std::next(_mesh.Cell2DDoublePropertySizes[p].begin(), cell2DIndex));
    }

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DNeighbourCell3D, _mesh.Cell2DNeighbourCell3Ds, _mesh.NumberCell2D, cell2DIndex, 0);
    _mesh.NumberCell2DNeighbourCell3D.erase(std::next(_mesh.NumberCell2DNeighbourCell3D.begin(), cell2DIndex));

    _mesh.UpdatedCell2Ds.erase(cell2DIndex);

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DVertices, _mesh.Cell2DVertices, _mesh.NumberCell2D, cell2DIndex, 0);
    _mesh.NumberCell2DVertices.erase(std::next(_mesh.NumberCell2DVertices.begin(), cell2DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DEdges, _mesh.Cell2DEdges, _mesh.NumberCell2D, cell2DIndex, 0);
    _mesh.NumberCell2DEdges.erase(std::next(_mesh.NumberCell2DEdges.begin(), cell2DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DSubdivision, _mesh.Cell2DSubdivision, _mesh.NumberCell2D, cell2DIndex, 0);
    _mesh.NumberCell2DSubdivision.erase(std::next(_mesh.NumberCell2DSubdivision.begin(), cell2DIndex));
    ...
    AlignContainerHigherElements(_mesh.Cell0DNeighbourCell2Ds, cell2DIndex, std::numeric_limits<unsigned int>::max());
    AlignContainerHigherElements(_mesh.Cell1DNeighbourCell2Ds, cell2DIndex, std::numeric_limits<unsigned int>::max());
    AlignContainerHigherElements(_mesh.Cell3DFaces, cell2DIndex, _mesh.NumberCell2D);
    AlignMapContainerHigherElements(_mesh.UpdatedCell2Ds, cell2DIndex, _mesh.NumberCell2D);
    AlignContainerHigherElements(_mesh.Cell2DOriginalCell2Ds, cell2DIndex, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
...
{
    Gedim::Output::Assert(numberCell2DsVertices.size() == _mesh.NumberCell2D);
...
// ***************************************************************************
void MeshMatricesDAO::Cell2DInitializeVertices(const unsigned int &cell2DIndex, const unsigned int &numberCell2DVertices)
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DVertices, _mesh.Cell2DVertices, _mesh.NumberCell2D, cell2DIndex, numberCell2DVertices);
}
// ***************************************************************************
...
{
    Output::Assert(numberCell2DsEdges.size() == Cell2DTotalNumber());
    InitializeNumberVector(_mesh.NumberCell2DEdges, _mesh.Cell2DEdges, numberCell2DsEdges, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
void MeshMatricesDAO::Cell2DInitializeEdges(const unsigned int &cell2DIndex, const unsigned int &numberCell2DEdges)
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DEdges, _mesh.Cell2DEdges, _mesh.NumberCell2D, cell2DIndex, numberCell2DEdges);
}
// ***************************************************************************
void MeshMatricesDAO::Cell2DAddVertices(const unsigned int &cell2DIndex, const vector<unsigned int> &verticesCell0DIndices)
{
    Cell2DInitializeVertices(cell2DIndex, verticesCell0DIndices.size());
    for (unsigned int v = 0; v < verticesCell0DIndices.size(); v++)
        Cell2DInsertVertex(cell2DIndex, v, verticesCell0DIndices[v]);
}
// ***************************************************************************
void MeshMatricesDAO::Cell2DAddEdges(const unsigned int &cell2DIndex, const vector<unsigned int> &edgesCell1DIndices)
{
    Cell2DInitializeEdges(cell2DIndex, edgesCell1DIndices.size());
    for (unsigned int e = 0; e < edgesCell1DIndices.size(); e++)
        Cell2DInsertEdge(cell2DIndex, e, edgesCell1DIndices[e]);
}
// ***************************************************************************
void MeshMatricesDAO::Cell2DAddVerticesAndEdges(const unsigned int &cell2DIndex, const Eigen::MatrixXi &verticesAndEdgesIndices)
{
    Cell2DInitializeVertices(cell2DIndex, verticesAndEdgesIndices.cols());
    Cell2DInitializeEdges(cell2DIndex, verticesAndEdgesIndices.cols());

    for (unsigned int v = 0; v < verticesAndEdgesIndices.cols(); v++)
    {
        Cell2DInsertVertex(cell2DIndex, v, verticesAndEdgesIndices(0, v));
        Cell2DInsertEdge(cell2DIndex, v, verticesAndEdgesIndices(1, v));
    }
}
// ***************************************************************************
...
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell2DFindVertex(const unsigned int &cell2DIndex, const unsigned int &cell0DIndex) const
{
    const vector<unsigned int> vertices = Cell2DVertices(cell2DIndex);
    const vector<unsigned int>::const_iterator it = std::find(vertices.begin(), vertices.end(), cell0DIndex);
    if (it != vertices.end())
...
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell2DFindEdge(const unsigned int &cell2DIndex, const unsigned int &cell1DIndex) const
{
    const vector<unsigned int> edges = Cell2DEdges(cell2DIndex);
    const vector<unsigned int>::const_iterator it = std::find(edges.begin(), edges.end(), cell1DIndex);
    if (it != edges.end())
                                                       ...
                                                       const unsigned int &endCell0DIndex) const
{
    ...
    {
        if (_mesh.Cell1DVertices[2 * edges[e]] == originCell0DIndex && _mesh.Cell1DVertices[2 * edges[e] + 1] == endCell0DIndex)
            return e;
    }
...
// ***************************************************************************
void MeshMatricesDAO::Cell2DInsertUpdatedCell2D(const unsigned int &cell2DIndex, const unsigned int &updatedCell2DIdex)
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
...
// ***************************************************************************
bool MeshMatricesDAO::Cell2DUpdatedCell2Ds(const unsigned int &cell2DIndex, list<unsigned int> &updatedCell2DIds) const
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
    ...
    for (const unsigned int &childId : iter->second)
    {
        Cell2DUpdatedCell2Ds(childId, updatedCell2DIds);
    }
...
// ***************************************************************************
void MeshMatricesDAO::Cell2DInitializeNeighbourCell3Ds(const unsigned int &cell2DIndex, const unsigned int &numberNeighbourCell3Ds)
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
...
// ***************************************************************************
void MeshMatricesDAO::Cell2DInitializeNeighbourCell3Ds(const unsigned int &cell2DIndex, const std::vector<unsigned int> &neighbourCell3Ds)
{
    Cell2DInitializeNeighbourCell3Ds(cell2DIndex, neighbourCell3Ds.size());

    for (unsigned int n = 0; n < neighbourCell3Ds.size(); n++)
    {
        Cell2DInsertNeighbourCell3D(cell2DIndex, n, neighbourCell3Ds.at(n));
    }
}
// ***************************************************************************
...
unsigned int MeshMatricesDAO::Cell2DAddDoubleProperty(const string &propertyId)
{
    ...
    _mesh.Cell2DDoublePropertyIndices.insert(pair<string, unsigned int>(propertyId, propertyIndex));
                                                            ...
                                                            const std::vector<unsigned int> &propertySizes)
{
    ...
    InitializeNumberVector(_mesh.Cell2DDoublePropertySizes[propertyIndex], _mesh.Cell2DDoublePropertyValues[propertyIndex], propertySizes, 0.0);
}
// ***************************************************************************
...
void MeshMatricesDAO::Cell2DsInitializeSubDivision(const std::vector<unsigned int> &numberSubDivisions)
{
    ...
    InitializeNumberVector(_mesh.NumberCell2DSubdivision, _mesh.Cell2DSubdivision, numberSubDivisions, static_cast<unsigned int>(0));
}
// ***************************************************************************
void MeshMatricesDAO::Cell2DInitializeSubDivision(const unsigned int &cell2DIndex, const unsigned int &numberSubDivision)
{
    Output::Assert(cell2DIndex < Cell2DTotalNumber());
    Output::Assert(numberSubDivision % 3 == 0);
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell2DSubdivision, _mesh.Cell2DSubdivision, _mesh.NumberCell2D, cell2DIndex, numberSubDivision);
}
// ***************************************************************************
...
void MeshMatricesDAO::Cell3DRemove(const unsigned int &cell3DIndex)
{
    ...
    for (unsigned int p = 0; p < Cell3DNumberDoubleProperties(); p++)
    {
        ResizeNumberVectorWithNewNumberElements(_mesh.Cell3DDoublePropertySizes[p],
                                                ...
                                                cell3DIndex,
                                                0);
        _mesh.Cell3DDoublePropertySizes[p].erase(std::next(_mesh.Cell3DDoublePropertySizes[p].begin(), cell3DIndex));
    }

    _mesh.UpdatedCell3Ds.erase(cell3DIndex);

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DVertices, _mesh.Cell3DVertices, _mesh.NumberCell3D, cell3DIndex, 0);
    _mesh.NumberCell3DVertices.erase(std::next(_mesh.NumberCell3DVertices.begin(), cell3DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DEdges, _mesh.Cell3DEdges, _mesh.NumberCell3D, cell3DIndex, 0);
    _mesh.NumberCell3DEdges.erase(std::next(_mesh.NumberCell3DEdges.begin(), cell3DIndex));

    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DFaces, _mesh.Cell3DFaces, _mesh.NumberCell3D, cell3DIndex, 0);
    _mesh.NumberCell3DFaces.erase(std::next(_mesh.NumberCell3DFaces.begin(), cell3DIndex));
    _mesh.Cell3DOriginalCell3Ds.erase(std::next(_mesh.Cell3DOriginalCell3Ds.begin(), cell3DIndex));
    ...
    AlignContainerHigherElements(_mesh.Cell0DNeighbourCell3Ds, cell3DIndex, std::numeric_limits<unsigned int>::max());
    AlignContainerHigherElements(_mesh.Cell1DNeighbourCell3Ds, cell3DIndex, std::numeric_limits<unsigned int>::max());
    AlignContainerHigherElements(_mesh.Cell2DNeighbourCell3Ds, cell3DIndex, std::numeric_limits<unsigned int>::max());

    AlignMapContainerHigherElements(_mesh.UpdatedCell3Ds, cell3DIndex, _mesh.NumberCell3D);
    AlignContainerHigherElements(_mesh.Cell3DOriginalCell3Ds, cell3DIndex, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DsInitializeVertices(const std::vector<unsigned int> &numberCell3DsVertices)
{
    Gedim::Output::Assert(numberCell3DsVertices.size() == _mesh.NumberCell3D);
...
{
    Gedim::Output::Assert(numberCell3DsEdges.size() == _mesh.NumberCell3D);
    InitializeNumberVector(_mesh.NumberCell3DEdges, _mesh.Cell3DEdges, numberCell3DsEdges, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DsInitializeFaces(const std::vector<unsigned int> &numberCell3DsFaces)
{
    Gedim::Output::Assert(numberCell3DsFaces.size() == _mesh.NumberCell3D);
    InitializeNumberVector(_mesh.NumberCell3DFaces, _mesh.Cell3DFaces, numberCell3DsFaces, std::numeric_limits<unsigned int>::max());
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DInitializeVertices(const unsigned int &cell3DIndex, const unsigned int &numberCell3DVertices)
{
    Output::Assert(cell3DIndex < Cell3DTotalNumber());
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DVertices, _mesh.Cell3DVertices, _mesh.NumberCell3D, cell3DIndex, numberCell3DVertices);
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DInitializeEdges(const unsigned int &cell3DIndex, const unsigned int &numberCell3DEdges)
{
    Output::Assert(cell3DIndex < Cell3DTotalNumber());
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DEdges, _mesh.Cell3DEdges, _mesh.NumberCell3D, cell3DIndex, numberCell3DEdges);
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DInitializeFaces(const unsigned int &cell3DIndex, const unsigned int &numberCell3DFaces)
{
    Output::Assert(cell3DIndex < Cell3DTotalNumber());
    ResizeNumberVectorWithNewNumberElements(_mesh.NumberCell3DFaces, _mesh.Cell3DFaces, _mesh.NumberCell3D, cell3DIndex, numberCell3DFaces);
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DAddVertices(const unsigned int &cell3DIndex, const vector<unsigned int> &verticesCell0DIndices)
{
    Cell3DInitializeVertices(cell3DIndex, verticesCell0DIndices.size());
    for (unsigned int v = 0; v < verticesCell0DIndices.size(); v++)
        Cell3DInsertVertex(cell3DIndex, v, verticesCell0DIndices[v]);
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DAddEdges(const unsigned int &cell3DIndex, const vector<unsigned int> &edgesCell1DIndices)
{
    Cell3DInitializeEdges(cell3DIndex, edgesCell1DIndices.size());
    for (unsigned int e = 0; e < edgesCell1DIndices.size(); e++)
        Cell3DInsertEdge(cell3DIndex, e, edgesCell1DIndices[e]);
}
// ***************************************************************************
void MeshMatricesDAO::Cell3DAddFaces(const unsigned int &cell3DIndex, const vector<unsigned int> &facesCell2DIndices)
{
    Cell3DInitializeFaces(cell3DIndex, facesCell2DIndices.size());
    for (unsigned int e = 0; e < facesCell2DIndices.size(); e++)
        Cell3DInsertFace(cell3DIndex, e, facesCell2DIndices[e]);
}
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell3DFindVertex(const unsigned int &cell3DIndex, const unsigned int &cell0DIndex) const
{
...
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell3DFindEdge(const unsigned int &cell3DIndex, const unsigned int &cell1DIndex) const
{
...
// ***************************************************************************
unsigned int MeshMatricesDAO::Cell3DFindFace(const unsigned int &cell3DIndex, const unsigned int &cell2DIndex) const
{
    const vector<unsigned int> faces = Cell3DFaces(cell3DIndex);
    const vector<unsigned int>::const_iterator it = std::find(faces.begin(), faces.end(), cell2DIndex);
    if (it != faces.end())
...
// ***************************************************************************
void MeshMatricesDAO::Cell3DInsertUpdatedCell3D(const unsigned int &cell3DIndex, const unsigned int &updatedCell3DIdex)
{
    Output::Assert(cell3DIndex < Cell3DTotalNumber());
...
// ***************************************************************************
bool MeshMatricesDAO::Cell3DUpdatedCell3Ds(const unsigned int &cell3DIndex, list<unsigned int> &updatedCell3DIds) const
{
    Output::Assert(cell3DIndex < Cell3DTotalNumber());
    ...
    for (const unsigned int &childId : iter->second)
    {
        Cell3DUpdatedCell3Ds(childId, updatedCell3DIds);
    }
...
unsigned int MeshMatricesDAO::Cell3DAddDoubleProperty(const string &propertyId)
{
    ...
    _mesh.Cell3DDoublePropertyIndices.insert(pair<string, unsigned int>(propertyId, propertyIndex));
                                                            ...
                                                            const std::vector<unsigned int> &propertySizes)
{
    ...
    InitializeNumberVector(_mesh.Cell3DDoublePropertySizes[propertyIndex], _mesh.Cell3DDoublePropertyValues[propertyIndex], propertySizes, 0.0);
}
// ***************************************************************************
