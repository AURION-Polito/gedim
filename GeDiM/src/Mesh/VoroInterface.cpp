...
#else
// ************************************************************************* //
bool VoroInterface::InsertNewPoints(Cell0D &cell0D, list<Cell0D> &cell0Ds)
{
    if (cell0Ds.size() == 0)
    {
        cell0D.id = 0;
                                                   ...
                                                   const unsigned int random_seed)
{
    ...
    GenerateRandomPoints(polygonVertices, numPoints, VoronoiPoints, random_seed);
                                                   ...
                                                   Gedim::IMeshDAO &mesh)
{
                    ...
                    for (unsigned int i = 0; i < nFaces; i++)
                    {
                        ...
                        if (faceNeigh < 0) // if it is a boundary face
                        {
                            ...
                            const Eigen::MatrixXd convexCell2DUnalignedVertices =
                                geometryUtilities.ExtractPoints(faceVertCoordinates, convexCell2DUnalignedVerticesFilter);
                            ...
                            const vector<Eigen::Matrix3d> convexCell2DTriangulationPoints =
                                geometryUtilities.ExtractTriangulationPoints(faceVertCoordinates, convexCell2DTriangulation);
    ...
    for (const Cell0D &cell0D : cell0Ds)
    {
        ...
        mesh.Cell0DInsertCoordinates(id, Eigen::Vector3d(cell0D.x, cell0D.y, cell0D.z));

        const bool is_xmin = (geometryUtilities.IsValueZero(cell0D.x - x_min, geometryUtilities.Tolerance1D())); // l ==
                                                                                                                 // 0
        const bool is_xmax = (geometryUtilities.IsValueZero(cell0D.x - x_max, geometryUtilities.Tolerance1D())); // l ==
                                                                                                                 // (numLengthPoints
                                                                                                                 // - 1)
        const bool is_ymin = (geometryUtilities.IsValueZero(cell0D.y - y_min, geometryUtilities.Tolerance1D())); // h ==
                                                                                                                 // 0
        const bool is_ymax = (geometryUtilities.IsValueZero(cell0D.y - y_max, geometryUtilities.Tolerance1D())); // h ==
                                                                                                                 // (numHeightPoints
                                                                                                                 // - 1)
    ...
    for (const auto &cell1D : cell1Ds)
    {
        ...
        mesh.Cell1DInsertExtremes(e, cell1D.first.first, cell1D.first.second);
    }
                                                   ...
                                                   const unsigned int random_seed)
{
    ...
    Eigen::MatrixXd VoronoiPoints;
    GenerateRandomPoints(polyhedronVertices, numPoints, VoronoiPoints, random_seed);
                            ...
                            for (unsigned int f = 0; f < cellFaceNeighbour[faceNeighs[i]].size(); f++)
                            {
                                ...
                                if (cellFaceNeighbour[faceNeighs[i]][f] == ID)
                                {
                                    ...
                                    for (unsigned int e = 0; e < cell2Ds[cellFaces[ID][i]].edges.size(); e++)
                                        if (find(cellEdges[ID].begin(), cellEdges[ID].end(), cell2Ds[cellFaces[ID][i]].edges[e]) ==
                                            cellEdges[ID].end())
    ...
    for (const Cell0D &cell0D : cell0Ds)
    {
        ...
        const bool is_ymax = (geometryUtilities.IsValueZero(cell0D.y - y_max, geometryUtilities.Tolerance1D())); // h ==
                                                                                                                 // (numHeightPoints
                                                                                                                 // - 1)
        const bool is_zmin = (geometryUtilities.IsValueZero(cell0D.z - z_min, geometryUtilities.Tolerance1D())); // w ==
                                                                                                                 // 0
        const bool is_zmax = (geometryUtilities.IsValueZero(cell0D.z - z_max, geometryUtilities.Tolerance1D())); // w ==
                                                                                                                 // (numWidthPoints
                                                                                                                 // - 1)
