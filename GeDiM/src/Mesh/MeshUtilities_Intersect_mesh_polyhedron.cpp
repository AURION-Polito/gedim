...
{
// ***************************************************************************
Gedim::MeshUtilities::Intersect_mesh_polyhedron_result Gedim::MeshUtilities::Intersect_mesh_polyhedron(
    ...
    const std::vector<std::vector<Eigen::Matrix3d>> &mesh_cell3Ds_faces_rotation_matrix) const
{
    ...
    auto find_intersection = [&geometry_utilities](std::list<Intersection> &intersections, Eigen::Vector3d new_intersection_coordinates) {
        ...
        for (std::list<Intersection>::iterator it = intersections.begin(); it != intersections.end(); it++)
        {
            ...
            if (!geometry_utilities.PointsAreCoincident(intersection.Intersection_coordinates, new_intersection_coordinates))
                continue;
    ...
    for (unsigned int p = 0; p < mesh.Cell0DTotalNumber(); ++p)
    {
        ...
        if (!geometry_utilities.IsPointInBoundingBox(cell0D_coordinates, polyhedron_boudingBox))
            continue;
    ...
    for (unsigned int e = 0; e < mesh.Cell1DTotalNumber(); ++e)
    {
        ...
        if (!geometry_utilities.BoundingBoxesIntersects(mesh_cell1Ds_boudingBox.at(e), polyhedron_boudingBox))
            continue;
        ...
        {
            if (!geometry_utilities.BoundingBoxesIntersects(mesh_cell1Ds_boudingBox.at(e), polyhedron_faces_boudingBox.at(p_f)))
                continue;
            ...
            const Eigen::Vector3d intersection_coordinate =
                cell1D_vertices.col(0) + intersection_face_plane_cell1D.SingleIntersection.CurvilinearCoordinate * cell1D_tangent;

            const Eigen::Vector3d intersection_2D =
                geometry_utilities.RotatePointsFrom3DTo2D(intersection_coordinate, face_rotation_matrix.transpose(), face_translation);

            const auto intersection_face_position = geometry_utilities.PointPolygonPosition(intersection_2D, face_2D_vertices);

            if (intersection_face_position.Type == Gedim::GeometryUtilities::PointPolygonPositionResult::Types::Outside)
                continue;

            auto intersection_found = find_intersection(intersections, intersection_coordinate);
    ...
    for (unsigned int f = 0; f < mesh.Cell2DTotalNumber(); ++f)
    {
        ...
        if (!geometry_utilities.BoundingBoxesIntersects(mesh_cell2Ds_boudingBox.at(f), polyhedron_boudingBox))
            continue;
        ...
        {
            if (!geometry_utilities.BoundingBoxesIntersects(mesh_cell2Ds_boudingBox.at(f), polyhedron_edges_boudingBox.at(p_e)))
                continue;
            ...
            const auto intersection_cell2D_plane_edge =
                geometry_utilities.IntersectionSegmentPlane(polyhedron_edge_vertices.col(0),
            ...
            const Eigen::Vector3d intersection_coordinate =
                polyhedron_edge_vertices.col(0) +
                intersection_cell2D_plane_edge.SingleIntersection.CurvilinearCoordinate * polyhedron_edge_tangent;

            const Eigen::Vector3d intersection_2D =
                geometry_utilities.RotatePointsFrom3DTo2D(intersection_coordinate, cell2D_rotation_matrix.transpose(), cell2D_translation);

            if (!geometry_utilities.IsPointInsidePolygon(intersection_2D, cell2D_2D_vertices))
                continue;
    ...
    for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); ++c)
    {
        ...
        if (!geometry_utilities.BoundingBoxesIntersects(mesh_cell3Ds_boudingBox.at(c), polyhedron_boudingBox))
            continue;
        ...
        for (unsigned int p_v = 0; p_v < polyhedron_vertices.cols(); ++p_v)
        {
            ...
            if (!geometry_utilities.IsPointInBoundingBox(vertex, mesh_cell3Ds_boudingBox.at(c)))
                continue;
            ...
            case GeometryUtilities::PointPolyhedronPositionResult::Types::BorderEdge:
            case GeometryUtilities::PointPolyhedronPositionResult::Types::BorderVertex: {
                auto intersection_found = find_intersection(intersections, vertex);
    ...
    {
        return {Intersect_mesh_polyhedron_result::Types::None, {}, {}, {}};
    }
    ...
    for (const auto &intersection : intersections)
    {
        ...
        result.Polyhedron_intersections[i].Cell0Ds_index =
            std::vector<unsigned int>(intersection.Cell0Ds_index.begin(), intersection.Cell0Ds_index.end());
        result.Polyhedron_intersections[i].Cell1Ds_index =
            std::vector<unsigned int>(intersection.Cell1Ds_index.begin(), intersection.Cell1Ds_index.end());
        result.Polyhedron_intersections[i].Cell2Ds_index =
            std::vector<unsigned int>(intersection.Cell2Ds_index.begin(), intersection.Cell2Ds_index.end());
        result.Polyhedron_intersections[i].Cell3Ds_index =
            std::vector<unsigned int>(intersection.Cell3Ds_index.begin(), intersection.Cell3Ds_index.end());
    ...
    {
        result.Mesh_intersections.Cell1Ds_intersections.insert(std::make_pair(
            cell1D_intersections.first,
            std::vector<unsigned int>(cell1D_intersections.second.begin(), cell1D_intersections.second.end())));
    }

    for (const auto &cell2D_intersections : cell2Ds_intersections)
    {
        result.Mesh_intersections.Cell2Ds_intersections.insert(std::make_pair(
            cell2D_intersections.first,
            std::vector<unsigned int>(cell2D_intersections.second.begin(), cell2D_intersections.second.end())));
    }

    for (const auto &cell3D_intersections : cell3Ds_intersections)
    {
        result.Mesh_intersections.Cell3Ds_intersections.insert(std::make_pair(
            cell3D_intersections.first,
            std::vector<unsigned int>(cell3D_intersections.second.begin(), cell3D_intersections.second.end())));
    }
