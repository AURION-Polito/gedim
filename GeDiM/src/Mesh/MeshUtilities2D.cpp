...
namespace Gedim
{
  ...
  void MeshUtilities::ComputeCell0DCell1DNeighbours(IMeshDAO& mesh) const
  {
    ...
    for (unsigned int c0D = 0; c0D < mesh.Cell0DTotalNumber(); c0D++)
    {
      ...
      for (const auto& cell1DIndex : cell0DsNeighbours[c0D])
        mesh.Cell0DInsertNeighbourCell1D(c0D,
                                         n++,
                                         cell1DIndex);
    }
  ...
  void MeshUtilities::ComputeCell0DCell2DNeighbours(IMeshDAO& mesh) const
  {
    ...
    for (unsigned int c0D = 0; c0D < mesh.Cell0DTotalNumber(); c0D++)
    {
      ...
      for (const auto& cell2DIndex : cell0DsNeighbours[c0D])
        mesh.Cell0DInsertNeighbourCell2D(c0D,
                                         n++,
                                         cell2DIndex);
    }
    ...
    else if(mesh.Dimension() == 3)
    {
      ...
      for (unsigned int c1D = 0; c1D < mesh.Cell1DTotalNumber(); c1D++)
      {
        ...
        for (const auto& cell2DIndex : cell1DsNeighbours[c1D])
          mesh.Cell1DInsertNeighbourCell2D(c1D,
                                           n++,
                                           cell2DIndex);
      }
    }
  ...
  // ***************************************************************************
  unsigned int MeshUtilities::AgglomerateCell2Ds(const std::vector<unsigned int>& subCell2DsIndex,
                                                 ...
                                                 std::vector<std::vector<unsigned int>>& meshCell2DToConvexCell2DIndices) const
  {
    ...
    for (const auto subCell2DIndex : subCell2DsIndex)
    {
      ...
      const auto& convexCells = meshCell2DToConvexCell2DIndices.at(subCell2DIndex);
      for (const auto& convexCell : convexCells)
        agglomeratedCell2DConvexCells.push_back(convexCell);
    }
    meshCell2DToConvexCell2DIndices.resize(meshCell2DToConvexCell2DIndices.size() + 1);
    meshCell2DToConvexCell2DIndices.at(agglomeratedCell2DIndex) =
        std::vector<unsigned int>(agglomeratedCell2DConvexCells.begin(),
                                  agglomeratedCell2DConvexCells.end());

    mesh.Cell2DAddVertices(agglomeratedCell2DIndex,
                           agglomerateCell2DVertices);
    mesh.Cell2DAddEdges(agglomeratedCell2DIndex,
                        agglomerateCell2DEdges);

    mesh.Cell3DSetMarker(agglomeratedCell2DIndex,
                         max_marker);
    mesh.Cell3DSetState(agglomeratedCell2DIndex,
                        true);
    ...
    for (unsigned int v = 0; v < mesh.Cell2DNumberVertices(agglomeratedCell2DIndex); v++)
    {
      ...
      for (unsigned int n = 0; n < mesh.Cell0DNumberNeighbourCell2D(cell0DIndex); n++)
      {
        ...
        const unsigned int neighCell2DIndex = mesh.Cell0DNeighbourCell2D(cell0DIndex,
                                                                         n);

        if (std::find(subCell2DsIndex.begin(),
                      subCell2DsIndex.end(),
                      neighCell2DIndex) != subCell2DsIndex.end())
        {
          mesh.Cell0DInsertNeighbourCell2D(cell0DIndex,
                                           n,
    ...
    for (unsigned int e = 0; e < mesh.Cell2DNumberEdges(agglomeratedCell2DIndex); e++)
    {
      ...
      for (unsigned int n = 0; n < mesh.Cell1DNumberNeighbourCell2D(cell1DIndex); n++)
      {
        ...
        const unsigned int neighCell2DIndex = mesh.Cell1DNeighbourCell2D(cell1DIndex,
                                                                         n);
