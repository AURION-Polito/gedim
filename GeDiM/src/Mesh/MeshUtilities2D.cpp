                                                                    ...
                                                                    const unsigned int starting_cell2D_index) const
{
    ...
    for (unsigned int c = 0; c < mesh.Cell2DTotalNumber(); ++c)
    {
        ...
        if (!geometryUtilities.IsPointInBoundingBox(point, cell2DsBoundingBox.at(c2D_index)))
            continue;

        const auto pointPosition = geometryUtilities.PointPolygonPosition(point, cell2DsVertices.at(c2D_index));
