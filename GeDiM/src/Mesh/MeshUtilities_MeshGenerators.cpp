                                        ...
                                        Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int h = 0; h < numHeightPoints; h++)
    {
        ...
        {
            const Eigen::Vector3d coordinate = rectangleOrigin + baseMeshCurvilinearCoordinates[b] * rectangleBaseTangent +
    ...
    for (unsigned int h = 0; h < numHeightPoints; h++)
    {
        for (unsigned int b = 0; b < numBasePoints - 1; b++)
        {
            ...
            const unsigned int marker = 5 * (h == 0) + 7 * (h == (numHeightPoints - 1));

            mesh.Cell1DInsertExtremes(cell1DIndex, cell1DOrigin, cell1DEnd);
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h++)
    {
        for (unsigned int b = 0; b < numBasePoints; b++)
        {
            ...
            const unsigned int marker = 8 * (b == 0) + 6 * (b == (numBasePoints - 1));
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h++)
    {
        for (unsigned int b = 0; b < numBasePoints - 1; b++)
        {
            ...
            vector<unsigned int> cell2DVertices = {cell0DIndex, cell0DIndex + 1, cell0DIndex + numBasePoints + 1, cell0DIndex + numBasePoints};
...
// ***************************************************************************
void MeshUtilities::CreateStructuredTriangularMesh(const Eigen::Vector3d &rectangleOrigin,
                                                   const Eigen::Vector3d &rectangleBaseTangent,
                                                   const Eigen::Vector3d &rectangleHeightTangent,
                                                   ...
                                                   Gedim::IMeshDAO &mesh) const
{
    ...
    const unsigned int numCell0Ds = numBasePoints * numHeightPoints;
    const unsigned int numCell1Ds = numHeightPoints * (numBasePoints - 1) + numBasePoints * (numHeightPoints - 1) +
                                    (numBasePoints - 1) * (numHeightPoints - 1);
    const unsigned int numCell2Ds = (numBasePoints - 1) * (numHeightPoints - 1) * 2;
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h++)
    {
        for (unsigned int b = 0; b < numBasePoints - 1; b++)
        {
            const unsigned int cell0DIndex = b + h * numBasePoints;
            const unsigned int cell1DOrigin = cell0DIndex;
            const unsigned int cell1DEnd = cell0DIndex + numBasePoints + 1;
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h++)
    {
        for (unsigned int b = 0; b < numBasePoints - 1; b++)
        {
            ...
            const unsigned int cell1DHorizontalIndex = b + h * (numBasePoints - 1);
            const unsigned int cell1DVerticalIndex = cell0DIndex + numHeightPoints * (numBasePoints - 1);
            const unsigned int cell1DObliqueIndex = numHeightPoints * (numBasePoints - 1) +
                                                    (numHeightPoints - 1) * numBasePoints + b + h * (numBasePoints - 1);

            vector<unsigned int> cell2DVertices = {cell0DIndex, cell0DIndex + 1, cell0DIndex + numBasePoints + 1};
            vector<unsigned int> cell2DEdges = {cell1DHorizontalIndex, cell1DVerticalIndex + 1, cell1DObliqueIndex};
            ...
            vector<unsigned int> cell2DVertices_2 = {cell0DIndex, cell0DIndex + numBasePoints + 1, cell0DIndex + numBasePoints};
            vector<unsigned int> cell2DEdges_2 = {cell1DObliqueIndex, cell1DHorizontalIndex + (numBasePoints - 1), cell1DVerticalIndex};

            mesh.Cell2DInsertVertices(cell2DIndex, cell2DVertices_2);
            mesh.Cell2DInsertEdges(cell2DIndex, cell2DEdges_2);
                                                       ...
                                                       Gedim::IMeshDAO &mesh) const
{
    ...
    const unsigned int numCell0Ds =
        ...
        floor(numHeightPoints * 0.5) *
            (floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
    ...
    const unsigned int numCell1Ds =
        ceil(numHeightPoints * 0.5) *
            ((ceil(numBasePoints * 0.5) + 1 + (floor(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) - 1) +
        floor(numHeightPoints * 0.5) *
            ((floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) - 1) +
        numHeightPoints * floor(numHangBasis * 0.5) * (numBasePoints - 1) +
    ...
    for (unsigned int h = 0; h < numHeightPoints; h++)
    {
        for (unsigned int b = 0; b < numBasePoints; b++)
        {
            ...
            if ((b == 0 || b == numBasePoints - 1 || numHangBasis % 2 == 1) || (b % 2 == 0 && h % 2 == 1) ||
                (b % 2 == 1 && h % 2 == 0))
            {
                ...
                if (b != 0)
                {
                    ...
                    mesh.Cell1DInsertExtremes(cell1DIndex, cell0DIndex - 2, cell0DIndex - 1);
            ...
            {
                std::vector<double> curvilinearPoints =
                    geometryUtilities.EquispaceCoordinates(numAddedPoints,
                ...
                {
                    const Eigen::Vector3d coordinate = rectangleOrigin + curvilinearPoints[s] * rectangleBaseTangent +
                                                       heightMeshCurvilinearCoordinates[h] * rectangleHeightTangent;
    ...
    const unsigned int numberCell0DsEven =
        (ceil(numBasePoints * 0.5) + 1 + (floor(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
        floor(numHangBasis * 0.5) * (numBasePoints - 1);

    const unsigned int numberCell0DsOdd =
        (floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
        floor(numHangBasis * 0.5) * (numBasePoints - 1);

    unsigned int cell1DOrigin = 0;

    std::vector<double> curvilinearPointsUpper = geometryUtilities.EquispaceCoordinates(numHangDiagonalUpper, 0.0, 1.0, false);
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h = h + 2)
    {
        for (unsigned int b = 0; b < numBasePoints; b++)
        {
            ...
            for (unsigned int s = 0; s < numHangDiagonalUpper; s++)
            {
                ...
                mesh.Cell1DInsertExtremes(cell1DIndex, cell1DLocalOrigin, cell0DIndex - 1);
            ...
            mesh.Cell1DInsertExtremes(cell1DIndex, cell1DLocalOrigin, cell1DEnd);
    ...
    std::vector<double> curvilinearPointsLower = geometryUtilities.EquispaceCoordinates(numHangDiagonalLower, 0.0, 1.0, false);
    ...
    for (unsigned int h = 1; h < numHeightPoints - 1; h = h + 2)
    {
        for (unsigned int b = 0; b < numBasePoints; b++)
        {
            ...
            const unsigned int marker = 8 * (b == 0) + 6 * (b == (numBasePoints - 1)) * (numBasePoints % 2 == 0);
    ...
    unsigned int upperOffsetVertices =
        ceil(numHeightPoints * 0.5) * (ceil(numBasePoints * 0.5) + 1 + (floor(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
        floor(numHeightPoints * 0.5) *
            (floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
        numHeightPoints * (floor(numHangBasis * 0.5)) * (numBasePoints - 1);

    unsigned int lowerOffsetVertices =
        upperOffsetVertices + floor(numHeightPoints * 0.5) * (numBasePoints + 1 - ceil(numBasePoints * 0.5)) * numHangDiagonalUpper;

    unsigned int upperOffsetEdges =
        ceil(numHeightPoints * 0.5) *
            ((ceil(numBasePoints * 0.5) + 1 + (floor(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) - 1) +
        floor(numHeightPoints * 0.5) *
            ((floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) - 1) +
    ...
    unsigned int lowerOffsetEdges = upperOffsetEdges + floor(numHeightPoints * 0.5) *
                                                           (numBasePoints + 1 - ceil(numBasePoints * 0.5)) *
                                                           (numHangDiagonalUpper + 1);
    ...
    for (unsigned int h = 0; h < numHeightPoints - 1; h = h + 2)
    {
        for (unsigned int l = 0; l < ceil(numBasePoints * 0.5); l++)
        {
            ...
            vector<unsigned int> cell2DVertices(3 + numLocalAddedPoints + numHangDiagonalUpper + numHangDiagonalLower);

            std::iota(cell2DVertices.begin(), cell2DVertices.begin() + numLocalAddedPoints + 2, orizontalOffsetVertices);
            ...
            std::generate(cell2DVertices.begin() + numLocalAddedPoints + 3 + numHangDiagonalLower, cell2DVertices.end(), [&n] {
                return n--;
            });

            vector<unsigned int> cell2DEdges(3 + numLocalAddedPoints + numHangDiagonalUpper + numHangDiagonalLower);

            std::iota(cell2DEdges.begin(), cell2DEdges.begin() + numLocalAddedPoints + 1, orizontalOffsetEdges);
            ...
            std::generate(cell2DEdges.begin() + numLocalAddedPoints + 2 + numHangDiagonalLower, cell2DEdges.end(), [&n] {
                return n--;
            });
        ...
        for (unsigned int l = 0; l < (numBasePoints + 1 - ceil(numBasePoints * 0.5)) - 1; l++)
        {
            ...
            std::generate(cell2DVertices.begin(), cell2DVertices.begin() + numLocalAddedPoints + 2, [&n] { return n--; });
            ...
            std::iota(cell2DVertices.begin() + numLocalAddedPoints + 3 + numHangDiagonalLower, cell2DVertices.end(), upperOffsetVertices);

            vector<unsigned int> cell2DEdges(3 + numLocalAddedPoints + numHangDiagonalUpper + numHangDiagonalLower);

            n = orizontalOffsetEdges + numLocalAddedPoints;
            std::generate(cell2DEdges.begin(), cell2DEdges.begin() + numLocalAddedPoints + 1, [&n] { return n--; });
            ...
            std::iota(cell2DEdges.begin() + numLocalAddedPoints + 2 + numHangDiagonalLower, cell2DEdges.end(), upperOffsetEdges);
    ...
    lowerOffsetVertices = ceil(numHeightPoints * 0.5) *
                              (ceil(numBasePoints * 0.5) + 1 + (floor(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
                          floor(numHeightPoints * 0.5) *
                              (floor(numBasePoints * 0.5) + 1 + (ceil(numBasePoints * 0.5) - 1) * (numHangBasis % 2 == 1)) +
                          numHeightPoints * (floor(numHangBasis * 0.5)) * (numBasePoints - 1) +
    ...
    upperOffsetVertices = lowerOffsetVertices + (ceil(numHeightPoints * 0.5) - 1) *
                                                    (numBasePoints + 1 - ceil(numBasePoints * 0.5)) * numHangDiagonalLower;
    ...
    upperOffsetEdges = lowerOffsetEdges + (ceil(numHeightPoints * 0.5) - 1) *
                                              (numBasePoints + 1 - ceil(numBasePoints * 0.5)) * (numHangDiagonalLower + 1);
                                                        ...
                                                        Gedim::IMeshDAO &mesh) const
{
    ...
    unsigned int totalAddedHangNodes =
        std::accumulate(numberOfAddedVerticesForEachRectangle.begin(), numberOfAddedVerticesForEachRectangle.end(), 0);
    ...
    else
    {
        ...
        const unsigned int numCell1Ds =
            ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) * (numberOfAddedVerticesForEachRectangle[0] + 1) +
            ceil(numBasePoints * 0.5) * (numHeightPoints - 1) * (numberOfAddedVerticesForEachRectangle[1] + 1) +
            (numHeightPoints / 2) * (numBasePoints - 1) * (numberOfAddedVerticesForEachRectangle[2] + 1) +
            (numBasePoints / 2) * (numHeightPoints - 1) * (numberOfAddedVerticesForEachRectangle[3] + 1);
        ...
        for (unsigned int h = 0; h < numHeightPoints; h = h + 2)
        {
            for (unsigned int b = 0; b < (numBasePoints - 1); b++)
            {
                ...
                std::vector<double> curvilinearPoints =
                    geometryUtilities.EquispaceCoordinates(numberOfAddedVerticesForEachRectangle[0],
                                                           baseMeshCurvilinearCoordinates[b],
                                                           baseMeshCurvilinearCoordinates[b + 1],
                                                           false);
        ...
        for (unsigned int h = 0; h < (numHeightPoints - 1); h++)
        {
            for (unsigned int b = 0; b < numBasePoints; b = b + 2)
            {
                ...
                std::vector<double> curvilinearPoints =
                    geometryUtilities.EquispaceCoordinates(numberOfAddedVerticesForEachRectangle[1],
        ...
        for (unsigned int h = 1; h < numHeightPoints; h = h + 2)
        {
            ...
            {
                std::vector<double> curvilinearPoints =
                    geometryUtilities.EquispaceCoordinates(numberOfAddedVerticesForEachRectangle[2],
                                                           baseMeshCurvilinearCoordinates[b],
                                                           baseMeshCurvilinearCoordinates[b + 1],
                                                           false);
        ...
        for (unsigned int h = 0; h < (numHeightPoints - 1); h++)
        {
            ...
            {
                std::vector<double> curvilinearPoints =
                    geometryUtilities.EquispaceCoordinates(numberOfAddedVerticesForEachRectangle[3],
                                                           heightMeshCurvilinearCoordinates[h],
                                                           heightMeshCurvilinearCoordinates[h + 1],
                                                           false);
        ...
        unsigned int cell0DIndexBaseHangsOddB =
            numBasePoints * numHeightPoints - 1 +
            ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) * numberOfAddedVerticesForEachRectangle[0] +
            ceil(numBasePoints * 0.5) * (numHeightPoints - 1) * numberOfAddedVerticesForEachRectangle[1];
        ...
        unsigned int cell0DIndexBaseHangsEvenH =
            numBasePoints * numHeightPoints - 1 +
            ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) * numberOfAddedVerticesForEachRectangle[0];
        ...
        unsigned int cell0DIndexBaseHangsOddH =
            numBasePoints * numHeightPoints - 1 +
            ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) * numberOfAddedVerticesForEachRectangle[0] +
            ceil(numBasePoints * 0.5) * (numHeightPoints - 1) * numberOfAddedVerticesForEachRectangle[1] +
            (numHeightPoints / 2) * (numBasePoints - 1) * numberOfAddedVerticesForEachRectangle[2];
        ...
        cell0DIndexBaseHangsEvenH = cell0DIndexBaseHangsEvenB + ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) *
                                                                    numberOfAddedVerticesForEachRectangle[0];

        cell0DIndexBaseHangsOddB = cell0DIndexBaseHangsEvenH + ceil(numBasePoints * 0.5) * (numHeightPoints - 1) *
                                                                   (numberOfAddedVerticesForEachRectangle[1]);

        cell0DIndexBaseHangsOddH = cell0DIndexBaseHangsOddB + (numHeightPoints / 2) * (numBasePoints - 1) *
                                                                  (numberOfAddedVerticesForEachRectangle[2]);

        unsigned int cell1DHorizontalEvenB = 0;
        unsigned int cell1DHorizontalOddB =
            ceil((numHeightPoints * 0.5)) * (numBasePoints - 1) * (numberOfAddedVerticesForEachRectangle[0] + 1);
        unsigned int cell1DVerticalEvenH = cell1DHorizontalOddB + (numHeightPoints / 2) * (numBasePoints - 1) *
                                                                      (numberOfAddedVerticesForEachRectangle[2] + 1);
        unsigned int cell1DVerticalOddH = cell1DVerticalEvenH + ceil(numBasePoints * 0.5) * (numHeightPoints - 1) *
                                                         ...
                                                         Gedim::IMeshDAO &mesh) const
{
    ...
    vector<double> baseMeshCurvilinearCoordinates =
        geometryUtilities.EquispaceCoordinates(numQuadrilateralsBaseTangent + 1, 0.0, 1.0, 1);

    vector<double> heightMeshCurvilinearCoordinates =
        geometryUtilities.EquispaceCoordinates(numQuadrilateralsHeightTangent + 1, 0.0, 1.0, 1);
    ...
    for (unsigned int h = 0; h < numHeightPoints; h++)
    {
        for (unsigned int b = 0; b < numBasePoints; b++)
        {
            ...
            const Eigen::Vector3d coordinate =
                rectangleOrigin + baseMeshCurvilinearCoordinates[b] * rectangleBaseTangent +
        ...
        for (unsigned int h = 0; h < numHeightPoints; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints; l++)
            {
                ...
                const unsigned int marker =
                    1 * (l == 0 && h == 0 && w == 0) + 2 * (l == (numLengthPoints - 1) && h == 0 && w == 0) +
                    3 * (l == (numLengthPoints - 1) && h == (numHeightPoints - 1) && w == 0) +
                    4 * (l == 0 && h == (numHeightPoints - 1) && w == 0) + 5 * (l == 0 && h == 0 && w == (numWidthPoints - 1)) +
                    ...
                    22 * (l != 0 && l != (numLengthPoints - 1) && h != 0 && h != (numHeightPoints - 1) &&
                          w == (numWidthPoints - 1)) +
                    23 * (l == 0 && h != 0 && h != (numHeightPoints - 1) && w != 0 && w != (numWidthPoints - 1)) +
                    24 * (l == (numLengthPoints - 1) && h != 0 && h != (numHeightPoints - 1) && w != 0 &&
                          w != (numWidthPoints - 1)) +
    ...
    for (unsigned int w = 0; w < numWidthPoints; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints - 1; l++)
            {
                ...
                const unsigned int marker = 9 * (h == 0 && w == 0) + 11 * (h == (numHeightPoints - 1) && w == 0) +
    ...
    for (unsigned int w = 0; w < numWidthPoints; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints - 1; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints; l++)
            {
                ...
                const unsigned int marker = 10 * (l == (numLengthPoints - 1) && w == 0) + 12 * (l == 0 && w == 0) +
    ...
    for (unsigned int w = 0; w < numWidthPoints - 1; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints; l++)
            {
                ...
                const unsigned int marker = 17 * (l == 0 && h == 0) + 18 * (l == (numLengthPoints - 1) && h == 0) +
    ...
    for (unsigned int w = 0; w < numWidthPoints; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints - 1; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints - 1; l++)
            {
                ...
                vector<unsigned int> cell2DVertices = {cell0DIndex, cell0DIndex + 1, cell0DIndex + numLengthPoints + 1, cell0DIndex + numLengthPoints};
                vector<unsigned int> cell2DEdges = {cell1DHorizontalIndex,
                                                    cell1DVerticalIndex + 1,
                ...
                const unsigned int marker = 21 * (w == 0) + 22 * (w == (numWidthPoints - 1));
    ...
    for (unsigned int w = 0; w < numWidthPoints - 1; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints - 1; l++)
            {
                const unsigned int cell0DIndex = l + h * numLengthPoints + w * numLengthPoints * numHeightPoints;
                const unsigned int cell1DHorizontalIndex = l + h * (numLengthPoints - 1) + w * (numLengthPoints - 1) * numHeightPoints;
                const unsigned int cell1DVerticalIndex = cell0DIndex + numHeightPoints * numWidthPoints * (numLengthPoints - 1) +
                                                         numLengthPoints * numWidthPoints * (numHeightPoints - 1);
                ...
                const unsigned int marker = 25 * (h == 0) + 26 * (h == (numHeightPoints - 1));
    ...
    for (unsigned int w = 0; w < numWidthPoints - 1; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints - 1; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints; l++)
            {
                ...
                const unsigned int marker = 23 * (l == 0) + 24 * (l == (numLengthPoints - 1));
    ...
    for (unsigned int w = 0; w < numWidthPoints - 1; w++)
    {
        for (unsigned int h = 0; h < numHeightPoints - 1; h++)
        {
            for (unsigned int l = 0; l < numLengthPoints - 1; l++)
            {
                ...
                const unsigned int cell2DHorizontalIndex =
                    l + h * (numLengthPoints - 1) + w * (numLengthPoints - 1) * (numHeightPoints - 1);
                                         ...
                                         Gedim::IMeshDAO &mesh) const
{
    ...
    tetgenInterface.CreateDelaunay(points, points_marker, mesh);
}
// ***************************************************************************
                                         ...
                                         const unsigned int random_seed) const
{
    ...
    voroInterface.GenerateVoronoiTassellations3D(polyhedronVertices, polyhedronEdges, polyhedronFaces, numPoints, numIterations, mesh, random_seed);
}
// ***************************************************************************
} // namespace Gedim
