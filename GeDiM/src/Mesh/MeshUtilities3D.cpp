    ...
    const Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int cell3DIndex = 0; cell3DIndex < mesh.Cell3DTotalNumber(); cell3DIndex++)
    {
        ...
        for (unsigned int ae = 0; ae < cell3DAlignedEdgesVertices.size(); ae++)
        {
            ...
            const bool direction = mesh.Cell3DVertex(cell3DIndex, cell3DAlignedEdgeVertices.at(0)) <
                                   mesh.Cell3DVertex(cell3DIndex, cell3DAlignedEdgeVertices.at(alignedEdgeNumVertices - 1));

            const unsigned int alignedEdgeOrigin = direction ? cell3DAlignedEdgeVertices.at(0)
                                                             : cell3DAlignedEdgeVertices.at(alignedEdgeNumVertices - 1);
            const unsigned int alignedEdgeEnd = direction ? cell3DAlignedEdgeVertices.at(alignedEdgeNumVertices - 1)
                                                          : cell3DAlignedEdgeVertices.at(0);
            const unsigned int cell0DOrigin = mesh.Cell3DVertex(cell3DIndex, alignedEdgeOrigin);
            const unsigned int cell0DEnd = mesh.Cell3DVertex(cell3DIndex, alignedEdgeEnd);

            const auto alignedEdge = alignedCell1DsByOrigin[cell0DOrigin].find(cell0DEnd);
            if (alignedEdge == alignedCell1DsByOrigin[cell0DOrigin].end())
            {
                ...
                {
                    const unsigned int direction_vertex_index = direction ? ae_v : (alignedEdgeNumVertices - 1) - ae_v;
                    const unsigned int direction_edge_index = direction ? ae_v : (alignedEdgeNumVertices - 2) - ae_v;

                    alignedCell1D_Cell0Ds[ae_v] =
                        mesh.Cell3DVertex(cell3DIndex, cell3DAlignedEdgeVertices.at(direction_vertex_index));
                    alignedCell1D_Cell1Ds[ae_v] = mesh.Cell3DEdge(cell3DIndex, cell3DAlignedEdgeEdges.at(direction_edge_index));
                ...
                alignedCell1DsByOrigin[cell0DOrigin].insert(std::pair<unsigned int, AlignedCell1D>(
                    cell0DEnd,
                    {alignedCell1DIndex++, alignedCell1D_Cell0Ds, alignedCell1D_Cell1Ds, {cell3DIndex}}));
            }
            else
            {
    ...
    for (unsigned int cell0DIndex = 0; cell0DIndex < mesh.Cell0DTotalNumber(); cell0DIndex++)
    {
        for (const auto &alignedEdge : alignedCell1DsByOrigin[cell0DIndex])
        {
            ...
            result.AlignedCell1Ds.col(alignedEdge.second.Index) << alignedEdge.second.Cell0Ds.at(0),
                alignedEdge.second.Cell0Ds.at(alignedEdge.second.Cell0Ds.size() - 1);
            result.AlignedCell1Ds_Cell3Ds[alignedEdge.second.Index] =
                std::vector<unsigned int>(alignedEdge.second.Cell3Ds.begin(), alignedEdge.second.Cell3Ds.end());
        }
    }
                                             ...
                                             const MeshGeometricData3D &geometricData) const
{
    ...
    if (configuration.Cell1D_CheckNormals)
    {
        for (unsigned int cell3DIndex = 0; cell3DIndex < mesh.Cell3DTotalNumber(); cell3DIndex++)
        {
            ...
            {
                const double area =
                    geometryUtilities.PolygonAreaByBoundaryIntegral(geometricData.Cell3DsFaces2DVertices[cell3DIndex][f],
    ...
    if (configuration.Cell2D_CheckTriangles)
    {
        for (unsigned int cell3DIndex = 0; cell3DIndex < mesh.Cell3DTotalNumber(); cell3DIndex++)
        {
            ...
            for (unsigned int f = 0; f < mesh.Cell3DNumberFaces(cell3DIndex); f++)
            {
                const double area = geometryUtilities.PolygonAreaByInternalIntegral(
                    geometricData.Cell3DsFaces2DTriangulations[cell3DIndex][f]);
    ...
    if (configuration.Cell2D_CheckNormals)
    {
        for (unsigned int cell3DIndex = 0; cell3DIndex < mesh.Cell3DTotalNumber(); cell3DIndex++)
        {
            ...
            const double volume = geometryUtilities.PolyhedronVolumeByBoundaryIntegral(
                geometricData.Cell3DsFaces2DTriangulations[cell3DIndex],
            ...
            if (!geometryUtilities.AreValuesEqual(geometricData.Cell3DsVolumes[cell3DIndex], volume, geometryUtilities.Tolerance3D()))
            {
                std::cout.precision(16);
    ...
    if (configuration.Cell3D_CheckTetrahedra)
    {
        for (unsigned int cell3DIndex = 0; cell3DIndex < mesh.Cell3DTotalNumber(); cell3DIndex++)
        {
            ...
            const double volume =
                geometryUtilities.PolyhedronVolumeByInternalIntegral(geometricData.Cell3DsTetrahedronPoints[cell3DIndex]);
                                         ...
                                         Gedim::IMeshDAO &mesh) const
{
    ...
    {
        mesh.Cell1DInsertExtremes(e, polyhedronEdges(0, e), polyhedronEdges(1, e));
        mesh.Cell1DSetState(e, true);
                                          ...
                                          Gedim::IMeshDAO &mesh) const
{
    ...
    {
        if (geometryUtilities.IsPointOnPlane(mesh.Cell0DCoordinates(v), planeNormal, planeOrigin))
        {
    ...
    for (unsigned int s = 0; s < mesh.Cell1DTotalNumber(); s++)
    {
        ...
        if (vertices_on_plane[extremes[0]] && vertices_on_plane[extremes[1]])
        {
                                               ...
                                               Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int f = 0; f < mesh.Cell2DTotalNumber(); ++f)
    {
        ...
        {
            mesh.Cell0DSetMarker(mesh.Cell2DVertex(f, v), marker);
        }

        for (unsigned int e = 0; e < mesh.Cell2DNumberEdges(f); ++e)
        {
            mesh.Cell1DSetMarker(mesh.Cell2DEdge(f, e), marker);
        }

        mesh.Cell2DSetMarker(f, marker);
    }
}
// ***************************************************************************
                                            ...
                                            Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int v = 0; v < mesh.Cell0DTotalNumber(); v++)
    {
        ...
        if (!geometryUtilities.IsPointOnPlane(vertex, polygon_plane_normal, polygon_plane_origin))
            continue;

        const Eigen::Vector3d vertex_2D =
            geometryUtilities.RotatePointsFrom3DTo2D(vertex, polygon_rotation_matrix.transpose(), polygon_translation);

        if (!geometryUtilities.IsPointInsidePolygon(vertex_2D, polygon_vertices_2D))
            continue;
    ...
    for (unsigned int s = 0; s < mesh.Cell1DTotalNumber(); s++)
    {
        ...
        if (vertices_on_polygon[extremes[0]] && vertices_on_polygon[extremes[1]])
        {
            mesh.Cell1DSetMarker(s, marker);
        }
    }
                                            ...
                                            Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int v = 0; v < mesh.Cell0DTotalNumber(); v++)
    {
        ...
        if (!geometryUtilities.IsPointInsidePolygon_RayCasting(vertex_2D, polygon_vertices_2D))
            continue;
    ...
    for (unsigned int s = 0; s < mesh.Cell1DTotalNumber(); s++)
    {
        ...
        const Eigen::Vector3d cell1D_centroid_2D =
            geometryUtilities.RotatePointsFrom3DTo2D(cell1D_centroid, polygon_rotation_matrix.transpose(), polygon_translation);

        if (!geometryUtilities.IsPointInsidePolygon_RayCasting(cell1D_centroid_2D, polygon_vertices_2D))
            continue;
    ...
    for (unsigned int p = 0; p < mesh.Cell2DTotalNumber(); p++)
    {
        ...
        const Eigen::Vector3d cell2D_centroid_2D =
            geometryUtilities.RotatePointsFrom3DTo2D(cell2D_centroid, polygon_rotation_matrix.transpose(), polygon_translation);

        if (!geometryUtilities.IsPointInsidePolygon_RayCasting(cell2D_centroid_2D, polygon_vertices_2D))
            continue;
                                                                          ...
                                                                          const Gedim::IMeshDAO &convexMesh) const
{
    ...
    for (unsigned int c = 0; c < convexMesh.Cell3DTotalNumber(); c++)
    {
        ...
        const GeometryUtilities::Polyhedron polyhedron = MeshCell3DToPolyhedron(convexMesh, c);
        ...
        result.Cell3DsEdgesCentroid[c] =
            geometryUtilities.PolyhedronEdgesCentroid(result.Cell3DsVertices[c], result.Cell3DsEdges[c]);
        result.Cell3DsEdgeLengths[c] =
            geometryUtilities.PolyhedronEdgesLength(result.Cell3DsVertices[c], result.Cell3DsEdges[c]);

        result.Cell3DsEdgeTangents[c] =
            geometryUtilities.PolyhedronEdgeTangents(result.Cell3DsVertices[c], result.Cell3DsEdges[c]);
        ...
        for (unsigned int e = 0; e < convexMesh.Cell3DNumberEdges(c); e++)
        {
            ...
            result.Cell3DsEdgeDirections[c][e] = (convexMesh.Cell3DFindEdgeByExtremes(c, meshOrigin, meshEnd) == e);
        }

        result.Cell3DsFaces3DVertices[c] =
            geometryUtilities.PolyhedronFaceVertices(result.Cell3DsVertices[c], result.Cell3DsFaces[c]);
        ...
        result.Cell3DsFacesRotationMatrices[c] =
            geometryUtilities.PolyhedronFaceRotationMatrices(result.Cell3DsFaces3DVertices[c],
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            for (unsigned int e = 0; e < numFaceEdges; e++)
            {
                ...
                result.Cell3DsFacesEdgeDirections[c][f][e] =
                    (convexMesh.Cell2DFindEdgeByExtremes(cell2DIndex, meshOrigin, meshEnd) == e);
            }
        }
        ...
        result.Cell3DsFaces2DVertices[c] =
            geometryUtilities.PolyhedronFaceRotatedVertices(result.Cell3DsFaces3DVertices[c],
                                                            result.Cell3DsFacesTranslations[c],
                                                            result.Cell3DsFacesRotationMatrices[c]);
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            {
                const auto cell2DIndex = convexMesh.Cell3DFace(c, f);
                ...
                for (const auto faceConvexHullIndex : faceConvexHull)
                    std::cout << " " << convexMesh.Cell2DVertex(cell2DIndex, faceConvexHullIndex);
                std::cout << std::endl;
            }
        ...
        result.Cell3DsFaces3DTriangulations[c] =
            geometryUtilities.PolyhedronFaceExtractTriangulationPoints(result.Cell3DsFaces3DVertices[c], polyhedronFaceTriangulations);
        result.Cell3DsFaces2DTriangulations[c] =
            geometryUtilities.PolyhedronFaceExtractTriangulationPoints(result.Cell3DsFaces2DVertices[c], polyhedronFaceTriangulations);
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            result.Cell3DsFacesEdges3DCentroid[c][f] =
                geometryUtilities.PolygonEdgesCentroid(result.Cell3DsFaces3DVertices[c][f]);
            result.Cell3DsFacesEdges2DCentroid[c][f] =
                geometryUtilities.PolygonEdgesCentroid(result.Cell3DsFaces2DVertices[c][f]);
            result.Cell3DsFacesEdge3DTangents[c][f] =
                geometryUtilities.PolygonEdgeTangents(result.Cell3DsFaces3DVertices[c][f]);
            result.Cell3DsFacesEdge2DTangents[c][f] =
                geometryUtilities.PolygonEdgeTangents(result.Cell3DsFaces2DVertices[c][f]);
        ...
        result.Cell3DsFacesNormalDirections[c] =
            geometryUtilities.PolyhedronFaceNormalDirections(result.Cell3DsFaces3DVertices[c],
        ...
        for (unsigned int f = 0; f < numFaces; ++f)
        {
            ...
            result.Cell3DsFacesNormalGlobalDirection[c][f] =
                convexMesh.Cell2DNeighbourCell3D(cell2D_index, first_cell3D_neigh_position) == c;

            result.Cell3DsFacesTangentsGlobalDirection[c][f][0] = result.Cell3DsFacesEdgeDirections[c][f][0];
            result.Cell3DsFacesTangentsGlobalDirection[c][f][1] =
                result.Cell3DsFacesTangentsGlobalDirection[c][f][0] == result.Cell3DsFacesNormalGlobalDirection[c][f];
        }

        result.Cell3DsVolumes[c] =
            geometryUtilities.PolyhedronVolumeByBoundaryIntegral(result.Cell3DsFaces2DTriangulations[c],
                                                                 result.Cell3DsFacesNormals[c],
                                                                 result.Cell3DsFacesNormalDirections[c],
                                                                 result.Cell3DsFacesTranslations[c],
                                                                 result.Cell3DsFacesRotationMatrices[c]);
        ...
        result.Cell3DsTetrahedronPoints[c] =
            geometryUtilities.ExtractTetrahedronPoints(result.Cell3DsVertices[c], result.Cell3DsCentroids[c], polyhedronTetrahedrons);
    }
                                                                          ...
                                                                          const std::vector<std::vector<unsigned int>> &meshCell3DToConvexCell3DIndices) const
{
    ...
    for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); c++)
    {
        ...
        for (unsigned int e = 0; e < mesh.Cell3DNumberEdges(c); e++)
        {
            ...
            result.Cell3DsEdgeDirections[c][e] = (mesh.Cell3DFindEdgeByExtremes(c, meshOrigin, meshEnd) == e);
        }
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            for (unsigned int e = 0; e < numFaceEdges; e++)
            {
                ...
                result.Cell3DsFacesEdgeDirections[c][f][e] =
                    (mesh.Cell2DFindEdgeByExtremes(cell2DIndex, meshOrigin, meshEnd) == e);
            }
        }
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            case Gedim::GeometryUtilities::PolygonOrientations::Clockwise: {
                ...
                face2DVerticesCCW[f] = geometryUtilities.ExtractPoints(result.Cell3DsFaces2DVertices[c][f], face2DsCCW[f]);
            }
            break;
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            for (unsigned int cct = 0; cct < numConcaveCell2DTriangulation; cct++)
            {
                ...
                concaveCell2DTriangulationCentroids.col(cct) =
                    geometryUtilities.PolygonBarycenter(concaveCell2DTriangulationPoints[cct]);
            }
            ...
            result.Cell3DsFacesEdge2DNormals[c][f] =
                facesNormalOrientation * geometryUtilities.PolygonEdgeNormals(result.Cell3DsFaces2DVertices[c][f]);
        }
        ...
        for (unsigned int cc = 0; cc < numConvexCell3Ds; cc++)
        {
            ...
            const GeometryUtilities::Polyhedron convexCell3DPolyhedron = MeshCell3DToPolyhedron(convexMesh, convexCell3DIndex);

            convexCell3DsFaces3DVertices[cc] =
                geometryUtilities.PolyhedronFaceVertices(convexCell3DPolyhedron.Vertices, convexCell3DPolyhedron.Faces);
            const std::vector<Eigen::Vector3d> convexCell3DFacesTranslation =
                geometryUtilities.PolyhedronFaceTranslations(convexCell3DsFaces3DVertices[cc]);
            convexCell3DsNormal[cc] = geometryUtilities.PolyhedronFaceNormals(convexCell3DsFaces3DVertices[cc]);
            const std::vector<Eigen::Matrix3d> convexCell3DFacesRotationMatrices =
                geometryUtilities.PolyhedronFaceRotationMatrices(convexCell3DsFaces3DVertices[cc],
                                                                 convexCell3DsNormal[cc],
                                                                 convexCell3DFacesTranslation);

            const std::vector<Eigen::MatrixXd> convexCell3DFaces2DVertices =
                geometryUtilities.PolyhedronFaceRotatedVertices(convexCell3DsFaces3DVertices[cc],
                                                                convexCell3DFacesTranslation,
            ...
            for (unsigned int ccf = 0; ccf < convexCell3DFaces2DVertices.size(); ccf++)
            {
                ...
                if (geometryUtilities.PolygonOrientation(faceConvexHull) == Gedim::GeometryUtilities::PolygonOrientations::Clockwise)
                {
                    const auto cell2DIndex = convexMesh.Cell3DFace(convexCell3DIndex, ccf);
            ...
            const std::vector<std::vector<unsigned int>> convexCell3DFacesTriangulations =
                geometryUtilities.PolyhedronFaceTriangulationsByFirstVertex(convexCell3DPolyhedron.Faces,
                                                                            convexCell3DsFaces3DVertices[cc]);
            const std::vector<std::vector<Eigen::Matrix3d>> convexCell3DFaces2DTriangulations =
                geometryUtilities.PolyhedronFaceExtractTriangulationPoints(convexCell3DFaces2DVertices, convexCell3DFacesTriangulations);

            convexCell3DsNormalDirections[cc] = geometryUtilities.PolyhedronFaceNormalDirections(
                convexCell3DsFaces3DVertices[cc],
        ...
        for (unsigned int cc = 0; cc < numConvexCell3Ds; cc++)
        {
            for (unsigned int cct = 0; cct < convexCell3DTetrahedronsPoints[cc].size(); cct++)
                result.Cell3DsTetrahedronPoints[c][tetraCounter++] = convexCell3DTetrahedronsPoints[cc][cct];
        }

        const FindConcaveCell3DFacesConvexCell2DResult convexCell2Ds =
            FindConcaveCell3DFacesConvexCell2D(geometryUtilities,
                                               c,
                                               mesh,
                                               convexMesh,
                                               convexCell3DIndices,
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            const Eigen::Vector3d outgoingFaceNormal =
                convexCell3DsNormalDirections[cc][ccf] ? convexCell3DsNormal[cc][ccf] : -1.0 * convexCell3DsNormal[cc][ccf];

            result.Cell3DsFacesNormalDirections[c][f] =
                geometryUtilities.IsValuePositive(faceNormal.dot(outgoingFaceNormal), geometryUtilities.Tolerance1DSquared());
        }
        ...
        for (unsigned int f = 0; f < numFaces; ++f)
        {
            ...
            result.Cell3DsFacesNormalGlobalDirection[c][f] =
                mesh.Cell2DNeighbourCell3D(cell2D_index, first_cell3D_neigh_position) == c;
    ...
    const std::vector<std::vector<Eigen::Matrix3d>> &cell2Ds_triangles_3D_vertices) const
{
    ...
    for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); c++)
    {
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            case Gedim::GeometryUtilities::PolygonOrientations::Clockwise: {
                face2DCCWOrientation[f] = false;
                const auto new_orientation_points =
                    geometryUtilities.ChangePolygonOrientation(result.Cell3DsFaces2DVertices[c][f].cols());
                face2DVerticesCCW[f] = geometryUtilities.ExtractPoints(result.Cell3DsFaces2DVertices[c][f], new_orientation_points);
            }
            break;
            case Gedim::GeometryUtilities::PolygonOrientations::CounterClockwise: {
                face2DCCWOrientation[f] = true;
...

        {
            ...
            for (unsigned int f = 0; f < numFaces; ++f)
            {
                ...
                for (unsigned int t = 0; t < num_triangles; ++t)
                {
                    ...
                    {
                    case Gedim::GeometryUtilities::PolygonOrientations::Clockwise: {
                        const auto new_orientation =
                            geometryUtilities.ChangePolygonOrientation(result.Cell3DsFaces2DTriangulations[c][f][t].cols());
                        result.Cell3DsFaces2DTriangulations[c][f][t] =
                            geometryUtilities.ExtractPoints(result.Cell3DsFaces2DTriangulations[c][f][t], new_orientation);
                    }
                    break;
                    case Gedim::GeometryUtilities::PolygonOrientations::CounterClockwise:
                        break;
        ...
        for (unsigned int t = 0; t < num_cell3D_tetra; ++t)
        {
            ...
            cell3D_tetra_faces_3D_vertices[t] = geometryUtilities.PolyhedronFaceVertices(tetra.Vertices, tetra.Faces);
            const std::vector<Eigen::Vector3d> cell3D_tetra_faces_translation =
                geometryUtilities.PolyhedronFaceTranslations(cell3D_tetra_faces_3D_vertices[t]);
            cell3D_tetra_normal[t] = geometryUtilities.PolyhedronFaceNormals(cell3D_tetra_faces_3D_vertices[t]);
            const std::vector<Eigen::Matrix3d> cell3D_tetra_faces_rotation =
                geometryUtilities.PolyhedronFaceRotationMatrices(cell3D_tetra_faces_3D_vertices[t],
                                                                 cell3D_tetra_normal[t],
                                                                 cell3D_tetra_faces_translation);

            const std::vector<Eigen::MatrixXd> cell3D_tetra_faces_2D_vertices =
                geometryUtilities.PolyhedronFaceRotatedVertices(cell3D_tetra_faces_3D_vertices[t],
                                                                cell3D_tetra_faces_translation,
            ...
            cell3D_tetra_faces_unaligned_vertices[t] =
                geometryUtilities.PolyhedronFacesUnalignedVertices(cell3D_tetra_faces_2D_vertices);

            const std::vector<std::vector<unsigned int>> cell3D_tetra_faces_triangulations =
                geometryUtilities.PolyhedronFaceTriangulationsByFirstVertex(tetra.Faces, cell3D_tetra_faces_3D_vertices[t]);
            const std::vector<std::vector<Eigen::Matrix3d>> cell3D_tetra_faces_2D_triangulations =
                geometryUtilities.PolyhedronFaceExtractTriangulationPoints(cell3D_tetra_faces_2D_vertices,
                                                                           cell3D_tetra_faces_triangulations);

            cell3D_tetra_normal_directions[t] =
                geometryUtilities.PolyhedronFaceNormalDirections(cell3D_tetra_faces_3D_vertices[t],
        ...
        for (unsigned int f = 0; f < numFaces; f++)
        {
            ...
            const Eigen::Vector3d outgoingFaceNormal =
                cell3D_tetra_normal_directions[cc][ccf] ? cell3D_tetra_normal[cc][ccf] : -1.0 * cell3D_tetra_normal[cc][ccf];
    ...
    for (unsigned int f = 0; f < numConcaveFaces; f++)
    {
        ...
        for (unsigned int cc = 0; cc < numConvexCell3Ds; cc++)
        {
            ...
            for (unsigned int ccf = 0; ccf < numConvexFaces; ccf++)
            {
                ...
                Eigen::MatrixXd convexFace2DTriangle =
                    geometryUtilities.RotatePointsFrom3DTo2D(convexFaceTriangle, faceRotationMatrix.transpose(), faceTranslation);

                if (geometryUtilities.IsValueNegative(faceRotationMatrix.determinant(), geometryUtilities.Tolerance1D()))
                ...
                if (!geometryUtilities.IsPointInsidePolygon_RayCasting(convexFace2DTriangle.col(0), face2DVertices))
                    continue;
                if (!geometryUtilities.IsPointInsidePolygon_RayCasting(convexFace2DTriangle.col(1), face2DVertices))
                    continue;
                if (!geometryUtilities.IsPointInsidePolygon_RayCasting(convexFace2DTriangle.col(2), face2DVertices))
                    continue;

                // check if convex cell centroid is inside concave cell
                if (!geometryUtilities.IsPointInsidePolygon_RayCasting(geometryUtilities.PolygonBarycenter(convexFace2DTriangle), face2DVertices))
                    continue;
        ...
        if (convexCellFound < 0 || convexFaceFound < 0)
        {
            ...
            std::cerr << "convex Cell2D "
                      << ((convexCellFound >= 0 && convexFaceFound >= 0)
    ...
    const std::vector<std::vector<std::vector<unsigned int>>> &convexCell3DsFacesUnalignedVertices) const
{
    ...
    for (unsigned int f = 0; f < numConcaveFaces; f++)
    {
        ...
        if (geometryUtilities.IsValueNegative(det_2D(face_2D_triangle.col(0), face_2D_triangle.col(1), face_2D_triangle.col(2)),
                                              geometryUtilities.Tolerance1D()))
        {
            throw std::runtime_error("Face triangle not unclockwise " + std::to_string(concaveCell3DIndex) + " " +
                                     std::to_string(f));
        ...
        for (unsigned int cc = 0; cc < numConvexCell3Ds; cc++)
        {
            for (unsigned int ccf = 0; ccf < 4; ++ccf)
            {
                ...
                if (geometryUtilities.IsValueNegative(
                        det_2D(convexFace2DTriangle.col(0), convexFace2DTriangle.col(1), convexFace2DTriangle.col(2)),
                        geometryUtilities.Tolerance1D()))
                {
                ...
                if (!geometryUtilities.CheckTrianglesIntersection(face_2D_triangle, convexFace2DTriangle, false))
                    continue;
                                                                                ...
                                                                                const Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int p = 0; p < find_point_cell3D_result.Cell3Ds_found.size(); p++)
    {
        ...
        case GeometryUtilities::PointPolyhedronPositionResult::Types::Inside:
            result.MeshPositions[p] = {FindPointMeshPositionResult::PointMeshPosition::Types::Cell3D, cell3D_found.Cell3D_index};
            break;
        case GeometryUtilities::PointPolyhedronPositionResult::Types::BorderFace:
            result.MeshPositions[p] = {FindPointMeshPositionResult::PointMeshPosition::Types::Cell2D,
                                       mesh.Cell3DFace(cell3D_found.Cell3D_index, cell3D_found.Cell3D_Position.BorderIndex)};
            break;
        case GeometryUtilities::PointPolyhedronPositionResult::Types::BorderEdge:
            result.MeshPositions[p] = {FindPointMeshPositionResult::PointMeshPosition::Types::Cell1D,
                                       mesh.Cell3DEdge(cell3D_found.Cell3D_index, cell3D_found.Cell3D_Position.BorderIndex)};
            break;
        case GeometryUtilities::PointPolyhedronPositionResult::Types::BorderVertex:
            result.MeshPositions[p] = {FindPointMeshPositionResult::PointMeshPosition::Types::Cell0D,
                                       mesh.Cell3DVertex(cell3D_found.Cell3D_index, cell3D_found.Cell3D_Position.BorderIndex)};
            break;
        default:
    ...
    const unsigned int starting_cell3D_index) const
{
    ...
    for (unsigned int c = 0; c < mesh.Cell3DTotalNumber(); ++c)
    {
        ...
        if (!geometryUtilities.IsPointInBoundingBox(point, cell3DsBoundingBox.at(c3D_index)))
            continue;

        const GeometryUtilities::PointPolyhedronPositionResult pointPosition =
            geometryUtilities.PointPolyhedronPosition(point,
        ...
        if (find_only_first_cell3D && cell3Ds_found.size() > 0)
            break;
    }

    return {std::vector<FindPointCell3DResult::PointCell3DFound>(cell3Ds_found.begin(), cell3Ds_found.end())};
}
// ***************************************************************************
MeshUtilities::FindPointCell3DResult MeshUtilities::FindPointCell3D(
    const GeometryUtilities &geometryUtilities,
...
void MeshUtilities::ComputeCell1DCell3DNeighbours(IMeshDAO &mesh) const
{
    ...
    for (unsigned int c1D = 0; c1D < mesh.Cell1DTotalNumber(); c1D++)
    {
        ...
        for (const auto &cell3DIndex : cell1DsNeighbours[c1D])
            mesh.Cell1DInsertNeighbourCell3D(c1D, n++, cell3DIndex);
    }
}
// ***************************************************************************
void MeshUtilities::ComputeCell2DCell3DNeighbours(IMeshDAO &mesh) const
{
    ...
    for (unsigned int c2D = 0; c2D < mesh.Cell2DTotalNumber(); c2D++)
    {
        ...
        for (const auto &cell3DIndex : cell2DsNeighbours[c2D])
            mesh.Cell2DInsertNeighbourCell3D(c2D, n++, cell3DIndex);
    }
}
// ***************************************************************************
GeometryUtilities::Polyhedron MeshUtilities::MeshCell3DToPolyhedron(const Gedim::IMeshDAO &mesh, const unsigned int &cell3DIndex) const
{
    ...
    for (unsigned int f = 0; f < polyhedron.Faces.size(); f++)
    {
        ...
        {
            polyhedron.Faces[f](0, v) = cell0DIndexToVertexIndex.at(mesh.Cell2DVertex(cell2DIndex, v));
            polyhedron.Faces[f](1, v) = cell1DIndexToEdgeIndex.at(mesh.Cell2DEdge(cell2DIndex, v));
        }
    }
...
MeshUtilities::VTPPolyhedron MeshUtilities::MeshCell3DToVTPPolyhedron(const Gedim::IMeshDAO &mesh, const unsigned int &cell3DIndex) const
{
    ...
    for (unsigned int f = 0; f < vtpPolyhedron.PolyhedronFaces.size(); f++)
    {
        ...
        for (unsigned int v = 0; v < numFaceVertices; v++)
        {
            vtpPolyhedron.PolyhedronFaces[f][v] = cell0DIndexToVertexIndex.at(mesh.Cell2DVertex(cell2DIndex, v));
        }
    }
                                                     ...
                                                     Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int c = 0; c < numSubCells; c++)
    {
        ...
        mesh.Cell3DAddVertices(newCell3DIndex, subCell3DsVertices[c]);
        mesh.Cell3DAddEdges(newCell3DIndex, subCell3DsEdges[c]);
        mesh.Cell3DAddFaces(newCell3DIndex, subCell3DsFaces[c]);

        mesh.Cell3DSetMarker(newCell3DIndex, mesh.Cell3DMarker(cell3DIndex));
        ...
        for (unsigned int e = 0; e < mesh.Cell3DNumberEdges(newCell3DIndex); e++)
        {
            ...
            for (unsigned int n = 0; n < mesh.Cell1DNumberNeighbourCell3D(cell1DIndex); n++)
            {
                ...
                if (mesh.Cell1DNeighbourCell3D(cell1DIndex, n) == cell3DIndex)
                    mesh.Cell1DInsertNeighbourCell3D(cell1DIndex, n, newCell3DIndex);
            }
        }
        ...
        for (unsigned int f = 0; f < mesh.Cell3DNumberFaces(newCell3DIndex); f++)
        {
            ...
            for (unsigned int n = 0; n < mesh.Cell2DNumberNeighbourCell3D(cell2DIndex); n++)
            {
                ...
                if (mesh.Cell2DNeighbourCell3D(cell2DIndex, n) == cell3DIndex)
                    mesh.Cell2DInsertNeighbourCell3D(cell2DIndex, n, newCell3DIndex);
            }
        }
    }
                                                                              ...
                                                                              const Gedim::IMeshDAO &mesh) const
{
    ...
    for (const unsigned int c3D_index : cell3DsIndex)
    {
        ...
        for (unsigned int f = 0; f < mesh.Cell3DNumberFaces(c3D_index); f++)
        {
            ...
            for (unsigned int n = 0; n < mesh.Cell2DNumberNeighbourCell3D(c2D_index); n++)
            {
                ...
                const unsigned int c3D_neigh_index = mesh.Cell2DNeighbourCell3D(c2D_index, n);
    ...
    result.AgglomerateCell3DVertices = std::vector<unsigned int>(agglomerated_cell0Ds.begin(), agglomerated_cell0Ds.end());
    result.AgglomerateCell3DEdges = std::vector<unsigned int>(agglomerated_cell1Ds.begin(), agglomerated_cell1Ds.end());
    result.AgglomerateCell3DFaces = std::vector<unsigned int>(agglomerated_cell2Ds.begin(), agglomerated_cell2Ds.end());
    result.SubCell3DsRemovedVertices = std::vector<unsigned int>(removed_cell0Ds.begin(), removed_cell0Ds.end());
    result.SubCell3DsRemovedEdges = std::vector<unsigned int>(removed_cell1Ds.begin(), removed_cell1Ds.end());
    result.SubCell3DsRemovedFaces = std::vector<unsigned int>(removed_cell2Ds.begin(), removed_cell2Ds.end());
                                               ...
                                               std::vector<std::vector<unsigned int>> &meshCell3DsOriginalCell3Ds) const
{
    ...
    meshCell3DsOriginalCell3Ds.at(agglomeratedCell3DIndex) =
        std::vector<unsigned int>(agglomeratedCell3DConvexCells.begin(), agglomeratedCell3DConvexCells.end());

    mesh.Cell3DAddVertices(agglomeratedCell3DIndex, agglomerateCell3DVertices);
    mesh.Cell3DAddEdges(agglomeratedCell3DIndex, agglomerateCell3DEdges);
    mesh.Cell3DAddFaces(agglomeratedCell3DIndex, agglomerateCell3DFaces);

    mesh.Cell3DSetMarker(agglomeratedCell3DIndex, max_marker);
    mesh.Cell3DSetState(agglomeratedCell3DIndex, true);
    ...
    for (unsigned int v = 0; v < mesh.Cell3DNumberVertices(agglomeratedCell3DIndex); v++)
    {
        ...
        mesh.Cell0DInitializeNeighbourCell3Ds(cell0DIndex, std::vector<unsigned int>(neighCell3Ds.begin(), neighCell3Ds.end()));
    }
    ...
    for (unsigned int e = 0; e < mesh.Cell3DNumberEdges(agglomeratedCell3DIndex); e++)
    {
        ...
        neighCell3Ds.push_back(agglomeratedCell3DIndex);
        mesh.Cell1DInitializeNeighbourCell3Ds(cell1DIndex, std::vector<unsigned int>(neighCell3Ds.begin(), neighCell3Ds.end()));
    }
    ...
    for (unsigned int f = 0; f < mesh.Cell3DNumberFaces(agglomeratedCell3DIndex); f++)
    {
        ...
        for (unsigned int n = 0; n < mesh.Cell2DNumberNeighbourCell3D(cell2DIndex); n++)
        {
            ...
            if (subCell3DsIndex.find(neighCell3DIndex) != subCell3DsIndex.end())
            {
                mesh.Cell2DInsertNeighbourCell3D(cell2DIndex, n, agglomeratedCell3DIndex);
            }
        }
    }
                                                          ...
                                                          const Gedim::IMeshDAO &mesh) const
{
    ...
    result.Cell3Ds = std::vector<unsigned int>(cell3Ds.begin(), cell3Ds.end());
                                                            ...
                                                            Gedim::IMeshDAO &mesh) const
{
    ...
    FillMesh3D(newCell0Ds, newCell1Ds, newCell2Ds, newCell3Ds, mesh);
                                            ...
                                            Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int f = 0; f < num_faces; f++)
    {
        ...
        for (unsigned int t = 0; t < numTriangles; t++)
        {
            ...
            mesh.Cell2DSetMarker(new_cell2D_index, mesh.Cell2DMarker(f));
            mesh.Cell2DSetState(new_cell2D_index, true);

            Eigen::MatrixXi vertices_edges(2, 3);
            vertices_edges.row(0) << face_cell0Ds_idex.at(face_triangles.at(3 * t)),
                face_cell0Ds_idex.at(face_triangles.at(3 * t + 1)), face_cell0Ds_idex.at(face_triangles.at(3 * t + 2));

            const unsigned int new_cell2D_first_edge = (t == 0) ? face_cell1Ds_idex.at(0) : new_cell1D_starting + (t - 1);
            const unsigned int new_cell2D_third_edge =
                (t == numTriangles - 1) ? face_cell1Ds_idex.at(num_face_vertices - 1) : new_cell1D_starting + t;
            vertices_edges.row(1) << new_cell2D_first_edge, face_cell1Ds_idex.at(t + 1), new_cell2D_third_edge;
    ...
    for (unsigned int c = 0; c < num_cell3Ds; c++)
    {
        ...
        mesh.Cell3DSetMarker(new_cell3D_index, mesh.Cell3DMarker(c));
        mesh.Cell3DSetState(new_cell3D_index, true);

        mesh.Cell3DInitializeVertices(new_cell3D_index, vertices.size());
        mesh.Cell3DInitializeEdges(new_cell3D_index, edges.size() + cell3D_new_edges.size());
        mesh.Cell3DInitializeFaces(new_cell3D_index, cell3D_new_faces.size());

        for (unsigned int v = 0; v < vertices.size(); v++)
            mesh.Cell3DInsertVertex(new_cell3D_index, v, vertices.at(v));

        for (unsigned int e = 0; e < edges.size(); e++)
            mesh.Cell3DInsertEdge(new_cell3D_index, e, edges.at(e));

        unsigned int e = 0;
        for (const auto &cell1D_index : cell3D_new_edges)
            mesh.Cell3DInsertEdge(new_cell3D_index, edges.size() + e++, cell1D_index);

        unsigned int f = 0;
        for (const unsigned int &cell2D_index : cell3D_new_faces)
            mesh.Cell3DInsertFace(new_cell3D_index, f++, cell2D_index);
                                                ...
                                                Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int e = 0; e < num_polyhedron_edges; ++e)
    {
        ...
        SetMeshMarkersOnSegment(geometryUtilities, edge_line_origin, edge_line_tangent, edge_length * edge_length, edge_marker, mesh);
    }
    ...
    for (unsigned int c_0D = 0; c_0D < mesh.Cell0DTotalNumber(); ++c_0D)
    {
        ...
        const auto vertex_found = geometryUtilities.FindPointInPoints(polyhedron_vertices, cell0D_coordinate);

        for (const auto &vertex_index : vertex_found)
        {
            mesh.Cell0DSetMarker(c_0D, polyhedron_vertices_marker.at(vertex_index));
        }
    }
}
// ***************************************************************************
