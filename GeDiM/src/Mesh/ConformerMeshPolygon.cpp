...
ConformerMeshPolygon::ConformerMeshPolygon(const GeometryUtilities &geometryUtilities,
    ...
    : _geometryUtilities(geometryUtilities), configuration(configuration)
{
}
                                                                                                 ...
                                                                                                 bool &found)
{
    ...
    for (std::map<double, ConformerMeshSegment::ConformMesh::ConformMeshPoint>::const_iterator it = result.Points.begin();
    ...
    result.Points.insert(pair<double, ConformerMeshSegment::ConformMesh::ConformMeshPoint>(
                                                                 ...
                                                                 const Gedim::IMeshDAO &mesh2DReader)
{
    ...
    if (origin.Vertex2DIds.size() == 0 && origin.Edge2DIds.size() == 0)
    {
        ...
        const unsigned int cell2DMesh2DId = *origin.Cell2DIds.rbegin(); // max element
        ...
        for (unsigned int e = 0; e < mesh2DReader.Cell2DNumberEdges(cell2DMesh2DId); e++)
        {
            ...
            GeometryUtilities::IntersectionSegmentSegmentResult result =
                _geometryUtilities.IntersectionSegmentSegment(segmentOrigin, segmentEnd, edgeOrigin, edgeEnd);
            ...
            bool found = false;
            ConformerMeshSegment::ConformMesh::ConformMeshPoint &newPoint =
                InsertNewIntersection(intersectionCurvilinearCoordinate, mesh1D, found);
            ...
            if (find(newPoint.Cell2DIds.begin(), newPoint.Cell2DIds.end(), cell2DMesh2DId) == newPoint.Cell2DIds.end())
                newPoint.Cell2DIds.insert(cell2DMesh2DId);
            ...
            for (unsigned int en = 0; en < mesh2DReader.Cell1DNumberNeighbourCell2D(edgeId); en++)
            {
                ...
                if (mesh2DReader.Cell1DNeighbourCell2D(edgeId, en) != cell2DMesh2DId)
                    newPoint.Cell2DIds.insert(mesh2DReader.Cell1DNeighbourCell2D(edgeId, en));
            }
    ...
    if (end.Vertex2DIds.size() == 0 && end.Edge2DIds.size() == 0)
    {
        ...
        const unsigned int cell2DMesh2DId = *end.Cell2DIds.rbegin(); // max element
                                                                    ...
                                                                    const std::vector<unsigned int> &cell1DMesh2DUpdated) const
{
    ...
    for (unsigned int e = 0; e < num_new_edges; e++)
    {
        ...
        cell1Ds_origin.insert(std::make_pair(mesh2DReader.Cell1DOrigin(cell1D), e));
        cell1Ds_end.insert(std::make_pair(mesh2DReader.Cell1DEnd(cell1D), e));
    }
    ...
    while (num_found_edges < num_new_edges)
    {
        ...
        if (origin_found != cell1Ds_origin.end() && !is_selected_cell1Ds.at(origin_found->second))
        {
        ...
        if (end_found != cell1Ds_end.end() && !is_selected_cell1Ds.at(end_found->second))
        {
                                                              ...
                                                              std::vector<unsigned int> &cell1DMesh2Ds)
{
    ...
    bool edgeListDirection = (mesh2DReader.Cell1DOrigin(cell1DMesh2DUpdated.front()) == cell0DMesh2Ds[0] ||
                              mesh2DReader.Cell1DEnd(cell1DMesh2DUpdated.front()) == cell0DMesh2Ds[0]); // true if
                                                                                                        // forward,
                                                                                                        // false if
                                                                                                        // backward
    ...
    {
        for (vector<unsigned int>::const_iterator iter = cell1DMesh2DUpdated.begin(); iter != cell1DMesh2DUpdated.end(); iter++)
        {
            cell0DMesh2Ds[counter] = (mesh2DReader.Cell1DOrigin(*iter) == cell0DMesh2Ds[counter - 1])
                                         ? mesh2DReader.Cell1DEnd(*iter)
                                         : mesh2DReader.Cell1DOrigin(*iter);
                                                             ...
                                                             vector<unsigned int> &cell2DMesh2DNewEdges)
{
    ...
    {
        const unsigned int oldVertex = mesh2DReader.Cell2DVertex(cell2DMesh2DId, oldVertexCounter);
        const unsigned int oldNextVertex =
            mesh2DReader.Cell2DVertex(cell2DMesh2DId, (oldVertexCounter + 1) % mesh2DReader.Cell2DNumberVertices(cell2DMesh2DId));
        if (oldVertex == cell0DMesh2DUpdated[0] && oldNextVertex == cell0DMesh2DUpdated[cell0DMesh2DUpdated.size() - 1])
        {
        ...
        else if (oldVertex == cell0DMesh2DUpdated[cell0DMesh2DUpdated.size() - 1] && oldNextVertex == cell0DMesh2DUpdated[0])
        {
    ...
    {
        const unsigned int oldEdge = mesh2DReader.Cell2DEdge(cell2DMesh2DId, oldEdgeCounter);
                                                    ...
                                                    GeometryUtilities::SplitPolygonInput &splitInput)
{
    ...
    // // check aligned edges
    // const vector<unsigned int> unalignedPoints =
    // _geometryUtilities.UnalignedPoints(mesh2D.Cell2DVerticesCoordinates(cell2DMesh2DId)); if (unalignedPoints.size()
    // != splitInput.NumberPolygonVertices)
    // {
                                             ...
                                             const GeometryUtilities::SplitPolygonWithSegmentResult &splitResult)
{
    ...
    GeometryUtilities::CompareTypes segment1DLength =
        _geometryUtilities.CompareValues(originCurvilinearCoordinate, endCurvilinearCoordinate, _geometryUtilities.Tolerance1D());
    ...
    for (const GeometryUtilities::SplitPolygonWithSegmentResult::NewEdge &newEdge : splitResult.NewEdges)
    {
        ...
        const unsigned int cell1DOrigin = cell1DOriginOriginal ? mesh2D.Cell2DVertex(cell2DMesh2DId, newEdge.OriginId)
                                                               : newVertexMap.at(newEdge.OriginId);
        const bool cell1DEndOriginal = (newEdge.EndId < numOriginalVerticesCell2DMesh2D);
        const unsigned int cell1DEnd = cell1DEndOriginal ? mesh2D.Cell2DVertex(cell2DMesh2DId, newEdge.EndId)
        ...
        {
            const unsigned int edgeIdToUpdate = mesh2D.Cell2DEdge(cell2DMesh2DId, newEdge.OldEdgeId);
            const bool newEdgeDirection = cell1DOriginOriginal ? (mesh2D.Cell1DOrigin(edgeIdToUpdate) == cell1DOrigin)
            ...
            if (newEdge.OriginId >= numOriginalVerticesCell2DMesh2D)
                mesh2D.Cell0DSetMarker(newVertexMap.at(newEdge.OriginId), mesh2D.Cell1DMarker(edgeIdToUpdate));
            if (newEdge.EndId >= numOriginalVerticesCell2DMesh2D)
                mesh2D.Cell0DSetMarker(newVertexMap.at(newEdge.EndId), mesh2D.Cell1DMarker(edgeIdToUpdate));

            mesh2D.Cell1DInitializeNeighbourCell2Ds(e, mesh2D.Cell1DNumberNeighbourCell2D(edgeIdToUpdate));
            ...
            for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(edgeIdToUpdate); n++)
            {
                ...
                const unsigned int neighPosition =
                    newEdgeDirection ? n : (n + 1) % mesh2D.Cell1DNumberNeighbourCell2D(edgeIdToUpdate);

                if (mesh2D.Cell1DNeighbourCell2D(edgeIdToUpdate, n) == cell2DMesh2DId)
                    mesh2D.Cell1DInsertNeighbourCell2D(e, neighPosition, c + newEdge.Cell2DNeighbours[0]);
                else
                    mesh2D.Cell1DInsertNeighbourCell2D(e, neighPosition, mesh2D.Cell1DNeighbourCell2D(edgeIdToUpdate, n));
            }
        ...
        {
            mesh2D.Cell1DInitializeNeighbourCell2Ds(e, 2);

            for (unsigned int n = 0; n < 2; n++)
            {
                mesh2D.Cell1DInsertNeighbourCell2D(e, n, c + newEdge.Cell2DNeighbours[n]);
            }
    ...
    for (const GeometryUtilities::SplitPolygonWithSegmentResult::NewPolygon &newPolygon : splitResult.NewPolygons)
    {
        ...
        for (const unsigned int &vId : newPolygon.Vertices)
        {
            ...
            mesh2D.Cell2DInsertVertex(c,
                                      cv,
                                      (vId < numOriginalVerticesCell2DMesh2D) ? mesh2D.Cell2DVertex(cell2DMesh2DId, vId)
                                                                              : newVertexMap.at(vId));
            cv++;
        }
        ...
        for (const unsigned int &eId : newPolygon.Edges)
        {
            if (eId < numOriginalVerticesCell2DMesh2D)
            {
                ...
                mesh2D.Cell2DInsertEdge(c, ce, edgeId);
                ...
                for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(edgeId); n++)
                {
                    ...
                    if (mesh2D.Cell1DNeighbourCell2D(edgeId, n) == cell2DMesh2DId)
                        mesh2D.Cell1DInsertNeighbourCell2D(edgeId, n, c);
                }
            }
            else
            {
                // update mesh2D
                mesh2D.Cell2DInsertEdge(c, ce, newEdgeMap.at(eId));
            }
                                                                ...
                                                                const unsigned int &cell2DMesh2DId)
{
    ...
    for (unsigned int i = 0; i < cell1DMesh1DIds.size(); i++)
    {
        ...
        Output::Assert(originCell0DMesh1D.Vertex2DIds.size() <= 1 && endCell0DMesh1D.Vertex2DIds.size() <= 1);
        ...
        {
            cell1DMesh2DsCell1DMesh1Ds.insert(make_pair(cell1DMesh2DToUpdate, list<unsigned int>()));
        }
        ...
        {
            Output::Assert(originCell0DMesh1D.Edge2DIds.size() == 1 && *originCell0DMesh1D.Edge2DIds.begin() == cell1DMesh2DToUpdate);
         ...
         it++)
    {
        ...
        ConformerMeshSegment::ConformMesh::ConformMeshSegment &firstCell1DMesh1D =
            mesh1D.Segments[cell1DMesh2DCell1DMesh1Ds.front()];
        ConformerMeshSegment::ConformMesh::ConformMeshSegment &lastCell1DMesh1D =
            mesh1D.Segments[cell1DMesh2DCell1DMesh1Ds.back()];
        ...
        vector<unsigned int> cell1DMesh2DOrderedCell1DMesh1Ds =
            edgeListDirection ? vector<unsigned int>(cell1DMesh2DCell1DMesh1Ds.begin(), cell1DMesh2DCell1DMesh1Ds.end())
                              : vector<unsigned int>(cell1DMesh2DCell1DMesh1Ds.rbegin(), cell1DMesh2DCell1DMesh1Ds.rend());
        ...
        cell1DMesh2DsNewCell1Ds.insert(
            make_pair(cell1DMesh2DToUpdate, vector<unsigned int>(cell1DMesh2DOrderedCell1DMesh1Ds.size())));
        ...
        for (unsigned int ci = 0; ci < cell1DMesh2DOrderedCell1DMesh1Ds.size(); ci++)
        {
            ...
            mesh2D.Cell1DInsertExtremes(newCell1DMesh2DId, originCell1D, endCell1D);
            ...
            mesh2D.Cell1DInitializeNeighbourCell2Ds(newCell1DMesh2DId, mesh2D.Cell1DNumberNeighbourCell2D(cell1DMesh2DToUpdate));
            ...
            for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(cell1DMesh2DToUpdate); n++)
            {
                ...
                mesh2D.Cell1DInsertNeighbourCell2D(newCell1DMesh2DId, n, mesh2D.Cell1DNeighbourCell2D(cell1DMesh2DToUpdate, n));
            }
    ...
    for (unsigned int e = 0; e < mesh2D.Cell2DNumberEdges(cell2DMesh2DId); e++)
    {
        ...
        else
        {
            ...
            const vector<unsigned int> newEdgeIdsOrdered =
                edgeVertexOrientation ? newEdgeIds : vector<unsigned int>(newEdgeIds.rbegin(), newEdgeIds.rend());
    ...
    mesh2D.Cell2DAddVertices(newCell2DMesh2DId,
                             vector<unsigned int>(newCell2DMesh2DVertices.begin(), newCell2DMesh2DVertices.end()));
    mesh2D.Cell2DAddEdges(newCell2DMesh2DId, vector<unsigned int>(newCell2DMesh2DEdges.begin(), newCell2DMesh2DEdges.end()));
    ...
    for (unsigned int e = 0; e < mesh2D.Cell2DNumberEdges(newCell2DMesh2DId); e++)
    {
        ...
        for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(edgeId); n++)
        {
            ...
            if (mesh2D.Cell1DNeighbourCell2D(edgeId, n) == cell2DMesh2DId)
                mesh2D.Cell1DInsertNeighbourCell2D(edgeId, n, newCell2DMesh2DId);
        }
    }
                                                                      ...
                                                                      const unsigned int &cell2DMesh2DId)
{
    ...
    mesh2D.Cell2DUpdatedCell2Ds(cell2DMesh2DId, cell2DMesh2DsToUpdate);
    ...
    for (const unsigned int &cell1DMesh1DId : cell1DMesh1DIdsRedirected)
    {
        ...
        if (*segment.Edge2DIds.rbegin() != cell1DMesh2DToUpdate)
            throw runtime_error("Something goes wrong with update of " + to_string(cell1DMesh2DToUpdate) +
                                " cell1DMesh2D in cell1DMesh1D " + to_string(cell1DMesh1DId));
    ...
    for (const unsigned int &cell1DMesh1DId : cell1DMesh1DIdsRedirected)
    {
        ...
        // add new Cell1Ds on Mesh2D
        mesh2D.Cell1DInsertExtremes(e, newCell0DMesh2Ds[counter], newCell0DMesh2Ds[counter + 1]);
        mesh2D.Cell1DSetState(e, true);
        ...
        mesh2D.Cell1DInitializeNeighbourCell2Ds(e, mesh2D.Cell1DNumberNeighbourCell2D(cell1DMesh2DToUpdate));
        ...
        for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(cell1DMesh2DToUpdate); n++)
        {
            ...
            mesh2D.Cell1DInsertNeighbourCell2D(e, n, mesh2D.Cell1DNeighbourCell2D(cell1DMesh2DToUpdate, n));
        }
    ...
    for (const unsigned int &cell2DMesh2DToUpdate : cell2DMesh2DsToUpdate)
    {
        ...
        Cell2DMesh2DUpdatedPointsAndEdges(mesh2D, cell2DMesh2DToUpdate, cell1DMesh2DToUpdate, newCell0DMesh2Ds, newCell1DMesh2Ds, newCell2DVertices, newCell2DEdges);
        ...
        if (edgeListDirection)
            mesh1D.Points[mesh1D.Segments[cell1DMesh1DIdsRedirected.back()].Points[1]].Cell2DIds.insert(c);
        else
            mesh1D.Points[mesh1D.Segments[cell1DMesh1DIdsRedirected.back()].Points[0]].Cell2DIds.insert(c);
                                                  ...
                                                  const unsigned int &cell2DMesh2DId)
{
    for (unsigned int e = 0; e < mesh2D.Cell2DNumberEdges(cell2DMesh2DId); e++)
    {
        ...
        for (unsigned int n = 0; n < mesh2D.Cell1DNumberNeighbourCell2D(cell1DId); n++)
        {
            ...
            if (!mesh2D.Cell1DUpdatedCell1Ds(cell1DId, newEdges))
            {
                ...
                const std::vector<unsigned int> new_edges_ordered =
                    Cell2DOrderNewEdges(mesh2D, cell1DId, std::vector<unsigned int>(newEdges.begin(), newEdges.end()));

                // get new points to insert between origin and end of old edge
                vector<unsigned int> newCell1DPoints, newCell1DEdges;
                Cell2DMesh2DListCell1DToListCell0D(mesh2D, cell1DId, new_edges_ordered, newCell1DPoints, newCell1DEdges);

                vector<unsigned int> newCell2DVertices, newCell2DEdges;
                Cell2DMesh2DUpdatedPointsAndEdges(mesh2D, cell2DNeigh, cell1DId, newCell1DPoints, newCell1DEdges, newCell2DVertices, newCell2DEdges);
                ...
                for (map<double, ConformerMeshSegment::ConformMesh::ConformMeshPoint>::iterator it = mesh1D.Points.begin();
                     it != mesh1D.Points.end();
                     it++)
                {
                    ConformerMeshSegment::ConformMesh::ConformMeshPoint &point = it->second;
                    if (point.Cell2DIds.find(cell2DNeigh) == point.Cell2DIds.end())
                        continue;

                    point.Cell2DIds.insert(c);
                }
                                                        ...
                                                        IMeshDAO &mesh2D)
{
    ...
    CheckSegmentOriginAndEndIntersections(segmentOrigin, segmentEnd, mesh1D, mesh2D);
    ...
    while (numVisitedCell1DMesh1D < mesh1D.Segments.size())
    {
        ...
        Cell2DMesh2DToMaps(mesh2D, cell2DId, cell2DMesh2DVerticesMap, cell2DMesh2DEdgesMap);

        // update cell2d of mesh2d
        GeometryUtilities::SplitPolygonInput splitInput;
        Cell2DMesh2DToSplitInput(cell1DMesh1DIds, mesh1D, mesh2D, cell2DId, cell2DMesh2DVerticesMap, cell2DMesh2DEdgesMap, splitInput);
        ...
        {
            // create new cells
            SplitCell2DMesh2D(segmentOrigin, segmentTangent, cell1DMesh1DIds, mesh1D, mesh2D, cell2DId, cell2DMesh2DVerticesMap, cell2DMesh2DEdgesMap, splitResult);
        }
        ...
        case GeometryUtilities::SplitPolygonWithSegmentResult::Types::PolygonCreation:
            InsertCell2DMesh2DMiddleEdgesPolygonCreation(segmentOrigin, segmentTangent, mesh1D, mesh2D, cell1DMesh1DIds, cell2DId);
            break;
        case GeometryUtilities::SplitPolygonWithSegmentResult::Types::PolygonUpdate:
            InsertCell2DMesh2DMiddleEdgesPolygonUpdate(segmentOrigin, segmentTangent, mesh1D, mesh2D, cell1DMesh1DIds, cell2DId);
            break;
        default:
        ...
        UpdateCell2DNeighbours(mesh1D, mesh2D, cell2DId);
    }
                                                        ...
                                                        IMeshDAO &mesh2D)
{
    ...
    while (numVisitedCell1DMesh1D < mesh1D.Segments.size())
    {
        ...
        {
            list<unsigned int> cell2DMesh2DsToUpdate;
            mesh2D.Cell2DUpdatedCell2Ds(cell2DId, cell2DMesh2DsToUpdate);
            Output::Assert(cell2DMesh2DsToUpdate.size() == 1);
        ...
        UpdateCell2DMesh2DWithSegmentOnEdges(segmentOrigin,
                                             ...
                                             mesh1D,
                                             mesh2D,
                                             vector<unsigned int>(cell1DMesh1DIds.begin(), cell1DMesh1DIds.end()),
                                             cell2DId);
                                             ...
                                             Gedim::IMeshDAO &mesh2D)
{
    ...
    case ConformerMeshPolygonConfiguration::Types::Generalized:
        return CreateConformMeshGeneralized(segmentOrigin, segmentEnd, segmentTangent, mesh1D, mesh2D);
    case ConformerMeshPolygonConfiguration::Types::OnlyOnEdges:
        return CreateConformMeshOnlyOnEdges(segmentOrigin, segmentEnd, segmentTangent, mesh1D, mesh2D);
    default:
