...
namespace Gedim
{
                                                  ...
                                                  IMeshDAO& mesh) const
  {
    ...
    const auto cell1Ds = ComputeMesh2DCell1Ds(cell0Ds,
                                              cell2Ds);
...

    {
      ...
      for (unsigned int t = 0; t < numCell2Ds; t++)
      {
        ...
        if (marker != 0)
        {
          ...
          unsigned int cell1DIndex = mesh.Cell1DByExtremes(correctedOriginIndex,
                                                           correctedEndIndex);
          if (cell1DIndex == mesh.Cell1DTotalNumber())
          {
            cell1DIndex = mesh.Cell1DByExtremes(correctedEndIndex,
                                                correctedOriginIndex);
          }
