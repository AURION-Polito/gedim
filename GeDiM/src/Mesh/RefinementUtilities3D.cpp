                                                                                                         ...
                                                                                                         const Eigen::VectorXd &edgesLength) const
{
    ...
    while (vertexFound < 2 && vertexNumber < 4)
    {
        if (vertexNumber != edgeOrigin && vertexNumber != edgeEnd)
        {
...
// ***************************************************************************
RefinementUtilities::RefinePolyhedron_Result RefinementUtilities::RefinePolyhedronCell_ByPlane(
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    if (geometryUtilities.IsValueZero(0.5 * cell3DVolume, geometryUtilities.Tolerance3D()))
    {
        result.ResultType = RefinePolyhedron_Result::ResultTypes::Cell3DSplitUnderTolerance;
        return result;
    }

    const Gedim::GeometryUtilities::SplitPolyhedronWithPlaneResult split_cell =
        geometryUtilities.SplitPolyhedronWithPlane(cell3DVertices,
                                                   cell3DEdges,
                                                   cell3DFaces,
    ...
    case Gedim::GeometryUtilities::SplitPolyhedronWithPlaneResult::Types::Split: {
        ...
        for (unsigned int nv = 0; nv < split_cell.Vertices.NewVerticesOriginalEdge.size(); nv++)
        {
            ...
            if (geometryUtilities.IsValueZero(0.5 * cell3DEdgesLength[originalEdgeIndex], geometryUtilities.Tolerance1D()))
            {
                result.ResultType = RefinePolyhedron_Result::ResultTypes::Cell3DSplitUnderTolerance;
                return result;
            }
    ...
    std::vector<unsigned int> splitCell0DsIndex(split_cell.Vertices.Vertices.cols(), std::numeric_limits<unsigned int>::max());
    ...
    {
        const unsigned int originalEdgeIndex = split_cell.Vertices.NewVerticesOriginalEdge.at(nv);
        const unsigned int originalCell1DIndex = mesh.Cell3DEdge(cell3DIndex, originalEdgeIndex);

        const SplitCell1D_Result splitResult =
            SplitCell1D(originalCell1DIndex, split_cell.Vertices.Vertices.col(numOriginalVertices + nv), mesh);
        ...
        if ((newEdgeOneCell0DOrigin == newCell1DOneOrigin && newEdgeOneCell0DEnd == newCell1DOneEnd) ||
            (newEdgeOneCell0DEnd == newCell1DOneOrigin && newEdgeOneCell0DOrigin == newCell1DOneEnd))
        {
            splitCell1DsIndex[newEdges[0]] = refinedCell1D.NewCell1DsIndex[0];
            splitCell1DsIndex[newEdges[1]] = refinedCell1D.NewCell1DsIndex[1];
        }
        else if ((newEdgeOneCell0DOrigin == newCell1DTwoOrigin && newEdgeOneCell0DEnd == newCell1DTwoEnd) ||
                 (newEdgeOneCell0DEnd == newCell1DTwoOrigin && newEdgeOneCell0DOrigin == newCell1DTwoEnd))
        {
        ...
        edgeIndexToNewCell1Ds.insert(std::make_pair(newEdges[0], newCell1Ds.size()));
        edgeIndexToNewCell1Ds.insert(std::make_pair(newEdges[1], newCell1Ds.size()));
    ...
    for (unsigned int ne = 0; ne < split_cell.Edges.NewEdgesOriginalEdges.size(); ne++)
    {
        if (split_cell.Edges.NewEdgesOriginalEdges[ne] >= 0)
        {
            ...
            splitCell1DsIndex[ne] = mesh.Cell3DEdge(cell3DIndex, split_cell.Edges.NewEdgesOriginalEdges[ne]);
            continue;
        }

        const unsigned int originalFaceIndex = split_cell.Edges.NewEdgesOriginalFace.at(ne);
        const unsigned int originalCell2DIndex = mesh.Cell3DFace(cell3DIndex, originalFaceIndex);
        ...
        mesh.Cell1DInsertExtremes(newCell1DIndex, cell0DIndexEnd, cell0DIndexOrigin);
        mesh.Cell1DSetMarker(newCell1DIndex, mesh.Cell2DMarker(originalCell2DIndex));
        mesh.Cell1DSetState(newCell1DIndex, true);
        mesh.Cell1DInitializeNeighbourCell3Ds(newCell1DIndex, 3);
        for (unsigned int nf = 0; nf < mesh.Cell2DNumberNeighbourCell3D(originalCell2DIndex); nf++)
        {
            ...
            mesh.Cell1DInsertNeighbourCell3D(newCell1DIndex, 0, neighCell3DIndex);
        }

        edgeIndexToNewCell1Ds.insert(std::make_pair(ne, newCell1Ds.size()));
    ...
    for (unsigned int ne = 0; ne < split_cell.Edges.NewEdgesOriginalEdges.size(); ne++)
    {
        ...
        for (unsigned int v = 0; v < newFaceOne.cols(); v++)
        {
            ...
            if (edgeIndexToNewCell1D != edgeIndexToNewCell1Ds.end())
            {
                if (newCell1DsPosition.find(edgeIndexToNewCell1D->second) == newCell1DsPosition.end())
        ...
        const std::vector<unsigned int> newCell2DIndices = meshUtilities.SplitCell2D(originalCell2DIndex, subCells, mesh);
        ...
        refinedCell2D.NewCell1DsPosition = std::vector<unsigned int>(newCell1DsPosition.begin(), newCell1DsPosition.end());
    ...
    for (unsigned int nf = 0; nf < split_cell.Faces.NewFacesOriginalFaces.size(); nf++)
    {
        if (split_cell.Faces.NewFacesOriginalFaces[nf] >= 0)
        {
            ...
            splitCell2DsIndex[nf] = mesh.Cell3DFace(cell3DIndex, split_cell.Faces.NewFacesOriginalFaces[nf]);
            continue;
        }
        ...
        for (unsigned int nfv = 0; nfv < newFace.cols(); nfv++)
        {
            ...
            if (split_cell.Edges.NewEdgesOriginalEdges.at(newFace(1, nfv)) >= 0)
            {
                ...
                mesh.Cell1DInitializeNeighbourCell3Ds(cell1DIndex, numCell1DNeigh3Ds + 1);
            }
        }
        ...
        mesh.Cell2DAddVerticesAndEdges(newCell2DIndex, cell2DExtremes);

        mesh.Cell2DSetMarker(newCell2DIndex, 0);
        mesh.Cell2DSetState(newCell2DIndex, true);
        mesh.Cell2DInitializeNeighbourCell3Ds(newCell2DIndex, 2);
    ...
    for (unsigned int nc = 0; nc < 2; nc++)
    {
        const Gedim::GeometryUtilities::SplitPolyhedronWithPlaneResult::NewPolyhedron newPolyhedron =
            (nc == 0) ? split_cell.PositivePolyhedron : split_cell.NegativePolyhedron;
    ...
    const std::vector<unsigned int> newCell3DIndices =
        meshUtilities.SplitCell3D(cell3DIndex, newCell3DsVertices, newCell3DsEdges, newCell3DsFaces, mesh);
    ...
    for (unsigned int nf = 0; nf < split_cell.Faces.NewFacesOriginalFaces.size(); nf++)
    {
        ...
        mesh.Cell2DInsertNeighbourCell3D(newCell2DIndex, 0, newCell3DIndices[0]);
        mesh.Cell2DInsertNeighbourCell3D(newCell2DIndex, 1, newCell3DIndices[1]);
        ...
        for (unsigned int nfe = 0; nfe < newFace.cols(); nfe++)
        {
            if (split_cell.Edges.NewEdgesOriginalEdges.at(newFace(1, nfe)) >= 0)
            {
                ...
                mesh.Cell1DInsertNeighbourCell3D(cell1DIndex,
                                                 numCell1DNeigh3Ds - 1,
                                                 hasPositiveCell3D ? newCell3DIndices.at(1) : newCell3DIndices.at(0));
            }
        }
    }
    ...
    for (const RefinePolyhedron_Result::RefinedCell1D &newCell1D : newCell1Ds)
    {
        ...
        mesh.Cell1DInsertNeighbourCell3D(cell1DIndex, 1, newCell3DIndices[0]);
        mesh.Cell1DInsertNeighbourCell3D(cell1DIndex, 2, newCell3DIndices[1]);
    }

    result.NewCell1DsIndex = std::vector<RefinePolyhedron_Result::RefinedCell1D>(newCell1Ds.begin(), newCell1Ds.end());
    result.NewCell2DsIndex = std::vector<RefinePolyhedron_Result::RefinedCell2D>(newCell2Ds.begin(), newCell2Ds.end());
...
// ***************************************************************************
RefinementUtilities::RefinePolyhedron_UpdateNeighbour_Result RefinementUtilities::RefinePolyhedronCell_UpdateFaceNeighbours(
    const unsigned int &cell3DIndex,
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int n = 0; n < mesh.Cell2DNumberNeighbourCell3D(cell2DIndex); n++)
    {
        ...
        const unsigned int neighFaceIndex = mesh.Cell3DFindFace(neighCell3DIndex, cell2DIndex);
        ...
        for (unsigned int ne = 0; ne < splitCell1DsOriginalIndex.size(); ne++)
        {
            ...
            const unsigned int neighEdgeIndex = mesh.Cell3DFindEdge(neighCell3DIndex, cell1DIndex);
            ...
            for (unsigned int nef = 0; nef < originalFaces.size(); nef++)
            {
                ...
                const unsigned int localFaceEdgeIndex = mesh.Cell2DFindEdge(cell1DCell2DIndex, cell1DIndex);
                if (localFaceEdgeIndex == mesh.Cell2DNumberVertices(cell1DCell2DIndex))
                    continue;

                const unsigned int localFaceIndex = mesh.Cell3DFindFace(neighCell3DIndex, cell1DCell2DIndex);
                Gedim::Output::Assert(localFaceIndex < originalFaces.size());

                if (updatedCell2Ds.find(cell1DCell2DIndex) == updatedCell2Ds.end())
                {
                    const unsigned int updatedCell2DIndex = UpdateCell2D_NewVertex(
                        cell1DCell2DIndex,
                    ...
                    updatedCell2Ds.insert(std::make_pair(cell1DCell2DIndex, updatedCell2DIndex));
                }
                else
        ...
        const std::vector<unsigned int> newCell3DIndices =
            meshUtilities.SplitCell3D(neighCell3DIndex, newCell3DsVertices, newCell3DsEdges, newCell3DsFaces, mesh);
    ...
    result.UpdatedCell3Ds =
        std::vector<RefinePolyhedron_UpdateNeighbour_Result::UpdatedCell3D>(newCell3DsIndex.begin(), newCell3DsIndex.end());
    return result;
}
// ***************************************************************************
RefinementUtilities::RefinePolyhedron_UpdateNeighbour_Result RefinementUtilities::RefinePolyhedronCell_UpdateEdgeNeighbours(
    const unsigned int &cell3DIndex,
    ...
    Gedim::IMeshDAO &mesh) const
{
    ...
    for (unsigned int n = 0; n < mesh.Cell1DNumberNeighbourCell3D(cell1DIndex); n++)
    {
        ...
        for (unsigned int nef = 0; nef < originalFaces.size(); nef++)
        {
            ...
            const unsigned int localFaceEdgeIndex = mesh.Cell2DFindEdge(cell2DIndex, cell1DIndex);
            if (localFaceEdgeIndex == mesh.Cell2DNumberVertices(cell2DIndex))
                continue;

            const unsigned int localFaceIndex = mesh.Cell3DFindFace(neighCell3DIndex, cell2DIndex);
            Gedim::Output::Assert(localFaceIndex < originalFaces.size());

            if (updatedCell2Ds.find(cell2DIndex) == updatedCell2Ds.end())
            {
                const unsigned int updatedCell2DIndex = UpdateCell2D_NewVertex(
                    cell2DIndex,
                    cell3DsFacesEdgesDirection.at(neighCell3DIndex).at(localFaceIndex).at(localFaceEdgeIndex),
                    ...
                    mesh);
                newCell3DsFaces[0][localFaceIndex] = updatedCell2DIndex;
                updatedCell2Ds.insert(std::make_pair(cell2DIndex, updatedCell2DIndex));
            }
            else
