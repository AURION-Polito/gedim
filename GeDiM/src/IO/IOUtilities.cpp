                               ...
                               const unsigned int &startingPosition)
{
    ...
    if (fileSize < (startingPosition * sizeOfSingleDataToWrite))
        throw runtime_error("Error in file '" + nameFile + "': uncorrect starting position. Expected " +
                            to_string(startingPosition) + ", obtained " + to_string(fileSize / sizeof(double)));
...
// ***************************************************************************
void Output::ReadBinaryFile(const string &nameFile,
                            void *dataToRead,
                            const unsigned int &sizeOfSingleDataToWrite,
                            ...
                            const unsigned int &startingPosition)
{
    ...
    if (fileSize < (dataSizeToRead * sizeOfSingleDataToWrite))
        throw runtime_error("Error in file '" + nameFile + "': uncorrect size. Expected " + to_string(dataSizeToRead) +
                            ", obtained " + to_string(fileSize / sizeof(double)));
                             ...
                             const bool &append)
{
    ...
    {
        Output::PrintErrorMessage("Error in write file '%s': uncorrect size. Required %d, given %d",
                                  false,
                                  nameFile.c_str(),
                                  dataSizeToWrite,
                                  dataToWrite.size());
    ...
    {
        Output::PrintErrorMessage("Error in write file '%s': uncorrect starting position. Required %d, given %d",
                                  false,
                                  nameFile.c_str(),
...
// *************************************************************************
void Profiler::InsertTime(const std::string &nameTime, const double &globalTime, const double &localTime)
{
    map<string, double>::iterator timeFound = times.find(nameTime);
    map<string, double>::iterator localTimeFound = localTimes.find(nameTime);
...
double Profiler::StopTime(const string &nameTime, const bool &printTime)
{
    ...
    WriteTime(nameTime, timeDifference, localTimeDifference);

    return timeDifference;
}
// *************************************************************************
void Profiler::WriteTime(const std::string &nameTime, const double &globalTime, const double &localTime)
{
    ...
    if (MpiParallelEnvironment::Process().Rank() == 0)
    {
        ...
        cout << " -- " << Output::BlueColor << "Profiling" << Output::EndColor << ": Time '" << nameTime
             << "': " << hours << " h " << minutes << " m " << seconds << " s " << milliseconds << " ms --" << endl;
#endif
...
void Profiler::CheckMemory(const string &nameMemory, const bool &checkProcessesMemory)
{
    ...
    unsigned long totalProcessMemory = (unsigned long)totalProcessesMemoryGigabytes * 1000000 +
                                       (unsigned long)totalProcessesMemoryMegabytes * 1000 +
                                       (unsigned long)totalProcessesMemoryKilobytes;
    ...
    totalProcessesMemoryKilobytes =
        (totalProcessMemory - totalProcessesMemoryGigabytes * 1000000 - totalProcessesMemoryMegabytes * 1000);
    ...
    if (MpiParallelEnvironment::Process().Rank() == 0)
    {
        ...
        outFile << MpiParallelEnvironment::Process().NumberProcesses() << " " << nameMemory << " " << totalProcessMemory
                << " " << totalAvailMemory << " "
                << (totalProcessMemory == 0 ? 0 : (unsigned int)(1.0 / (1.0 + totalAvailStartingMemory / totalProcessMemory) * 100.0))
                << endl;
    }
#if USE_MPI == 1
    MPI::COMM_WORLD.Barrier();
