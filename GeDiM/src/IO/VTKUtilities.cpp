...
// ***************************************************************************
void VTKUtilities::AddPoint(const Eigen::Vector3d &point, const vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    const VTPPoint vtpPoint(point);
    GeometryToPolyData<VTPPoint> polyData(vtpPoint, properties);
...
// ***************************************************************************
void VTKUtilities::AddPoints(const Eigen::MatrixXd &points, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    const VTPPoints vtpPoints(points);
    GeometryToPolyData<VTPPoints> polyData(vtpPoints, properties);
    AddToExportData(polyData);
#else
...
// ***************************************************************************
void VTKUtilities::AddSegment(const Eigen::Vector3d &origin, const Eigen::Vector3d &end, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    ...
    const VTPSegment vtpSegment(vertices, edge);
    GeometryToPolyData<VTPSegment> polyData(vtpSegment, properties);
    AddToExportData(polyData);
#else
...
// ***************************************************************************
void VTKUtilities::AddSegment(const Eigen::MatrixXd &vertices, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    const Eigen::VectorXi edge = (Eigen::VectorXi(2) << 0, 1).finished();
    const VTPSegment vtpSegment(vertices, edge);
...
// ***************************************************************************
void VTKUtilities::AddSegment(const Eigen::MatrixXd &vertices, const Eigen::VectorXi &edge, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    const VTPSegment vtpSegment(vertices, edge);
    GeometryToPolyData<VTPSegment> polyData(vtpSegment, properties);
...
// ***************************************************************************
void VTKUtilities::AddSegments(const Eigen::MatrixXd &vertices, const Eigen::MatrixXi &edges, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    const VTPSegments vtpSegments(vertices, edges);
    GeometryToPolyData<VTPSegments> polyData(vtpSegments, properties);
    AddToExportData(polyData);
#else
    Utilities::Unused(vertices);
...
// ***************************************************************************
void VTKUtilities::AddPolygon(const Eigen::MatrixXd &vertices, const std::vector<VTPProperty> &properties)
{
#if ENABLE_VTK == 1
    std::vector<unsigned int> polygon(vertices.cols());
    std::iota(polygon.begin(), polygon.end(), 0);

    const VTPPolygon vtpPolygon(vertices, polygon);
    GeometryToPolyData<VTPPolygon> polyData(vtpPolygon, properties);
    AddToExportData(polyData);
#else
    Utilities::Unused(vertices);
    Utilities::Unused(properties);
                               ...
                               const std::vector<VTPProperty> &properties)
{
    ...
    AddPolygons(vertices, polygonVertices, properties);
}
// ***************************************************************************
void VTKUtilities::AddPolygons(const Eigen::MatrixXd &vertices,
...
{
#if ENABLE_VTK == 1
    const VTPPolygons vtpPolygons(vertices, polygons);
    GeometryToPolyData<VTPPolygons> polyData(vtpPolygons, properties);
    AddToExportData(polyData);
#else
    Utilities::Unused(vertices);
                                 ...
                                 const std::vector<VTPProperty> &properties)
{
    ...
    const VTPPolyhedron vtpPolyhedron(vertices, polyhedronFaces);
    GeometryToPolyData<VTPPolyhedron> polyData(vtpPolyhedron, properties);
    AddToExportData(polyData);
#else
    Utilities::Unused(vertices);
    Utilities::Unused(edges);
...
{
#if ENABLE_VTK == 1
    const VTPPolyhedrons vtpPolyhedrons(vertices, polyhedronsFaces);
    GeometryToPolyData<VTPPolyhedrons> polyData(vtpPolyhedrons, properties);
    AddToExportData(polyData);
#else
    Utilities::Unused(vertices);
...
// ***************************************************************************
void VTKUtilities::Export(const std::string &filePath, const ExportFormats &format) const
{
#if ENABLE_VTK == 1
    exportData->Update();
...
// ***************************************************************************
template <typename T>
GeometryToPolyData<T>::GeometryToPolyData(const T &geometry, const std::vector<VTPProperty> &properties)
    : geometry(geometry), properties(properties)
{
}
template <typename T> GeometryToPolyData<T>::~GeometryToPolyData()
{
}
// ***************************************************************************
#if ENABLE_VTK == 1
template <typename T>
void GeometryToPolyData<T>::AddPoint(const Eigen::Vector3d &point, vtkNew<vtkPoints> &points) const
{
    points->InsertNextPoint(point(0), point(1), point(2));
}
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddPoints(const Eigen::MatrixXd &points, vtkNew<vtkPoints> &vtkPoints) const
{
    for (unsigned int p = 0; p < points.cols(); p++)
    {
        vtkPoints->InsertNextPoint(points(0, p), points(1, p), points(2, p));
    }
}
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddVertex(const unsigned int &pointId, vtkNew<vtkCellArray> &vertices) const
{
...
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddVertices(const std::vector<unsigned int> &pointIds, vtkNew<vtkCellArray> &vertices) const
{
...
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddLine(const unsigned int &originId, const unsigned int &endId, vtkNew<vtkCellArray> &lines) const
{
...
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddLines(const Eigen::MatrixXi &edges, vtkNew<vtkCellArray> &lines) const
{
...
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddPolygon(const vector<unsigned int> &faceVerticesIds, vtkNew<vtkCellArray> &faces) const
{
...
// ***************************************************************************
template <typename T>
void GeometryToPolyData<T>::AddPolygons(const std::vector<std::vector<unsigned int>> &facesVerticesIds, vtkNew<vtkCellArray> &faces) const
{
...
// ***************************************************************************
template <typename T> void GeometryToPolyData<T>::AppendSolution(vtkDataSet *polyData) const
{
    ...
    for (unsigned int s = 0; s < properties.size(); s++)
    {
        ...
        case VTPProperty::Formats::PointsArray: {
            ...
            for (unsigned int p = 0; p < numTuples; p++)
            {
                vtkSolution->SetTuple3(p, property.Data[3 * p], property.Data[3 * p + 1], property.Data[3 * p + 2]);
            }
        }
        break;
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPoint>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    ...
    AddPoint(geometry.Point, points);
    AddVertex(0, vertices);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPoints>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkPolyData> polyData;
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPSegment>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkPolyData> polyData;
    ...
    AddPoint(geometry.Vertices.col(0), points);
    AddPoint(geometry.Vertices.col(1), points);

    AddLine(geometry.Edge(0), geometry.Edge(1), lines);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPSegments>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkPolyData> polyData;
    ...
    AddPoints(geometry.Vertices, points);

    AddLines(geometry.Edges, lines);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPolygon>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkPolyData> polyData;
    ...
    AddPoints(geometry.Vertices, points);
    AddPolygon(geometry.Polygon, faces);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPolygons>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkPolyData> polyData;
    ...
    AddPoints(geometry.Vertices, points);
    AddPolygons(geometry.Polygons, faces);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPolyhedron>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    ...
    AddPoints(geometry.Vertices, points);
    AddPolygons(geometry.PolyhedronFaces, faces);
...
// ***************************************************************************
template <> void GeometryToPolyData<VTPPolyhedrons>::Convert(vtkNew<vtkAppendFilter> &exportData) const
{
    vtkNew<vtkUnstructuredGrid> polyData;
    ...
    for (unsigned int p = 0; p < numPolyhedrons; p++)
    {
        ...
        AddPolygons(geometry.PolyhedronsFaces[p], faces);
