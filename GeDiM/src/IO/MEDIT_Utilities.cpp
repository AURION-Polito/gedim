...
namespace Gedim
{
    ...
    switch (format)
    {
      case ExportFormats::Ascii:
        ExportMEDIT_Ascii(points,
                          points_reference_ids,
                          {},
                          CreateEdgeCells(cells, cells_reference_ids),
                          filePath);
        break;
      default:
        throw std::runtime_error("Unknown format");
  ...
  void MEDIT_Utilities::ExportPolygons(const std::string &filePath,
                                       ...
                                       const std::vector<unsigned int> &points_reference_ids, const std::vector<unsigned int>& cells_reference_ids) const
  {
    ExportFormats format = ExportFormats::Ascii;
    ...
    switch (format)
    {
      case ExportFormats::Ascii:
        ExportMEDIT_Ascii(points,
                          points_reference_ids,
                          {},
                          CreatePolygonCells(cells_vertices, cells_reference_ids),
                          filePath);
        break;
      default:
        throw std::runtime_error("Unknown format");
                                        ...
                                        const std::vector<unsigned int>& cells_reference_ids) const
  {
    ...
    switch (format)
    {
      case ExportFormats::Ascii:
        ExportMEDIT_Ascii(points,
                          points_reference_ids,
                          CreatePolygonCells(faces_vertices,
                                             faces_reference_ids),
                          CreatePolyhedraCells(cells_vertices,
                                               cells_reference_ids),
                          filePath);
        break;
      default:
  ...
  // ***************************************************************************
  std::vector<MEDIT_Cell> MEDIT_Utilities::CreateEdgeCells(const Eigen::MatrixXi &edges,
                                                           const std::vector<unsigned int> &reference_ids) const
  {
    std::vector<MEDIT_Cell> cells;
                                                              ...
                                                              const std::vector<unsigned int> &reference_ids) const
  {
    ...
    for (unsigned int l = 0; l < polygons_vertices.size(); l++)
    {
      ...
      cells.push_back(MEDIT_Cell(polygon_type,
                                 polygons_vertices[l],
                                                                ...
                                                                const std::vector<unsigned int> &reference_ids) const
  {
    ...
    for (unsigned int l = 0; l < polyhedra_vertices.size(); l++)
    {
      ...
      cells.push_back(MEDIT_Cell(polyhedra_type,
