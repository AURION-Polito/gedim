...
// ***************************************************************************
void Eigen_Utilities::RemoveRow(Eigen::MatrixXd &matrix, const unsigned int &rowToRemove)
{
...
// ***************************************************************************
void Eigen_Utilities::RemoveColumn(Eigen::MatrixXd &matrix, const unsigned int &colToRemove)
{
                                         ...
                                         const unsigned int &startingPosition)
{
    ...
    if (fileContentSize == 0)
        throw runtime_error("Error in file '" + nameFile + "': uncorrect content size. Obtained " + to_string(fileContentSize));

    dataToRead.resize(fileContentSize);
    Output::ReadBinaryFile(nameFile, dataToRead.data(), sizeof(double), fileContentSize, startingPosition);
                                        ...
                                        const bool &append)
{
    ...
    if (dataSizeToWrite > vec.size())
        throw runtime_error("Error in write file '" + nameFile + "': uncorrect size. Required " +
                            to_string(dataSizeToWrite) + ", given " + to_string(vec.size()));

    if (dataStartingPositionToWrite > vec.size())
        throw runtime_error("Error in write file '" + nameFile + "': starting position. Required " +
                            to_string(dataStartingPositionToWrite) + ", given " + to_string(vec.size()));
    ...
    /// <li> Exporting file
    Output::WriteBinaryFile(nameFile, vec.data() + dataStartingPositionToWrite, sizeof(double), fileDataSize, append);
                                         ...
                                         const unsigned int &startingPosition)
{
    ...
    if (!Output::ReadBinaryFile(nameFile, vectorToImport, 0, startingPosition))
        throw runtime_error("Error in read matrix from file '" + nameFile + "'");

    if (vectorToImport.size() < 3)
        throw runtime_error("Error in read matrix from file '" + nameFile + "': no enough information get from file");
    ...
    if (rows == 0)
        throw runtime_error("Error in read matrix from file '" + nameFile + "': no rows value");

    if (cols == 0)
        throw runtime_error("Error in read matrix from file '" + nameFile + "': no cols value");

    if (nonZeros == 0)
        throw runtime_error("Error in read matrix from file '" + nameFile + "': no nonZeros value");

    if (vectorToImport.size() - 3 < 3 * nonZeros)
        throw runtime_error("Error in read matrix from file '" + nameFile + "': no enough non zeros values");
    ...
    for (unsigned int counter = 0; counter < 3 * nonZeros; counter += 3)
        triplets.push_back(
            Triplet<double>(vectorToImport[3 + counter], vectorToImport[3 + counter + 1], vectorToImport[3 + counter + 2]));
}
// ***************************************************************************
void Eigen_Utilities::ReadFromBinaryFile(const string &nameFile, SparseMatrix<double, RowMajor> &matrix, const unsigned int &startingPosition)
{
    unsigned int rows, cols;
    vector<Triplet<double>> triplets;

    ReadFromBinaryFile(nameFile, rows, cols, triplets, startingPosition);
...
// ***************************************************************************
void Eigen_Utilities::ReadFromBinaryFile(const string &nameFile, SparseMatrix<double, ColMajor> &matrix, const unsigned int &startingPosition)
{
    unsigned int rows, cols;
    vector<Triplet<double>> triplets;
                                        ...
                                        const bool &append)
{
    ...
    if (nonZeros == 0)
        throw runtime_error("Error in write matrix to file '" + nameFile + "': empty matrix");
