...
template <typename PETSc_ArrayType = Vec, typename PETSc_SparseArrayType = Mat>
class PETSc_SparseArray final : public ISparseArray
{
  ...
  public:
    ...
    PETSc_SparseArray(const PETSc_SparseArrayType &matrix, const SparseArrayTypes &type = SparseArrayTypes::None)
    {
        _matrix = matrix;
        _matrixType = type;
    }

    operator PETSc_SparseArrayType &()
    {
        return _matrix;
    }
    operator const PETSc_SparseArrayType &() const
    {
        return _matrix;
    }
    ...
    void ToBinaryFile(const std::string &, const bool &) const
    {
        throw std::runtime_error("unimplemented method");
    }
