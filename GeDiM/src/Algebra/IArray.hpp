...
class IArray
{
  public:
    ...
    /// \brief Set the size the Array. Resize a Array for Eigen application.
    /// \param numCols the number of cols
    /// \param numLocalCols the local size for the Array (It can be set for each process. For PETSC applciation when not
    /// specified is set to PETSC_DECIDE)
    ...
    /// \brief operator <<
    inline friend std::ostream &operator<<(std::ostream &output, const IArray &b)
    {
