...
class Eigen_Utilities
{
  ...
  public:
    ...
    {
        return Eigen::Map<const Eigen::SparseMatrix<Derived>>(rows, cols, nnz, innerVectors, innerIndeces, values);
    }
    ...
    template <typename Derived, int Rows, int Cols>
    static std::vector<Eigen::Map<const Eigen::Matrix<Derived, Rows, Cols>>> MatrixToMap(
        const std::vector<Eigen::Matrix<Derived, Rows, Cols>> &matrices)
    {
    ...
    template <typename collection_type> static Eigen::MatrixXd CoordinatesToMatrix(const collection_type &coordinates)
    {
        Eigen::MatrixXd matrix(3, coordinates.size());

        unsigned int c = 0;
        for (const auto &coordinate : coordinates)
            matrix.col(c++) << coordinate;
