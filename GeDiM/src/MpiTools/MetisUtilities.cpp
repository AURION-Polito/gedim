                                                                ...
                                                                const Eigen::SparseMatrix<unsigned int> &networkEdgesWeight) const
{
    ...
    for (unsigned int f = 0; f < mesh.Cell2DTotalNumber(); f++)
    {
        for (unsigned int n1 = 0; n1 < mesh.Cell2DNumberNeighbourCell3D(f); n1++)
        {
            ...
            for (unsigned int n2 = n1 + 1; n2 < mesh.Cell2DNumberNeighbourCell3D(f); n2++)
            {
                ...
                const bool cell1DConstrained = checkConstraints ? cell2DsConstrained[f] : false;
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        ...
        network.Adjacency.Rows[v + 1] = network.Adjacency.Rows[v] + numberConnections;
    ...
    network.Adjacency.Cols = std::vector<unsigned int>(adjacencyCols.begin(), adjacencyCols.end());
    meshToNetwork.EdgesMeshCellIndex =
        std::vector<unsigned int>(adjacencyColsMeshCellIndex.begin(), adjacencyColsMeshCellIndex.end());
    network.EdgesConstrained = std::vector<bool>(adjacencyColsConstrained.begin(), adjacencyColsConstrained.end());
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        for (const Connection &connection : verticesConnections[v])
        {
            ...
            network.EdgesWeight[counter++] =
                !checkConstraints
                    ? weight
                    : (constraints.find(std::make_pair(v, connection.Cell3DIndex)) != constraints.end() ? 1 : weight + numEdges);
        }
    }
                                                         ...
                                                         const std::vector<unsigned int> &edgesWeight) const
{
    ...
    for (unsigned int e = 0; e < numEdges; e++)
    {
        ...
        verticesConnections[edge[0]].push_back(
            Connection{static_cast<unsigned int>(edge[0]),
        ...
        if (undirectEdges)
        {
            verticesConnections[edge[1]].push_back(
                Connection{static_cast<unsigned int>(edge[1]),
    ...
    network.EdgesWeight = std::vector<unsigned int>(networkEdgesWeight.begin(), networkEdgesWeight.end());

    return network;
}
// ***************************************************************************
MetisUtilities::MetisNetwork::MetisAdjacency MetisUtilities::GraphAdjacencyToMetisAdjacency(
    const std::vector<std::vector<unsigned int>> &graphAdjacency) const
{
    ...
    metisAdjacency.Cols = std::vector<unsigned int>(adjacencyCols.begin(), adjacencyCols.end());
    ...
    for (unsigned int e = 0; e < mesh.Cell1DTotalNumber(); e++)
    {
        for (unsigned int n1 = 0; n1 < mesh.Cell1DNumberNeighbourCell2D(e); n1++)
        {
            ...
            for (unsigned int n2 = n1 + 1; n2 < mesh.Cell1DNumberNeighbourCell2D(e); n2++)
            {
                ...
                const bool cell1DConstrained = checkConstraints ? cell1DsConstrained[e] : false;
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        for (const Connection &connection : verticesConnections[v])
        {
            ...
            network.EdgesWeight[counter++] =
                !checkConstraints
                    ? weight
                    : (constraints.find(std::make_pair(v, connection.Cell2DIndex)) != constraints.end() ? 1 : weight + numEdges);
        }
    }
...
// ***************************************************************************
std::vector<unsigned int> MetisUtilities::NetworkPartition(const NetworkPartitionOptions &options, const MetisNetwork &network) const
{
    /// <ul>
    ...
    memcpy(xadj.data(), network.Adjacency.Rows.data(), sizeof(idx_t) * (numberElements + 1));
    memcpy(adjncy.data(), network.Adjacency.Cols.data(), sizeof(idx_t) * numberConnections);

    METIS_SetDefaultOptions(metisOptions);

    metisOptions[METIS_OPTION_CTYPE] = options.CoarseningSchema == NetworkPartitionOptions::CoarseningSchemes::Default
                                           ? -1
                                           : static_cast<mctype_et>(options.CoarseningSchema);
    metisOptions[METIS_OPTION_IPTYPE] = options.InitialPartitioningSchema == NetworkPartitionOptions::InitialPartitioningSchemes::Default
                                            ? -1
                                            : static_cast<miptype_et>(options.InitialPartitioningSchema);
    metisOptions[METIS_OPTION_RTYPE] = options.RefinementSchema == NetworkPartitionOptions::RefinementSchemes::Default
                                           ? -1
                                           : static_cast<mrtype_et>(options.RefinementSchema);

    metisOptions[METIS_OPTION_CONTIG] = options.ContigousPartitions ? 1 : 0;
    metisOptions[METIS_OPTION_DBGLVL] =
        options.DebugLevel == NetworkPartitionOptions::DebugLevels::None ? -1 : static_cast<mdbglvl_et>(options.DebugLevel);
    ...
    case NetworkPartitionOptions::PartitionTypes::CutBalancing: {
        ...
        if (network.NodesWeight.size() > 0)
        {
            ...
            const idx_t smallestWeight = *std::min_element(vwgt, vwgt + numberElements);
            if (smallestWeight < 0)
        ...
        if (network.EdgesWeight.size() > 0)
        {
            ...
            const idx_t smallestWeight = *std::min_element(adjwgt, adjwgt + numberConnections);
            if (smallestWeight < 0)
    ...
    case NetworkPartitionOptions::PartitionTypes::VolBalancing: {
        ...
        if (network.NodesWeight.size() > 0)
        {
            ...
            const idx_t smallestWeight = *std::min_element(v_size, v_size + numberElements);
            if (smallestWeight < 0)
                throw std::runtime_error("NodesWeight are too large");
        }
    }
    break;
    default:
        throw std::runtime_error("MetisDivisionType " + std::to_string(static_cast<unsigned int>(metisDivisionType)) + " not supported");
    }
    ...
    // print
    if (options.DebugLevel != NetworkPartitionOptions::DebugLevels::None)
    {
                                                                    ...
                                                                    const std::vector<unsigned int> &partitions) const
{
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        ...
        for (unsigned int c = 0; c < numConnections; c++)
        {
            ...
            if (fixedPartition.at(v) != fixedPartition.at(adj_v))
                continue;
                                                                            ...
                                                                            const std::vector<unsigned int> &partitions) const
{
    ...
    for (unsigned int v = 0; v < numVertices; v++)
    {
        ...
        subGraphs_filter[partion].insert(std::make_pair(v, subGraphs_filter[partion].size()));
    }
    ...
    {
        const std::vector<std::vector<unsigned int>> subGraph_adjacency =
            graphUtilities.ExtractSubGraph(graph_adjacency, subGraphs_filter[p]);

        const std::vector<std::vector<unsigned int>> connectedComponents =
            graphUtilities.ComputeStronglyConnectedComponents(subGraph_adjacency);

        if (connectedComponents.size() < 2)
            continue;

        std::vector<unsigned int> subGraph_vertices(subGraphs_vertices[p].begin(), subGraphs_vertices[p].end());
