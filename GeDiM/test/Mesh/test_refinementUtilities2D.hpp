...
TEST(TestRefinementUtilities, TestRefineTriangles)
{
    ...
    const Gedim::RefinementUtilities::TriangleMaxEdgeDirection direction =
        refinementUtilities.ComputeTriangleMaxEdgeDirection(meshGeometricData.Cell2DsEdgeLengths.at(cell2DToRefineIndex));
    EXPECT_EQ(2, direction.MaxEdgeIndex);
    EXPECT_EQ(1, direction.OppositeVertexIndex);

    const Gedim::RefinementUtilities::RefinePolygon_Result result =
        refinementUtilities.RefineTriangleCell_ByEdge(cell2DToRefineIndex,
                                                      ...
                                                      meshDAO);
    EXPECT_EQ(std::vector<unsigned int>({4}), result.NewCell0DsIndex);
    EXPECT_EQ(2, result.NewCell1DsIndex.size());
    EXPECT_EQ(Gedim::RefinementUtilities::RefinePolygon_Result::RefinedCell1D::Types::Updated, result.NewCell1DsIndex[0].Type);
    EXPECT_EQ(std::vector<unsigned int>({5, 6}), result.NewCell1DsIndex[0].NewCell1DsIndex);
    EXPECT_EQ(2, result.NewCell1DsIndex[0].OriginalCell1DIndex);
    EXPECT_EQ(4, result.NewCell1DsIndex[0].NewCell0DIndex);
    EXPECT_EQ(Gedim::RefinementUtilities::RefinePolygon_Result::RefinedCell1D::Types::New, result.NewCell1DsIndex[1].Type);
    EXPECT_EQ(std::vector<unsigned int>({7}), result.NewCell1DsIndex[1].NewCell1DsIndex);
    EXPECT_EQ(std::vector<unsigned int>({2, 3}), result.NewCell2DsIndex);
    ...
    for (unsigned int e = 0; e < result.NewCell1DsIndex.size(); e++)
    {
        ...
        refinementUtilities.RefineTriangleCell_UpdateNeighbours(
            cell2DToRefineIndex,
...
TEST(TestRefinementUtilities, TestRefineTriangles_ByArea)
{
    ...
    for (unsigned int r = 0; r < maxRefinements; r++)
    {
        ...
        {
            meshGeometricData = meshUtilities.FillMesh2DGeometricData(geometryUtilities, meshDAO);

            std::list<unsigned int> updatedCell2Ds;
            meshDAO.Cell2DUpdatedCell2Ds(cell2DsToRefineIndex[c], updatedCell2Ds);

            if (updatedCell2Ds.size() > 1)
                continue;

            const unsigned int cell2DToRefineIndex = updatedCell2Ds.size() == 0 ? cell2DsToRefineIndex[c]
            ...
            {
                if (refineResult.NewCell1DsIndex[e].Type !=
...
TEST(TestRefinementUtilities, TestRefinePolygons_NoNewVertices)
{
    ...
    std::iota(std::begin(cell1DsAligned), std::end(cell1DsAligned), 0);
...

    {
        ...
        vtpUtilities.AddSegment(lineOrigin, lineOrigin + lineTangent);
        vtpUtilities.Export(exportFolder + "/" + "direction.vtu");
    }
    ...
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolygon_CheckResult::ResultTypes::Cell2DToBeSplitted, refineCheckResult.ResultType);

    const Gedim::RefinementUtilities::RefinePolygon_Result refineResult =
        refinementUtilities.RefinePolygonCell_ByDirection(cell2DToRefineIndex,
...
TEST(TestRefinementUtilities, TestRefinePolygons_ByArea_MaxDiameter)
{
    ...
    for (unsigned int r = 0; r < maxRefinements; r++)
    {
        ...
        for (unsigned int c = 0; c < cell2DsToRefineIndex.size(); c++)
        {
            ...
            Gedim::RefinementUtilities::PolygonDirection direction = refinementUtilities.ComputePolygonMaxDiameterDirection(
                meshGeometricData.Cell2Ds.UnalignedVertices.at(cell2DToRefineIndex),
            ...
            refinementUtilities.RefinePolygonCell_UpdateGeometricData(
                meshDAO,
            ...
            if (meshDAO.Cell1DTotalNumber() > 0)
            {
                ...
                vtpUtilities.AddSegments(
                    ...
                    {{"Id", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(id.size()), id.data()},
                     {"Active", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(active.size()), active.data()},
                     {"Aligned", Gedim::VTPProperty::Formats::Cells, static_cast<unsigned int>(aligned.size()), aligned.data()}});
                vtpUtilities.Export(exportFolder + "/" + "Mesh_R" + to_string(r) + "_Cell1Ds.vtu");
            }
        }
    }
...
TEST(TestRefinementUtilities, TestRefinePolygons_ByArea_MaxInertia)
{
    ...
    for (unsigned int r = 0; r < maxRefinements; r++)
    {
        ...
        for (unsigned int c = 0; c < cell2DsToRefineIndex.size(); c++)
        {
            ...
            Gedim::RefinementUtilities::PolygonDirection direction = refinementUtilities.ComputePolygonMaxInertiaDirection(
                meshGeometricData.Cell2Ds.UnalignedVertices.at(cell2DToRefineIndex),
    ...
    try
    {
        ...
        for (unsigned int r = 0; r < maxRefinements; r++)
        {
            ...
            for (unsigned int c = 0; c < cell2DsToRefineIndex.size(); c++)
            {
                ...
                const Gedim::RefinementUtilities::RefinePolygon_CheckResult refineCheckResult =
                    refinementUtilities.RefinePolygonCell_CheckRefinement(
                        cell2DToRefineIndex,
                        meshGeometricData.Cell2Ds.Vertices[cell2DToRefineIndex],
                        direction.LineTangent,
                        direction.LineOrigin,
                ...
                const Gedim::RefinementUtilities::RefinePolygon_Result refineResult = refinementUtilities.RefinePolygonCell_ByDirection(
                    cell2DToRefineIndex,
                    Gedim::GeometryUtilities::PolygonTypes::Generic_Convex,
                    Gedim::GeometryUtilities::PolygonTypes::Generic_Convex,
...
TEST(TestRefinementUtilities, TestCheckSplit)
{
...

    {
        ...
        ASSERT_TRUE(refinementUtilities.SplitPolygon_CheckIsNotToExtend(cell1DSplitOne, cell1DSplitTwo));
    }

    {
        ...
        ASSERT_TRUE(refinementUtilities.SplitPolygon_CheckIsNotToExtend(cell1DSplitOne, cell1DSplitTwo));
        ASSERT_TRUE(refinementUtilities.SplitPolygon_CheckIsNotToExtend(cell1DSplitTwo, cell1DSplitOne));
    }
...

    {
        ...
        ASSERT_FALSE(refinementUtilities.SplitPolygon_CheckIsNotToExtend(cell1DSplitOne, cell1DSplitTwo));
        ASSERT_FALSE(refinementUtilities.SplitPolygon_CheckIsNotToExtend(cell1DSplitTwo, cell1DSplitOne));
    }
