...
TEST(TestRefinementUtilities, TestRefineTetrahedrons)
{
    ...
    const Gedim::RefinementUtilities::TetrahedronMaxEdgeDirection direction =
        refinementUtilities.ComputeTetrahedronMaxEdgeDirection(cell3DEdges, meshGeometricData.Cell3DsEdgeLengths.at(cell3DToRefineIndex));
    ...
    const Gedim::RefinementUtilities::RefinePolyhedron_Result result = refinementUtilities.RefinePolyhedronCell_ByPlane(
        cell3DToRefineIndex,
        cell3DVertices,
        cell3DEdges,
        ...
        planeTranslation,
        meshDAO);
    ASSERT_EQ(std::vector<unsigned int>({20}), result.NewCell0DsIndex);
    ASSERT_EQ(3, result.NewCell1DsIndex.size());
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell1D::Types::Updated,
              result.NewCell1DsIndex[0].Type);
    ASSERT_EQ(std::vector<unsigned int>({59, 60}), result.NewCell1DsIndex[0].NewCell1DsIndex);
    ASSERT_EQ(2, result.NewCell1DsIndex[0].OriginalCell1DIndex);
    ASSERT_EQ(20, result.NewCell1DsIndex[0].NewCell0DIndex);
    ASSERT_EQ(2, result.NewCell1DsIndex[0].OriginalCell3DEdgeIndex);
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell1D::Types::New, result.NewCell1DsIndex[1].Type);
    ASSERT_EQ(std::vector<unsigned int>({61}), result.NewCell1DsIndex[1].NewCell1DsIndex);
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell1D::Types::New, result.NewCell1DsIndex[2].Type);
    ASSERT_EQ(std::vector<unsigned int>({62}), result.NewCell1DsIndex[2].NewCell1DsIndex);
    ASSERT_EQ(3, result.NewCell2DsIndex.size());
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell2D::Types::Updated,
              result.NewCell2DsIndex[0].Type);
    ASSERT_EQ(std::vector<unsigned int>({62, 63}), result.NewCell2DsIndex[0].NewCell2DsIndex);
    ASSERT_EQ(2, result.NewCell2DsIndex[0].OriginalCell2DIndex);
    ASSERT_EQ(61, result.NewCell2DsIndex[0].NewCell1DIndex);
    ASSERT_EQ(3, result.NewCell2DsIndex[0].OriginalCell3DFaceIndex);
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell2D::Types::Updated,
              result.NewCell2DsIndex[1].Type);
    ASSERT_EQ(std::vector<unsigned int>({64, 65}), result.NewCell2DsIndex[1].NewCell2DsIndex);
    ASSERT_EQ(3, result.NewCell2DsIndex[1].OriginalCell2DIndex);
    ASSERT_EQ(62, result.NewCell2DsIndex[1].NewCell1DIndex);
    ASSERT_EQ(0, result.NewCell2DsIndex[1].OriginalCell3DFaceIndex);
    ASSERT_EQ(Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell2D::Types::New, result.NewCell2DsIndex[2].Type);
    ASSERT_EQ(std::vector<unsigned int>({66}), result.NewCell2DsIndex[2].NewCell2DsIndex);
    ASSERT_EQ(std::vector<unsigned int>({22, 23}), result.NewCell3DsIndex);
    ...
    for (unsigned int f = 0; f < result.NewCell2DsIndex.size(); f++)
    {
        ...
        for (unsigned int e = 0; e < result.NewCell2DsIndex[f].NewCell1DsPosition.size(); e++)
        {
            ...
            if (refinedCell1D.Type != Gedim::RefinementUtilities::RefinePolyhedron_Result::RefinedCell1D::Types::Updated)
                continue;
        ...
        const Gedim::RefinementUtilities::RefinePolyhedron_UpdateNeighbour_Result updateResult =
            refinementUtilities.RefinePolyhedronCell_UpdateFaceNeighbours(
                cell3DToRefineIndex,
                cell2DIndex,
                result.NewCell2DsIndex[f].NewCell1DIndex,
                std::vector<unsigned int>(splitCell1DsOriginalIndex.begin(), splitCell1DsOriginalIndex.end()),
                std::vector<unsigned int>(splitCell1DsNewCell0DIndex.begin(), splitCell1DsNewCell0DIndex.end()),
                std::vector<std::vector<unsigned int>>(splitCell1DsUpdatedIndices.begin(), splitCell1DsUpdatedIndices.end()),
    ...
    for (unsigned int e = 0; e < result.NewCell1DsIndex.size(); e++)
    {
        ...
        const Gedim::RefinementUtilities::RefinePolyhedron_UpdateNeighbour_Result updateResult =
            refinementUtilities.RefinePolyhedronCell_UpdateEdgeNeighbours(cell3DToRefineIndex,
                                                                          cell1DIndex,
...
TEST(TestRefinementUtilities, TestRefineTetrahedrons_ByVolume)
{
    ...
    std::iota(std::begin(cell2DsAligned), std::end(cell2DsAligned), 1);
    unsigned int maxCell2DAligned = meshDAO.Cell2DTotalNumber();

    for (unsigned int r = 0; r < maxRefinements; r++)
    {
        Gedim::MeshUtilities::MeshGeometricData3D meshGeometricData =
        ...
        {
            meshGeometricData = meshUtilities.FillMesh3DGeometricData(geometryUtilities, meshDAO);

            std::list<unsigned int> updatedCell3Ds;
            meshDAO.Cell3DUpdatedCell3Ds(cell3DsToRefineIndex[c], updatedCell3Ds);

            if (updatedCell3Ds.size() > 1)
                continue;

            const unsigned int cell3DToRefineIndex = updatedCell3Ds.size() == 0 ? cell3DsToRefineIndex[c]
            ...
            const std::vector<std::vector<unsigned int>> facesUnalignedPoints =
                geometryUtilities.PolyhedronFacesUnalignedVertices(meshGeometricData.Cell3DsFaces2DVertices.at(cell3DToRefineIndex));
            ...
            for (unsigned int f = 0; f < cell3DFaces.size(); f++)
            {
                ...
                if (polyhedronUnaligedFacesList.find(cell2DAligned) == polyhedronUnaligedFacesList.end())
                {
                    polyhedronUnaligedFacesList.insert(std::make_pair(cell2DAligned, std::list<unsigned int>()));
                }
            ...
            const std::vector<unsigned int> unalignedVertices = geometryUtilities.UnalignedPolyhedronPoints(
                cell3DVertices,
                cell3DFaces,
                meshGeometricData.Cell3DsFacesTranslations.at(cell3DToRefineIndex),
                meshGeometricData.Cell3DsFacesRotationMatrices.at(cell3DToRefineIndex),
            ...
            const Gedim::GeometryUtilities::Polyhedron unalignedTetrahedron =
                geometryUtilities.CreateTetrahedronWithVertices(cell3DVertices.col(unalignedVertices.at(0)),
...

            {
                ...
                exporter.AddPolyhedron(unalignedTetrahedron.Vertices, unalignedTetrahedron.Edges, unalignedTetrahedron.Faces);
    ...
    EXPECT_EQ(92, meshDAO.Cell0DTotalNumber());
    EXPECT_EQ(348, meshDAO.Cell1DTotalNumber());
    EXPECT_EQ(420, meshDAO.Cell2DTotalNumber());
    EXPECT_EQ(163, meshDAO.Cell3DTotalNumber());
}
} // namespace GedimUnitTesting
