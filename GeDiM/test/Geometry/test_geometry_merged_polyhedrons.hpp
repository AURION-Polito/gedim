...
TEST(TestGeometryUtilities, Test_MergePolyhedrons_no_intersections)
{
    ...
    const std::array<Gedim::GeometryUtilities::Polyhedron, 2> polyhedrons = {polyhedron_one, polyhedron_two};
    ...
    ASSERT_EQ(merged_polyhedron.MergedToOriginalEdges.size(), polyhedrons[0].Edges.cols() + polyhedrons[1].Edges.cols());
    ASSERT_EQ(merged_polyhedron.MergedToOriginalFaces.size(), polyhedrons[0].Faces.size() + polyhedrons[1].Faces.size());
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Vertices.cols(),
              polyhedrons[0].Vertices.cols() + polyhedrons[1].Vertices.cols());
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Edges.cols(), polyhedrons[0].Edges.cols() + polyhedrons[1].Edges.cols());
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Faces.size(), polyhedrons[0].Faces.size() + polyhedrons[1].Faces.size());
    for (unsigned int p = 0; p < polyhedrons.size(); ++p)
    {
        const auto &polyhedron = polyhedrons[p];
        ASSERT_EQ(merged_polyhedron.OriginalToMergedVertices[p].size(), polyhedron.Vertices.cols());
        ASSERT_EQ(merged_polyhedron.OriginalToMergedEdges[p].size(), polyhedron.Edges.cols());
        ASSERT_EQ(merged_polyhedron.OriginalToMergedFaces[p].size(), polyhedron.Faces.size());

        const unsigned int shift_vertices = (p == 0) ? 0 : polyhedrons[p - 1].Vertices.cols();
        const unsigned int shift_edges = (p == 0) ? 0 : polyhedrons[p - 1].Edges.cols();
        const unsigned int shift_faces = (p == 0) ? 0 : polyhedrons[p - 1].Faces.size();

        for (unsigned int v = 0; v < polyhedron.Vertices.cols(); ++v)
        {
            ASSERT_EQ(shift_vertices + v, merged_polyhedron.OriginalToMergedVertices[p][v]);
            ASSERT_EQ(v, merged_polyhedron.MergedToOriginalVertices[shift_vertices + v][p]);
        }
        for (unsigned int e = 0; e < polyhedron.Edges.cols(); ++e)
        {
            ASSERT_EQ(shift_edges + e, merged_polyhedron.OriginalToMergedEdges[p][e]);
            ASSERT_EQ(e, merged_polyhedron.MergedToOriginalEdges[shift_edges + e][p]);
        }
        for (unsigned int f = 0; f < polyhedron.Faces.size(); ++f)
        {
            ASSERT_EQ(shift_faces + f, merged_polyhedron.OriginalToMergedFaces[p][f]);
            ASSERT_EQ(f, merged_polyhedron.MergedToOriginalFaces[shift_faces + f][p]);
        }
    }
}
...
TEST(TestGeometryUtilities, Test_MergePolyhedrons_face_intersections)
{
    ...
    const auto polyhedron_one = geometryUtilities.CreateCubeWithOrigin(Eigen::Vector3d(0.0, 0.0, 0.0), 1.0);
    const auto polyhedron_two = geometryUtilities.CreateCubeWithOrigin(Eigen::Vector3d(0.0, 0.0, 1.0), 1.0);
    const std::array<Gedim::GeometryUtilities::Polyhedron, 2> polyhedrons = {polyhedron_one, polyhedron_two};

    const auto merged_polyhedron_input = geometryUtilities.MergePolyhedronByFace(polyhedrons, {1, 0}, true);
    ...
    for (unsigned int p = 0; p < polyhedrons.size(); ++p)
    {
        ...
        ASSERT_EQ(merged_polyhedron_input.Vertices_Type[p].size(), polyhedron.Vertices.cols());
        ASSERT_EQ(merged_polyhedron_input.Edges_Type[p].size(), polyhedron.Edges.cols());
        ASSERT_EQ(merged_polyhedron_input.Faces_Type[p].size(), polyhedron.Faces.size());
    ...
    ASSERT_EQ(merged_polyhedron_input.Common_vertices, expected_common_vertices);
    ASSERT_EQ(merged_polyhedron_input.Common_edges, expected_common_edges);
    ASSERT_EQ(merged_polyhedron_input.Faces_Type[0][1],
              std::make_pair(Gedim::GeometryUtilities::MergePolyhedronsInput::MergeTypes::Remove,
                             Gedim::GeometryUtilities::MergePolyhedronsInput::none));
    ASSERT_EQ(merged_polyhedron_input.Faces_Type[1][0],
              std::make_pair(Gedim::GeometryUtilities::MergePolyhedronsInput::MergeTypes::Remove,
                             Gedim::GeometryUtilities::MergePolyhedronsInput::none));

    const auto merged_polyhedron = geometryUtilities.MergePolyhedrons(polyhedrons, merged_polyhedron_input);
    ...
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Edges.cols(),
              polyhedrons[0].Edges.cols() + polyhedrons[1].Edges.cols() - merged_polyhedron_input.Common_edges.size());
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Faces.size(), polyhedrons[0].Faces.size() + polyhedrons[1].Faces.size() - 2);
    ...
    ASSERT_EQ(merged_polyhedron.OriginalToMergedVertices, expected_original_to_merged_vertices);
    ASSERT_EQ(merged_polyhedron.OriginalToMergedEdges, expected_original_to_merged_edges);
    ASSERT_EQ(merged_polyhedron.OriginalToMergedFaces, expected_original_to_merged_faces);
    ...
    ASSERT_EQ(merged_polyhedron.MergedToOriginalVertices, expected_merged_to_original_vertices);
    ASSERT_EQ(merged_polyhedron.MergedToOriginalEdges, expected_merged_to_original_edges);
    ASSERT_EQ(merged_polyhedron.MergedToOriginalFaces, expected_merged_to_original_faces);
}
...
TEST(TestGeometryUtilities, Test_MergePolyhedrons_edge_removed)
{
    ...
    merged_polyhedrons_input.Vertices_Type[0] = {{merge_input_type::MergeTypes::Common, 0},
                                                 ...
                                                 {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                                 {merge_input_type::MergeTypes::Remove, merge_input_type::none}};
    merged_polyhedrons_input.Vertices_Type[1] = {{merge_input_type::MergeTypes::Common, 0},
                                                 {merge_input_type::MergeTypes::Common, 1},
                                                 {merge_input_type::MergeTypes::Common, 2},
                                                 {merge_input_type::MergeTypes::Common, 3}};
    merged_polyhedrons_input.Common_vertices = {{0, 0}, {1, 1}, {3, 2}, {4, 3}};

    merged_polyhedrons_input.Edges_Type[0] = {{merge_input_type::MergeTypes::Common, 0},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Common, 1},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              ...
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none}};
    merged_polyhedrons_input.Edges_Type[1] = {{merge_input_type::MergeTypes::Common, 0},
                                              {merge_input_type::MergeTypes::Common, 1},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Common, 2},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none}};
    merged_polyhedrons_input.Common_edges = {{0, 0}, {3, 1}, {8, 3}};

    merged_polyhedrons_input.Faces_Type[0] = {{merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none},
                                              {merge_input_type::MergeTypes::Remove, merge_input_type::none}};
    merged_polyhedrons_input.Faces_Type[1] = {{merge_input_type::MergeTypes::None, merge_input_type::none},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none},
                                              {merge_input_type::MergeTypes::None, merge_input_type::none}};
    merged_polyhedrons_input.Common_faces = {};

    const auto merged_polyhedron = geometryUtilities.MergePolyhedrons(polyhedrons, merged_polyhedrons_input);
    ...
    ASSERT_EQ(merged_polyhedron.MergedPolyhedron.Faces.size(), 4);
    ...
    expected_original_to_merged_vertices[1] = {0u, 1u, 2u, 3u};

    std::array<std::vector<unsigned int>, 2> expected_original_to_merged_edges;
    expected_original_to_merged_edges[0] = {0u,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            1u,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            ...
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none};
    expected_original_to_merged_edges[1] = {0u, 1u, 3u, 2u, 4u, 5u};

    std::array<std::vector<unsigned int>, 2> expected_original_to_merged_faces;
    expected_original_to_merged_faces[0] = {Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none,
                                            Gedim::GeometryUtilities::MergePolyhedronsResult::none};
    expected_original_to_merged_faces[1] = {0u, 1u, 2u, 3u};

    ASSERT_EQ(merged_polyhedron.OriginalToMergedVertices, expected_original_to_merged_vertices);
    ASSERT_EQ(merged_polyhedron.OriginalToMergedEdges, expected_original_to_merged_edges);
    ASSERT_EQ(merged_polyhedron.OriginalToMergedFaces, expected_original_to_merged_faces);

    const std::vector<std::array<unsigned int, 2>> expected_merged_to_original_vertices = {{0u, 0u}, {1u, 1u}, {3u, 2u}, {4u, 3u}};
    const std::vector<std::array<unsigned int, 2>> expected_merged_to_original_edges = {
        {0u, 0u},
        {3u, 1u},
        {8u, 3u},
