...
TEST(TestGraphUtilities, TestGraphConnectivityAndGraphAdjacency)
{
    ...
    const Gedim::GraphUtilities::GraphAdjacencyData graph_adjacency =
        graphUtilities.GraphConnectivityToGraphAdjacency(graph_numVertices, graph_connectivity_expected);
    ...
    const Eigen::MatrixXi graph_connectivity =
        graphUtilities.GraphAdjacencyToGraphConnectivity(graph_numEdges, graph_adjacency.GraphAdjacencyVertices);
...
TEST(TestGraphUtilities, TestExtractSubGraph)
{
    ...
    const std::vector<std::vector<unsigned int>> subGraph_adjacency =
        graphUtilities.ExtractSubGraph(graph_adjacency.GraphAdjacencyVertices, graph_filter);
...
TEST(TestGraphUtilities, TestStronglyConnectedComponents)
{
    ...
    const Gedim::GraphUtilities::GraphAdjacencyData graph_adjacency =
        graphUtilities.GraphConnectivityToGraphAdjacency(graph_numVertices, graph_connectivity);

    const std::vector<std::vector<unsigned int>> stronglyConnectedComponents =
        graphUtilities.ComputeStronglyConnectedComponents(graph_adjacency.GraphAdjacencyVertices);
...
TEST(TestGraphUtilities, TestBreadthFirstSearch)
{
    ...
    const Gedim::GraphUtilities::GraphAdjacencyData graph_adjacency =
        graphUtilities.GraphConnectivityToGraphAdjacency(graph_numVertices, graph_connectivity_expected);
    const std::vector<unsigned int> visitedVertices = graphUtilities.BreadthFirstSearch(2, graph_adjacency.GraphAdjacencyVertices);

    ASSERT_EQ(std::vector<unsigned int>({2, 1, 4, 3, 0}), visitedVertices);
}
// ***************************************************************************
} // namespace UnitTesting
