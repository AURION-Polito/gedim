  ...
  TEST(TestGeometryUtilities, TestSplitPolygonWithCircle)
  {
    try
    {
      ...
      // Circle Inside Polygon Intersects With Multiple SubPolygons
      {
        ...
        ASSERT_EQ(result.NewEdges[9].VertexIndices, vector<unsigned int>({ 2, 5 }));
        ASSERT_EQ(result.NewEdges[11].Type, Gedim::GeometryUtilities::SplitPolygonWithCircleResult::NewEdge::Types::Arc);
        ASSERT_EQ(result.NewEdges[11].ArcType, Gedim::GeometryUtilities::SplitPolygonWithCircleResult::NewEdge::ArcTypes::InsidePolygon);
        ASSERT_EQ(result.NewEdges[11].VertexIndices, vector<unsigned int>({ 2, 5 }));
